...
use slashstep_server::resources::status::{
    DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, InitialStatusProperties, Status,
};
...
fn assert_status_is_equal_to_initial_properties(
    status: &Status,
    initial_properties: &InitialStatusProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    for _ in 0..MAXIMUM_RESOURCE_COUNT {
        let resource = test_environment
            .create_random_status(Some(&project_id))
            .await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Status::get_by_id(&created_status.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    for index in 0..created_resources.len() {
        created_resources[index] = Status::get_by_id(
            &created_resources[index].id,
    ...
    let retrieved_resources =
        Status::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Status> = created_resources
        .iter()
        .filter(|status| status.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let mut current_resources =
        Status::list("", &test_environment.database_pool, None, None).await?;
    if current_resources.len() < MINIMUM_RESOURCE_COUNT as usize {
        ...
        for index in 0..current_resources.len() {
            current_resources[index] = Status::get_by_id(
                &current_resources[index].id,
                &test_environment.database_pool,
            )
            .await?;
        }
    ...
    let retrieved_resources = Status::list(
        "",
        &test_environment.database_pool,
