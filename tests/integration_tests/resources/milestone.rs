...
fn assert_milestones_are_equal(milestone_1: &Milestone, milestone_2: &Milestone) {
    ...
    assert_eq!(
        milestone_1
            .start_date
            .and_then(|start_date| DateTime::from_timestamp_millis(start_date.timestamp_millis())),
        milestone_2
            .start_date
            .and_then(|start_date| DateTime::from_timestamp_millis(start_date.timestamp_millis()))
    );
    assert_eq!(
        milestone_1
            .end_date
            .and_then(|end_date| DateTime::from_timestamp_millis(end_date.timestamp_millis())),
        milestone_2
            .end_date
            .and_then(|end_date| DateTime::from_timestamp_millis(end_date.timestamp_millis()))
    );
    assert_eq!(
        milestone_1.parent_resource_type,
        milestone_2.parent_resource_type
    );
    assert_eq!(
        milestone_1.parent_workspace_id,
        milestone_2.parent_workspace_id
    );
    assert_eq!(milestone_1.parent_project_id, milestone_2.parent_project_id);
}

fn assert_milestone_is_equal_to_initial_properties(
    milestone: &Milestone,
    initial_properties: &InitialMilestoneProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource_count =
        Milestone::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let milestone =
        Milestone::create(&milestone_properties, &test_environment.database_pool).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Milestone::get_by_id(&created_milestone.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Milestone::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Milestone> = created_resources
        .iter()
        .filter(|milestone| milestone.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for created_milestone in &created_resources {
        let retrieved_milestone_option = retrieved_resources
            .iter()
            .find(|retrieved_milestone| retrieved_milestone.id == created_milestone.id);
        assert!(retrieved_milestone_option.is_some());
    }
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Get the "milestones.get" action one time.
    ...
    // Grant access to the "milestones.get" action to the user for half of the actions.
    ...
    let retrieved_resources = Milestone::list(
        "",
        &test_environment.database_pool,
