use slashstep_server::resources::view_field::{
...
fn assert_view_fields_are_equal(view_field_1: &ViewField, view_field_2: &ViewField) {
    ...
    assert_eq!(
        view_field_1.next_view_field_id,
        view_field_2.next_view_field_id
    );
}

fn assert_view_field_is_equal_to_initial_properties(
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let view_field =
        ViewField::create(&view_field_properties, &test_environment.database_pool).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        ViewField::get_by_id(&created_view_field.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    for index in 0..created_resources.len() {
        created_resources[index] = ViewField::get_by_id(
    ...
    let retrieved_resources =
        ViewField::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&ViewField> = created_resources
        .iter()
        .filter(|view_field| view_field.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    if current_resources.len() < MINIMUM_RESOURCE_COUNT as usize {
        ...
        for index in 0..current_resources.len() {
            current_resources[index] = ViewField::get_by_id(
    ...
    let retrieved_resources = ViewField::list(
        "",
        &test_environment.database_pool,
