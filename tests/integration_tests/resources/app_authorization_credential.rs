...
fn assert_app_authorization_credentials_are_equal(
    app_authorization_credential_1: &AppAuthorizationCredential,
    app_authorization_credential_2: &AppAuthorizationCredential,
) {
    assert_eq!(
        app_authorization_credential_1.id,
        app_authorization_credential_2.id
    );
    assert_eq!(
        app_authorization_credential_1.app_authorization_id,
        app_authorization_credential_2.app_authorization_id
    );
    assert_eq!(
        app_authorization_credential_1.access_token_expiration_date,
        app_authorization_credential_2.access_token_expiration_date
    );
    assert_eq!(
        app_authorization_credential_1.refresh_token_expiration_date,
        app_authorization_credential_2.refresh_token_expiration_date
    );
    assert_eq!(
        app_authorization_credential_1.refreshed_app_authorization_credential_id,
        app_authorization_credential_2.refreshed_app_authorization_credential_id
    );
}

fn assert_app_authorization_is_equal_to_initial_properties(
    app_authorization_credential: &AppAuthorizationCredential,
    initial_properties: &InitialAppAuthorizationCredentialProperties,
) {
    assert_eq!(
    ...
    assert_eq!(
        ...
        .expect("Failed to parse access token expiration date.")
    );
    assert_eq!(
        ...
        .expect("Failed to parse refresh token expiration date.")
    );
    assert_eq!(
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_app_authorization_credential_count =
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    assert_app_authorization_credentials_are_equal(
        &created_app_authorization_credential,
        &retrieved_app_authorization_credential,
    );
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_app_authorization_credentials =
        AppAuthorizationCredential::list(&query, &test_environment.database_pool, None, None)
            .await?;

    let created_app_authorization_credentials_with_specific_app_id: Vec<
        &AppAuthorizationCredential,
    > = created_app_authorization_credentials
        .iter()
        .filter(|app_authorization_credential| {
            app_authorization_credential.app_authorization_id
                == created_app_authorization_credentials[0].app_authorization_id
        })
        .collect();
    assert_eq!(
        created_app_authorization_credentials_with_specific_app_id.len(),
        retrieved_app_authorization_credentials.len()
    );
    for i in 0..created_app_authorization_credentials_with_specific_app_id.len() {
        let created_app_authorization =
            &created_app_authorization_credentials_with_specific_app_id[i];
...
-> Result<(), TestSlashstepServerError> {
    ...
    let get_app_authorization_credentials_action = Action::get_by_name(
    ...
    for index in 0..allowed_action_count {
        ...
        AccessPolicy::create(
            &InitialAccessPolicyProperties {
                ...
                scoped_resource_type: ResourceType::AppAuthorizationCredential,
                scoped_app_authorization_credential_id: Some(
                    scoped_app_authorization_credential.id.clone(),
                ),
                ..Default::default()
            },
            &test_environment.database_pool,
    ...
    let retrieved_app_authorization_credentials = AppAuthorizationCredential::list(
        "",
        &test_environment.database_pool,
    ...
    assert_eq!(
        allowed_app_authorization_credentials.len(),
        retrieved_app_authorization_credentials.len()
    );
    for allowed_app_authorization in allowed_app_authorization_credentials {
        let retrieved_app_authorization = &retrieved_app_authorization_credentials
            .iter()
