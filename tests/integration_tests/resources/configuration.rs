...
fn assert_configuration_is_equal_to_initial_properties(
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let configuration =
        Configuration::create(&configuration_properties, &test_environment.database_pool).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Configuration::get_by_id(&created_configuration.id, &test_environment.database_pool)
            .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Configuration::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Configuration> = created_resources
        .iter()
        .filter(|configuration| configuration.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources = Configuration::list(
        "",
        &test_environment.database_pool,
