...
fn assert_password_reset_authorizations_are_equal(
    password_reset_authorization_1: &PasswordResetAuthorization,
    password_reset_authorization_2: &PasswordResetAuthorization,
) {
    assert_eq!(
        password_reset_authorization_1.id,
        password_reset_authorization_2.id
    );
    assert_eq!(
        password_reset_authorization_1.user_id,
        password_reset_authorization_2.user_id
    );
    assert_eq!(
        password_reset_authorization_1
            .expiration_date
            .timestamp_millis(),
        password_reset_authorization_2
            .expiration_date
            .timestamp_millis()
    );
}

fn assert_app_authorization_is_equal_to_initial_properties(
    password_reset_authorization: &PasswordResetAuthorization,
    initial_properties: &InitialPasswordResetAuthorizationProperties,
) {
    assert_eq!(
        password_reset_authorization.user_id,
        initial_properties.user_id
    );
    assert_eq!(
        password_reset_authorization.expiration_date,
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    for _ in 0..MAXIMUM_PASSWORD_RESET_AUTHORIZATION_COUNT {
        let password_reset_authorization = test_environment
            .create_random_password_reset_authorization(None)
            .await?;
        created_password_reset_authorizations.push(password_reset_authorization);
    }

    let retrieved_password_reset_authorization_count =
        PasswordResetAuthorization::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    assert_app_authorization_is_equal_to_initial_properties(
        &password_reset_authorization,
        &password_reset_authorization_properties,
    );
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    if let Err(error) = PasswordResetAuthorization::get_by_id(
        &created_app_authorization.id,
        &test_environment.database_pool,
    )
    .await
    {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_password_reset_authorization = test_environment
        .create_random_password_reset_authorization(None)
        .await?;
    let retrieved_password_reset_authorization = PasswordResetAuthorization::get_by_id(
        &created_password_reset_authorization.id,
        &test_environment.database_pool,
    )
    .await?;
    assert_password_reset_authorizations_are_equal(
        &created_password_reset_authorization,
        &retrieved_password_reset_authorization,
    );
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_password_reset_authorizations =
        PasswordResetAuthorization::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let app_authorization_with_same_user_id = test_environment
        .create_random_password_reset_authorization(Some(
            &created_password_reset_authorizations[0].user_id,
        ))
        .await?;
    ...
    let retrieved_password_reset_authorizations =
        PasswordResetAuthorization::list(&query, &test_environment.database_pool, None, None)
            .await?;

    let created_password_reset_authorizations_with_specific_user_id: Vec<
        &PasswordResetAuthorization,
    > = created_password_reset_authorizations
        .iter()
        .filter(|password_reset_authorization| {
            password_reset_authorization.user_id == created_password_reset_authorizations[0].user_id
        })
        .collect();
    assert_eq!(
        created_password_reset_authorizations_with_specific_user_id.len(),
        retrieved_password_reset_authorizations.len()
    );
    for i in 0..created_password_reset_authorizations_with_specific_user_id.len() {
        let created_password_reset_authorization =
            &created_password_reset_authorizations_with_specific_user_id[i];
...
-> Result<(), TestSlashstepServerError> {
    ...
    let mut current_password_reset_authorizations =
        PasswordResetAuthorization::list("", &test_environment.database_pool, None, None).await?;
    if current_password_reset_authorizations.len() < MINIMUM_ACTION_COUNT as usize {
        let remaining_action_count =
            MINIMUM_ACTION_COUNT - current_password_reset_authorizations.len() as i32;
    ...
    let user = test_environment.create_random_user(None).await?;
    let get_password_reset_authorizations_action = Action::get_by_name(
        "passwordResetAuthorizations.get",
        &test_environment.database_pool,
    )
    .await?;
    ...
    for index in 0..allowed_action_count {
        ...
        AccessPolicy::create(
            &InitialAccessPolicyProperties {
                ...
                scoped_password_reset_authorization_id: Some(
                    scoped_password_reset_authorization.id.clone(),
                ),
                ..Default::default()
            },
            &test_environment.database_pool,
    ...
    let retrieved_password_reset_authorizations = PasswordResetAuthorization::list(
        "",
        &test_environment.database_pool,
    ...
    assert_eq!(
        allowed_password_reset_authorizations.len(),
        retrieved_password_reset_authorizations.len()
    );
    for allowed_app_authorization in allowed_password_reset_authorizations {
        let retrieved_app_authorization = &retrieved_password_reset_authorizations
            .iter()
            .find(|action| action.id == allowed_app_authorization.id)
            .unwrap();

        assert_password_reset_authorizations_are_equal(
            &allowed_app_authorization,
