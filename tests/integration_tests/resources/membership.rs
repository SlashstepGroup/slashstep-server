use slashstep_server::resources::membership::{
    DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, InitialMembershipProperties, Membership,
};
...
fn assert_membership_types_are_equal(membership_1: &Membership, membership_2: &Membership) {
    ...
    assert_eq!(
        membership_1.parent_resource_type,
        membership_2.parent_resource_type
    );
    ...
    assert_eq!(
        membership_1.principal_user_id,
        membership_2.principal_user_id
    );
    assert_eq!(
        membership_1.principal_group_id,
        membership_2.principal_group_id
    );
    assert_eq!(membership_1.principal_app_id, membership_2.principal_app_id);
}

fn assert_membership_is_equal_to_initial_properties(
    membership: &Membership,
    initial_properties: &InitialMembershipProperties,
) {
    assert_eq!(
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let membership =
        Membership::create(&membership_properties, &test_environment.database_pool).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Membership::get_by_id(&created_membership.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Membership::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Membership::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Membership> = created_resources
        .iter()
        .filter(|membership| membership.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for created_membership in &created_resources {
        let retrieved_membership_option = retrieved_resources
            .iter()
            .find(|retrieved_membership| retrieved_membership.id == created_membership.id);
        assert!(retrieved_membership_option.is_some());
    }
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Get the "membership_types.get" action one time.
    let user = test_environment.create_random_user(None).await?;
    let get_membership_types_action =
        Action::get_by_name(GET_RESOURCE_ACTION_NAME, &test_environment.database_pool).await?;

    // Grant access to the "membership_types.get" action to the user for half of the actions.
    ...
    let retrieved_resources = Membership::list(
        "",
        &test_environment.database_pool,
