...
use slashstep_server::resources::project::{
    DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, InitialProjectProperties, Project,
};
...
fn assert_project_is_equal_to_initial_properties(
    project: &Project,
    initial_properties: &InitialProjectProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let initial_resource_count =
        Project::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Project::get_by_id(&created_project.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Project::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Project::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Project> = created_resources
        .iter()
        .filter(|project| project.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for created_project in &created_resources {
        let retrieved_project_option = retrieved_resources
            .iter()
            .find(|retrieved_project| retrieved_project.id == created_project.id);
        assert!(retrieved_project_option.is_some());
    }
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources = Project::list(
        "",
        &test_environment.database_pool,
