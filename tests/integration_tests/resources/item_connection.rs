use slashstep_server::resources::item_connection::{
...
fn assert_item_connections_are_equal(
    item_connection_1: &ItemConnection,
    item_connection_2: &ItemConnection,
) {
    assert_eq!(item_connection_1.id, item_connection_2.id);
    assert_eq!(
        item_connection_1.item_connection_type_id,
        item_connection_2.item_connection_type_id
    );
    assert_eq!(
        item_connection_1.inward_item_id,
        item_connection_2.inward_item_id
    );
    assert_eq!(
        item_connection_1.outward_item_id,
        item_connection_2.outward_item_id
    );
}

fn assert_item_connection_is_equal_to_initial_properties(
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource_count =
        ItemConnection::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let item_connection =
        ItemConnection::create(&item_connection_properties, &test_environment.database_pool)
            .await?;
    assert_item_connection_is_equal_to_initial_properties(
        &item_connection,
        &item_connection_properties,
    );
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    created_item_connection
        .delete(&test_environment.database_pool)
        .await?;

    // Ensure that the access policy is no longer in the database.
    match ItemConnection::get_by_id(&created_item_connection.id, &test_environment.database_pool)
        .await
    {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        ItemConnection::get_by_id(&created_item_connection.id, &test_environment.database_pool)
            .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        ItemConnection::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&ItemConnection> = created_resources
        .iter()
        .filter(|item_connection| item_connection.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources = ItemConnection::list(
        "",
        &test_environment.database_pool,
