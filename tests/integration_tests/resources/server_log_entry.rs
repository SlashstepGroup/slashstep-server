...
use slashstep_server::resources::server_log_entry::{
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource_count =
        ServerLogEntry::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let server_log_entry = ServerLogEntry::create(
        &server_log_entry_properties,
        &test_environment.database_pool,
    )
    .await?;
    assert_server_log_entry_is_equal_to_initial_properties(
        &server_log_entry,
        &server_log_entry_properties,
    );
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_server_log_entry = test_environment.create_random_server_log_entry().await?;
    let retrieved_resource = ServerLogEntry::get_by_id(
        &created_server_log_entry.id,
        &test_environment.database_pool,
    )
    .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        ServerLogEntry::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&ServerLogEntry> = created_resources
        .iter()
        .filter(|server_log_entry| server_log_entry.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    for index in 0..allowed_resource_count {
        ...
        AccessPolicy::create(
            &InitialAccessPolicyProperties {
                ...
                scoped_resource_type: ResourceType::ServerLogEntry,
    ...
    let retrieved_resources = ServerLogEntry::list(
        "",
        &test_environment.database_pool,
