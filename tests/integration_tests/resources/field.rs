...
fn assert_field_is_equal_to_initial_properties(
    field: &Field,
    initial_properties: &InitialFieldProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    assert_eq!(
        ...
        MAXIMUM_FIELD_COUNT + initial_resource_count
    );
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_field = test_environment.create_random_field(None).await?;
    let retrieved_field =
        Field::get_by_id(&created_field.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let created_fields_with_specific_id: Vec<&Field> = created_fields
        .iter()
        .filter(|field| field.id == created_fields[0].id)
        .collect();
    assert_eq!(
        created_fields_with_specific_id.len(),
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for created_field in &created_fields {
        let retrieved_field_option = retrieved_fields
            .iter()
            .find(|retrieved_field| retrieved_field.id == created_field.id);
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_fields = Field::list(
        "",
        &test_environment.database_pool,
