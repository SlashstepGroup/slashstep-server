...
fn assert_workspace_is_equal_to_initial_properties(
    workspace: &Workspace,
    initial_properties: &InitialWorkspaceProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource_count =
        Workspace::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let workspace =
        Workspace::create(&workspace_properties, &test_environment.database_pool).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Workspace::get_by_id(&created_workspace.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Workspace::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Workspace> = created_resources
        .iter()
        .filter(|workspace| workspace.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Get the "workspaces.get" action one time.
    ...
    // Grant access to the "workspaces.get" action to the user for half of the actions.
    ...
    let retrieved_resources = Workspace::list(
        "",
        &test_environment.database_pool,
