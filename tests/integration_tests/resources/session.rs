...
fn assert_session_is_equal_to_initial_properties(
    session: &Session,
    ...
    assert_eq!(
        ...
        DateTime::from_timestamp_millis(initial_properties.expiration_date.timestamp_millis())
            .expect("Expected the expiration date to be a valid DateTime.")
    );
    assert_eq!(
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource_count =
        Session::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Session::get_by_id(&created_session.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Session::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Session> = created_resources
        .iter()
        .filter(|session| session.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let get_server_log_entries_action =
    ...
    let retrieved_resources = Session::list(
        "",
        &test_environment.database_pool,
