...
fn assert_server_log_entries_are_equal(user_1: &User, user_2: &User) {
    ...
    assert_eq!(user_1.get_ip_address(), user_2.get_ip_address());
    assert_eq!(user_1.is_anonymous, user_2.is_anonymous);
}

fn assert_user_is_equal_to_initial_properties(
    user: &User,
    ...
    assert_eq!(user.get_ip_address(), initial_properties.ip_address);
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let initial_resource_count =
        User::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        User::get_by_id(&created_user.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources = User::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        User::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&User> = created_resources
        .iter()
        .filter(|user| user.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for created_user in &created_resources {
        let retrieved_user_option = retrieved_resources
            .iter()
            .find(|retrieved_user| retrieved_user.id == created_user.id);
        assert!(retrieved_user_option.is_some());
    }
...
-> Result<(), TestSlashstepServerError> {
    ...
    let mut current_resources = User::list("", &test_environment.database_pool, None, None).await?;
    ...
    let retrieved_resources = User::list(
        "",
        &test_environment.database_pool,
