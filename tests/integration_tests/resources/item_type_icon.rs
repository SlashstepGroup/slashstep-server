...
use slashstep_server::resources::item_type_icon::{
...
fn assert_item_type_icons_are_equal(
    item_type_icon_1: &ItemTypeIcon,
    item_type_icon_2: &ItemTypeIcon,
) {
    assert_eq!(item_type_icon_1.id, item_type_icon_2.id);
    assert_eq!(item_type_icon_1.display_name, item_type_icon_2.display_name);
    assert_eq!(
        item_type_icon_1.parent_resource_type,
        item_type_icon_2.parent_resource_type
    );
    assert_eq!(
        item_type_icon_1.parent_project_id,
        item_type_icon_2.parent_project_id
    );
}

fn assert_item_type_icon_is_equal_to_initial_properties(
    item_type_icon: &ItemTypeIcon,
    initial_properties: &InitialItemTypeIconProperties,
) {
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let item_type_icon =
        ItemTypeIcon::create(&item_type_icon_properties, &test_environment.database_pool).await?;
    assert_item_type_icon_is_equal_to_initial_properties(
        &item_type_icon,
        &item_type_icon_properties,
    );
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        ItemTypeIcon::get_by_id(&created_item_type_icon.id, &test_environment.database_pool)
            .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        ItemTypeIcon::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&ItemTypeIcon> = created_resources
        .iter()
        .filter(|item_type_icon| item_type_icon.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources = ItemTypeIcon::list(
        "",
        &test_environment.database_pool,
