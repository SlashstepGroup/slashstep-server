...
use slashstep_server::resources::item_connection_type::{
...
fn assert_item_connection_type_types_are_equal(
    item_connection_type_1: &ItemConnectionType,
    item_connection_type_2: &ItemConnectionType,
) {
    assert_eq!(item_connection_type_1.id, item_connection_type_2.id);
    assert_eq!(
        item_connection_type_1.display_name,
        item_connection_type_2.display_name
    );
    assert_eq!(
        item_connection_type_1.inward_description,
        item_connection_type_2.inward_description
    );
    assert_eq!(
        item_connection_type_1.outward_description,
        item_connection_type_2.outward_description
    );
    assert_eq!(
        item_connection_type_1.parent_resource_type,
        item_connection_type_2.parent_resource_type
    );
    assert_eq!(
        item_connection_type_1.parent_project_id,
        item_connection_type_2.parent_project_id
    );
}

fn assert_item_connection_type_is_equal_to_initial_properties(
    item_connection_type: &ItemConnectionType,
    initial_properties: &InitialItemConnectionTypeProperties,
) {
    assert_eq!(
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource_count =
        ItemConnectionType::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let item_connection_type = ItemConnectionType::create(
        &item_connection_type_properties,
        &test_environment.database_pool,
    )
    .await?;
    assert_item_connection_type_is_equal_to_initial_properties(
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    initialize_required_tables(&test_environment.database_pool).await?;
    initialize_predefined_actions(&test_environment.database_pool).await?;
    let created_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;

    created_item_connection_type
        .delete(&test_environment.database_pool)
        .await?;

    // Ensure that the access policy is no longer in the database.
    match ItemConnectionType::get_by_id(
        &created_item_connection_type.id,
        &test_environment.database_pool,
    )
    .await
    {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    let retrieved_resource = ItemConnectionType::get_by_id(
        &created_item_connection_type.id,
        &test_environment.database_pool,
    )
    .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        ItemConnectionType::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&ItemConnectionType> = created_resources
        .iter()
        .filter(|item_connection_type| item_connection_type.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for i in 0..created_resources.len() {
        ...
        assert_item_connection_type_types_are_equal(
            created_item_connection_type,
            retrieved_resource,
        );
    }
...
-> Result<(), TestSlashstepServerError> {
    ...
    let get_item_connection_type_types_action =
    ...
    let retrieved_resources = ItemConnectionType::list(
        "",
        &test_environment.database_pool,
