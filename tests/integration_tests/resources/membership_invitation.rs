use slashstep_server::resources::membership_invitation::{
...
fn assert_membership_invitation_types_are_equal(
    membership_invitation_1: &MembershipInvitation,
    membership_invitation_2: &MembershipInvitation,
) {
    assert_eq!(membership_invitation_1.id, membership_invitation_2.id);
    assert_eq!(
        membership_invitation_1.parent_resource_type,
        membership_invitation_2.parent_resource_type
    );
    assert_eq!(
        membership_invitation_1.parent_group_id,
        membership_invitation_2.parent_group_id
    );
    assert_eq!(
        membership_invitation_1.parent_role_id,
        membership_invitation_2.parent_role_id
    );
    assert_eq!(
        membership_invitation_1.invitee_principal_type,
        membership_invitation_2.invitee_principal_type
    );
    assert_eq!(
        membership_invitation_1.invitee_principal_user_id,
        membership_invitation_2.invitee_principal_user_id
    );
    assert_eq!(
        membership_invitation_1.invitee_principal_group_id,
        membership_invitation_2.invitee_principal_group_id
    );
    assert_eq!(
        membership_invitation_1.invitee_principal_app_id,
        membership_invitation_2.invitee_principal_app_id
    );
    assert_eq!(
        membership_invitation_1.inviter_principal_type,
        membership_invitation_2.inviter_principal_type
    );
    assert_eq!(
        membership_invitation_1.inviter_principal_user_id,
        membership_invitation_2.inviter_principal_user_id
    );
    assert_eq!(
        membership_invitation_1.inviter_principal_app_id,
        membership_invitation_2.inviter_principal_app_id
    );
}

fn assert_membership_invitation_is_equal_to_initial_properties(
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let initial_resource_count =
        MembershipInvitation::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let membership_invitation = MembershipInvitation::create(
        &membership_invitation_properties,
        &test_environment.database_pool,
    )
    .await?;
    assert_membership_invitation_is_equal_to_initial_properties(
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    let test_environment = IntegrationTestEnvironment::new().await?;
    let created_membership_invitation = test_environment
        .create_random_membership_invitation()
        .await?;

    created_membership_invitation
        .delete(&test_environment.database_pool)
        .await?;

    // Ensure that the access policy is no longer in the database.
    match MembershipInvitation::get_by_id(
        &created_membership_invitation.id,
        &test_environment.database_pool,
    )
    .await
    {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_membership_invitation = test_environment
        .create_random_membership_invitation()
        .await?;
    let retrieved_resource = MembershipInvitation::get_by_id(
        &created_membership_invitation.id,
        &test_environment.database_pool,
    )
    .await?;
    assert_membership_invitation_types_are_equal(
        &created_membership_invitation,
        &retrieved_resource,
    );
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        MembershipInvitation::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&MembershipInvitation> = created_resources
        .iter()
        .filter(|membership_invitation| membership_invitation.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let get_membership_invitation_types_action =
    ...
    let retrieved_resources = MembershipInvitation::list(
        "",
        &test_environment.database_pool,
