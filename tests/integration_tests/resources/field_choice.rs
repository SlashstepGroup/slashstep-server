...
fn assert_field_is_equal_to_initial_properties(
    ...
    initial_properties: &InitialFieldChoiceProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let initial_resource_count =
        FieldChoice::count("", &test_environment.database_pool, None, None).await?;
    ...
    assert_eq!(retrieved_field_count, MAXIMUM_FIELD_CHOICE_COUNT + initial_resource_count);
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let field_choice =
        FieldChoice::create(&field_properties, &test_environment.database_pool).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_field_choice = test_environment.create_random_field_choice(None).await?;
    let retrieved_field =
        FieldChoice::get_by_id(&created_field_choice.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_fields =
        FieldChoice::list(&query, &test_environment.database_pool, None, None).await?;

    let created_field_choices_with_specific_id: Vec<&FieldChoice> = created_field_choices
        .iter()
        .filter(|field_choice| field_choice.id == created_field_choices[0].id)
        .collect();
    assert_eq!(
        created_field_choices_with_specific_id.len(),
        retrieved_fields.len()
    );
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_fields = FieldChoice::list(
        "",
        &test_environment.database_pool,
    ...
    for allowed_field in allowed_fields {
        let retrieved_field = &retrieved_fields
            .iter()
            .find(|action| action.id == allowed_field.id)
            .unwrap();
