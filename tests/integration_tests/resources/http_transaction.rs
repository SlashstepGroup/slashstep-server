...
use slashstep_server::resources::http_transaction::{
...
fn assert_http_transactions_are_equal(
    http_transaction_1: &HTTPTransaction,
    http_transaction_2: &HTTPTransaction,
) {
    ...
    assert_eq!(
        http_transaction_1.status_code,
        http_transaction_2.status_code
    );
    assert_eq!(
        http_transaction_1
...
fn assert_http_transaction_is_equal_to_initial_properties(
    http_transaction: &HTTPTransaction,
    initial_properties: &InitialHTTPTransactionProperties,
) {
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let http_transaction = HTTPTransaction::create(
        &http_transaction_properties,
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_http_transaction = test_environment.create_random_http_transaction().await?;
    let retrieved_resource = HTTPTransaction::get_by_id(
        &created_http_transaction.id,
        &test_environment.database_pool,
    )
    .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        HTTPTransaction::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&HTTPTransaction> = created_resources
        .iter()
        .filter(|http_transaction| http_transaction.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Get the "http_transactions.get" action one time.
    ...
    // Grant access to the "http_transactions.get" action to the user for half of the actions.
    ...
    let retrieved_resources = HTTPTransaction::list(
        "",
        &test_environment.database_pool,
...
async fn verify_deletion_of_expired_http_transactions() -> Result<(), TestSlashstepServerError> {
    ...
    let created_http_transaction = HTTPTransaction::create(
        &InitialHTTPTransactionProperties {
            method: "GET".to_string(),
            url: Uuid::now_v7().to_string(),
            ip_address: local_ip()?,
