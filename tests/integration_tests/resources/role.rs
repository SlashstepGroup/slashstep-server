use slashstep_server::resources::role::{DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, InitialRoleProperties, Role};
...
fn assert_role_is_equal_to_initial_properties(
    role: &Role,
    initial_properties: &InitialRoleProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    for _ in 0..MAXIMUM_RESOURCE_COUNT {
        let resource = test_environment.create_random_role(None, None).await?;
        created_resources.push(resource);
    }

    let retrieved_resource_count =
        Role::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let role_properties = InitialRoleProperties {
        ...
        parent_resource_type: slashstep_server::resources::role::RoleParentResourceType::Project,
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    initialize_required_tables(&test_environment.database_pool).await?;
    initialize_predefined_actions(&test_environment.database_pool).await?;
    let created_role = test_environment.create_random_role(None, None).await?;
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resource =
        Role::get_by_id(&created_role.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_resources =
        Role::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&Role> = created_resources
        .iter()
        .filter(|role| role.id == created_resources[0].id)
        .collect();
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let mut current_resources = Role::list("", &test_environment.database_pool, None, None).await?;
    ...
    // Get the "roles.get" action one time.
    ...
    // Grant access to the "roles.get" action to the user for half of the actions.
    ...
    let retrieved_resources = Role::list(
        "",
        &test_environment.database_pool,
