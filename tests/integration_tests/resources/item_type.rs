use uuid::Uuid;

use slashstep_server::resources::item_type::{
    DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, InitialItemTypeProperties, ItemType,
};
use slashstep_server::resources::{
    ResourceError, ResourceType,
    access_policy::{AccessPolicy, AccessPolicyPrincipalType, InitialAccessPolicyProperties},
    action::{Action, DEFAULT_ACTION_LIST_LIMIT},
};

use crate::test_utilities::{
    integration_test_environment::IntegrationTestEnvironment,
    test_slashstep_server_error::TestSlashstepServerError,
};

fn assert_item_types_are_equal(item_type_1: &ItemType, item_type_2: &ItemType) {
    assert_eq!(item_type_1.id, item_type_2.id);
    assert_eq!(item_type_1.name, item_type_2.name);
    assert_eq!(item_type_1.display_name, item_type_2.display_name);
    assert_eq!(item_type_1.parent_project_id, item_type_2.parent_project_id);
    assert_eq!(item_type_1.item_type_icon_id, item_type_2.item_type_icon_id);
    assert_eq!(item_type_1.description, item_type_2.description);
}

fn assert_item_type_is_equal_to_initial_properties(
    item_type: &ItemType,
    initial_properties: &InitialItemTypeProperties,
) {
    assert_eq!(item_type.name, initial_properties.name);
    assert_eq!(item_type.display_name, initial_properties.display_name);
    assert_eq!(
        item_type.parent_project_id,
        initial_properties.parent_project_id
    );
    assert_eq!(
        item_type.item_type_icon_id,
        initial_properties.item_type_icon_id
    );
    assert_eq!(item_type.description, initial_properties.description);
}

#[tokio::test]
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    const MAXIMUM_RESOURCE_COUNT: i64 = DEFAULT_RESOURCE_LIST_LIMIT + 1;
    let mut created_resources: Vec<ItemType> = Vec::new();
    for _ in 0..MAXIMUM_RESOURCE_COUNT {
        let resource = test_environment.create_random_item_type(None).await?;
        created_resources.push(resource);
    }

    let retrieved_resource_count =
        ItemType::count("", &test_environment.database_pool, None, None).await?;

    assert_eq!(retrieved_resource_count, MAXIMUM_RESOURCE_COUNT);

    return Ok(());
}

#[tokio::test]
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;

    let item_type_properties = InitialItemTypeProperties {
        name: Uuid::now_v7().to_string(),
        display_name: Uuid::now_v7().to_string(),
        parent_project_id: test_environment.create_random_project(None).await?.id,
        item_type_icon_id: None,
        description: Some(Uuid::now_v7().to_string()),
    };

    let item_type =
        ItemType::create(&item_type_properties, &test_environment.database_pool).await?;
    assert_item_type_is_equal_to_initial_properties(&item_type, &item_type_properties);

    return Ok(());
}

#[tokio::test]
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    // Create the access policy.
    let test_environment = IntegrationTestEnvironment::new().await?;
    let created_item_type = test_environment.create_random_item_type(None).await?;

    created_item_type
        .delete(&test_environment.database_pool)
        .await?;

    // Ensure that the access policy is no longer in the database.
    match ItemType::get_by_id(&created_item_type.id, &test_environment.database_pool).await {
        Ok(_) => panic!("Expected a resource not found error."),

        Err(error) => match error {
            ResourceError::NotFoundError(_) => {}

            error => return Err(TestSlashstepServerError::ResourceError(error)),
        },
    };

    return Ok(());
}

#[tokio::test]
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;

    let created_item_type = test_environment.create_random_item_type(None).await?;
    let retrieved_resource =
        ItemType::get_by_id(&created_item_type.id, &test_environment.database_pool).await?;
    assert_item_types_are_equal(&created_item_type, &retrieved_resource);

    return Ok(());
}

/// Verifies that the implementation can return up to a maximum number of resources by default.
#[tokio::test]
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    const MAXIMUM_RESOURCE_COUNT: i64 = DEFAULT_RESOURCE_LIST_LIMIT + 1;
    let mut item_types: Vec<ItemType> = Vec::new();
    for _ in 0..MAXIMUM_RESOURCE_COUNT {
        let item_type = test_environment.create_random_item_type(None).await?;
        item_types.push(item_type);
    }

    let retrieved_resources =
        ItemType::list("", &test_environment.database_pool, None, None).await?;

    assert_eq!(
        retrieved_resources.len(),
        DEFAULT_ACTION_LIST_LIMIT as usize
    );

    return Ok(());
}

/// Verifies that a list of resources can be retrieved with a query.
#[tokio::test]
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    const MAXIMUM_RESOURCE_COUNT: i32 = 5;
    let mut created_resources: Vec<ItemType> = Vec::new();
    for _ in 0..MAXIMUM_RESOURCE_COUNT {
        let resource = test_environment.create_random_item_type(None).await?;
        created_resources.push(resource);
    }

    let query = format!("id = \"{}\"", created_resources[0].id);
    let retrieved_resources =
        ItemType::list(&query, &test_environment.database_pool, None, None).await?;

    let created_resources_with_specific_id: Vec<&ItemType> = created_resources
        .iter()
        .filter(|item_type| item_type.id == created_resources[0].id)
        .collect();
    assert_eq!(
        created_resources_with_specific_id.len(),
        retrieved_resources.len()
    );
    for i in 0..created_resources_with_specific_id.len() {
        let created_resource = &created_resources_with_specific_id[i];
        let retrieved_resource = &retrieved_resources[i];

        assert_item_types_are_equal(created_resource, retrieved_resource);
    }

    return Ok(());
}

#[tokio::test]
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    const MAXIMUM_RESOURCE_COUNT: i32 = 25;
    let mut created_resources: Vec<ItemType> = Vec::new();
    for _ in 0..MAXIMUM_RESOURCE_COUNT {
        let item_type = test_environment.create_random_item_type(None).await?;
        created_resources.push(item_type);
    }

    let retrieved_resources =
        ItemType::list("", &test_environment.database_pool, None, None).await?;

    for created_item_type in &created_resources {
        let retrieved_item_type_option = retrieved_resources
            .iter()
            .find(|retrieved_item_type| retrieved_item_type.id == created_item_type.id);
        assert!(retrieved_item_type_option.is_some());
    }

    return Ok(());
}

/// Verifies that a list of resources can be retrieved without a query.
#[tokio::test]
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions()
-> Result<(), TestSlashstepServerError> {
    // Make sure there are at least two actions.
    let test_environment = IntegrationTestEnvironment::new().await?;

    const MINIMUM_RESOURCE_COUNT: i32 = 2;
    let mut current_resources =
        ItemType::list("", &test_environment.database_pool, None, None).await?;
    if current_resources.len() < MINIMUM_RESOURCE_COUNT as usize {
        let remaining_action_count = MINIMUM_RESOURCE_COUNT - current_resources.len() as i32;
        for _ in 0..remaining_action_count {
            let item_type = test_environment.create_random_item_type(None).await?;
            current_resources.push(item_type);
        }
    }

    // Get the "item_types.get" action one time.
    let user = test_environment.create_random_user(None).await?;
    let get_item_types_action =
        Action::get_by_name(GET_RESOURCE_ACTION_NAME, &test_environment.database_pool).await?;

    // Grant access to the "item_types.get" action to the user for half of the actions.
    let allowed_resource_count = current_resources.len() / 2;
    let mut allowed_resources = Vec::new();
    for index in 0..allowed_resource_count {
        let scoped_item_type = &current_resources[index];

        AccessPolicy::create(
            &InitialAccessPolicyProperties {
                action_id: get_item_types_action.id.clone(),
                permission_level: slashstep_server::resources::access_policy::PermissionLevel::User,
                principal_type:
                    slashstep_server::resources::access_policy::AccessPolicyPrincipalType::User,
                principal_user_id: Some(user.id.clone()),
                scoped_resource_type: ResourceType::ItemType,
                scoped_item_type_id: Some(scoped_item_type.id.clone()),
                ..Default::default()
            },
            &test_environment.database_pool,
        )
        .await?;

        allowed_resources.push(scoped_item_type.clone());
    }

    // Make sure the user only sees the allowed actions.
    let retrieved_resources = ItemType::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    )
    .await?;

    assert_eq!(allowed_resources.len(), retrieved_resources.len());
    for allowed_resource in allowed_resources {
        let retrieved_resource = &retrieved_resources
            .iter()
            .find(|action| action.id == allowed_resource.id)
            .expect("Expected a retrieved resource with the same ID.");

        assert_item_types_are_equal(&allowed_resource, retrieved_resource);
    }

    return Ok(());
}
