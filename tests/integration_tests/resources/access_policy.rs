...
fn assert_access_policy_is_equal_to_initial_properties(
    ...
    initial_properties: &InitialAccessPolicyProperties,
) {
    assert_eq!(access_policy.action_id, initial_properties.action_id);
    assert_eq!(
        access_policy.permission_level,
        initial_properties.permission_level
    );
    assert_eq!(
        access_policy.is_inheritance_enabled,
        initial_properties.is_inheritance_enabled
    );
    assert_eq!(
        access_policy.principal_type,
        initial_properties.principal_type
    );
    assert_eq!(
        access_policy.principal_user_id,
        initial_properties.principal_user_id
    );
    assert_eq!(
        access_policy.principal_group_id,
        initial_properties.principal_group_id
    );
    assert_eq!(
        access_policy.principal_role_id,
        initial_properties.principal_role_id
    );
    assert_eq!(
        access_policy.principal_app_id,
        initial_properties.principal_app_id
    );
    assert_eq!(
        access_policy.scoped_resource_type,
        initial_properties.scoped_resource_type
    );
    assert_eq!(
        access_policy.scoped_action_id,
        initial_properties.scoped_action_id
    );
    assert_eq!(
        access_policy.scoped_app_id,
        initial_properties.scoped_app_id
    );
    assert_eq!(
        access_policy.scoped_group_id,
        initial_properties.scoped_group_id
    );
    assert_eq!(
        access_policy.scoped_item_id,
        initial_properties.scoped_item_id
    );
    assert_eq!(
        access_policy.scoped_milestone_id,
        initial_properties.scoped_milestone_id
    );
    assert_eq!(
        access_policy.scoped_project_id,
        initial_properties.scoped_project_id
    );
    assert_eq!(
        access_policy.scoped_role_id,
        initial_properties.scoped_role_id
    );
    assert_eq!(
        access_policy.scoped_user_id,
        initial_properties.scoped_user_id
    );
}

fn assert_access_policies_are_equal(
    access_policy_1: &AccessPolicy,
    access_policy_2: &AccessPolicy,
) {
    assert_eq!(access_policy_1.id, access_policy_2.id);
    assert_eq!(access_policy_1.action_id, access_policy_2.action_id);
    assert_eq!(
        access_policy_1.permission_level,
        access_policy_2.permission_level
    );
    assert_eq!(
        access_policy_1.is_inheritance_enabled,
        access_policy_2.is_inheritance_enabled
    );
    assert_eq!(
        access_policy_1.principal_type,
        access_policy_2.principal_type
    );
    assert_eq!(
        access_policy_1.principal_user_id,
        access_policy_2.principal_user_id
    );
    assert_eq!(
        access_policy_1.principal_group_id,
        access_policy_2.principal_group_id
    );
    assert_eq!(
        access_policy_1.principal_role_id,
        access_policy_2.principal_role_id
    );
    assert_eq!(
        access_policy_1.principal_app_id,
        access_policy_2.principal_app_id
    );
    assert_eq!(
        access_policy_1.scoped_resource_type,
        access_policy_2.scoped_resource_type
    );
    assert_eq!(
        access_policy_1.scoped_action_id,
        access_policy_2.scoped_action_id
    );
    assert_eq!(access_policy_1.scoped_app_id, access_policy_2.scoped_app_id);
    assert_eq!(
        access_policy_1.scoped_group_id,
        access_policy_2.scoped_group_id
    );
    assert_eq!(
        access_policy_1.scoped_item_id,
        access_policy_2.scoped_item_id
    );
    assert_eq!(
        access_policy_1.scoped_milestone_id,
        access_policy_2.scoped_milestone_id
    );
    assert_eq!(
        access_policy_1.scoped_project_id,
        access_policy_2.scoped_project_id
    );
    assert_eq!(
        access_policy_1.scoped_role_id,
        access_policy_2.scoped_role_id
    );
    assert_eq!(
...
async fn get_access_policy_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_access_policy =
        AccessPolicy::get_by_id(&created_access_policy.id, &test_environment.database_pool).await?;
...
async fn list_access_policies_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let mut remaining_resource_count = MAXIMUM_RESOURCE_COUNT;
    while remaining_resource_count > 0 {
    ...
    let retrieved_access_policies =
        AccessPolicy::list("", &test_environment.database_pool, None, None).await?;

    for created_access_policy in &created_access_policies {
        let retrieved_access_policy_option = retrieved_access_policies
            .iter()
            .find(|retrieved_access_policy| retrieved_access_policy.id == created_access_policy.id);
        assert!(retrieved_access_policy_option.is_some());
    }
...
-> Result<(), TestSlashstepServerError> {
    ...
    while remaining_action_count > 0 {
        ...
        let access_policy_properties = InitialAccessPolicyProperties {
            ...
            permission_level: if remaining_action_count > denied_access_policy_count {
        ...
        let access_policy =
            AccessPolicy::create(&access_policy_properties, &test_environment.database_pool)
                .await?;
    ...
    let retrieved_access_policies = AccessPolicy::list(
        "",
    ...
    .await?;
    assert_eq!(
        created_access_policies.len(),
        retrieved_access_policies.len()
    );
...
async fn list_access_policies_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    while remaining_action_count > 0 {
        ...
        let access_policy_properties = InitialAccessPolicyProperties {
            ...
            is_inheritance_enabled: true,
            principal_type: AccessPolicyPrincipalType::User,
            principal_user_id: if remaining_action_count == 1 {
                created_access_policies[0].principal_user_id
            } else {
                Some(user.id)
            },
            scoped_resource_type: ResourceType::Server,
    ...
    let principal_user_id = created_access_policies[0]
        .principal_user_id
        .expect("Principal user ID is not set.");
    let query = format!("principal_user_id = \"{}\"", principal_user_id);
    let retrieved_access_policies =
        AccessPolicy::list(&query, &test_environment.database_pool, None, None).await?;

    let created_access_policies_with_specific_user: Vec<&AccessPolicy> = created_access_policies
        .iter()
        .filter(|access_policy| access_policy.principal_user_id == Some(principal_user_id))
        .collect();
    assert_eq!(
        created_access_policies_with_specific_user.len(),
        retrieved_access_policies.len()
    );
...
async fn count_access_policies() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    let current_access_policy_count =
    ...
    let mut created_access_policies: Vec<AccessPolicy> = Vec::new();
    let mut remaining_action_count = MAXIMUM_RESOURCE_COUNT;
    while remaining_action_count > 0 {
        let action = test_environment.create_random_action(None).await?;
        let user = test_environment.create_random_user(None).await?;
        let access_policy_properties = InitialAccessPolicyProperties {
...
async fn delete_access_policy() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_access_policy_result =
        AccessPolicy::get_by_id(&created_access_policy.id, &test_environment.database_pool).await;
...
async fn update_access_policy() -> Result<(), TestSlashstepServerError> {
    ...
    let instance_access_policy = AccessPolicy::create(
        &instance_access_policy_properties,
        &test_environment.database_pool,
    )
    .await?;
    ...
    let updated_access_policy = instance_access_policy
        .update(
            &updated_access_policy_properties,
            &test_environment.database_pool,
        )
        .await?;

    assert_eq!(
        updated_access_policy.permission_level,
