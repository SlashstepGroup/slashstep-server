...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        item_type::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, ItemType},
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_delegation_policies_action =
        Action::get_by_name("itemTypes.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    let router = slashstep_server::routes::item_types::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/item-types"))
    ...
    let actual_item_type_count = ItemType::count(
        "",
    ...
    let actual_delegation_policies = ItemType::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_item_type in actual_delegation_policies {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|item_type| item_type.id == actual_item_type.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/item-types")).add_query_param(
            "query",
            format!(
                "id ~ {}",
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_types::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/item-types")).await;
