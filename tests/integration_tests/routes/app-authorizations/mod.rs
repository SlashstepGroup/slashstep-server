...
async fn verify_returned_resource_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_app_authorizations_action =
        Action::get_by_name("appAuthorizations.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    let router = slashstep_server::routes::app_authorizations::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/app-authorizations"))
    ...
    let actual_app_authorization_count = AppAuthorization::count(
        "",
    ...
    let actual_app_authorizations = AppAuthorization::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
...
async fn verify_default_resource_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    assert_eq!(
        ...
        DEFAULT_APP_AUTHORIZATION_LIST_LIMIT as usize
    );
...
async fn verify_maximum_resource_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        ...
        .add_query_param(
            "query",
            format!("LIMIT {}", DEFAULT_APP_AUTHORIZATION_LIST_LIMIT + 1),
        )
        .add_cookie(Cookie::new("session_access_token", &session_token))
        .await;
...
async fn verify_query_when_listing_resources() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM app_authorizations WHERE id = {}",
                    get_app_authorizations_action.id
                ),
            ),
    ];
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("app_ied = {}", get_app_authorizations_action.id),
            ),
        test_server
            .get(&format!("/app-authorizations"))
...
async fn verify_authentication_when_listing_resources() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::app_authorizations::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/app-authorizations")).await;
