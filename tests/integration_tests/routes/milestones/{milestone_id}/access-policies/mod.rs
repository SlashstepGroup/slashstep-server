...
async fn create_milestone_access_policy(
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::milestones::milestone_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!(
            "/milestones/{}/access-policies",
            dummy_milestone.id
        ))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_milestone = test_environment.create_random_milestone(None).await?;
    let shown_access_policy = create_milestone_access_policy(
    ...
    let query = format!(
        ...
        quote_literal(&dummy_milestone.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_milestone = test_environment.create_random_milestone(None).await?;
    create_milestone_access_policy(
        &test_environment.database_pool,
        &dummy_milestone.id,
        &user.id,
        &list_access_policies_action.id,
    ...
    let query = format!(
        "scoped_resource_type = 'Milestone' AND scoped_milestone_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_milestone.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
