...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, milestone::{EditableMilestoneProperties, Milestone}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::milestones::milestone_id::get_router(state.clone())
        .with_state(state)
    ...
    let response = test_server
        .get(&format!("/milestones/{}", milestone.id))
    ...
    let get_milestone_response_body = response.json::<GetResourceResponseBody<Milestone>>();
    let response_milestone = get_milestone_response_body.data;
    ...
    assert_eq!(
        ...
        milestone.parent_resource_type
    );
    assert_eq!(
        response_milestone.parent_project_id,
        milestone.parent_project_id
    );
    assert_eq!(
        response_milestone.parent_workspace_id,
        milestone.parent_workspace_id
    );
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/milestones/not-a-uuid").await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/milestones/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_milestones_action =
        Action::get_by_name("milestones.delete", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/milestones/{}", milestone.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    match Milestone::get_by_id(&milestone.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an milestone not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/milestones/not-a-uuid").await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/milestones/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_milestones_action =
        Action::get_by_name("milestones.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_milestones_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/milestones/{}", original_milestone.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_milestone_response_body: PatchResourceResponseBody<Milestone> = response.json();
    let updated_milestone = patch_milestone_response_body.data;
    assert_eq!(original_milestone.id, updated_milestone.id);
    assert_eq!(
        updated_milestone.display_name,
        updated_milestone_properties.display_name.unwrap()
    );
    assert_eq!(
        updated_milestone.name,
        updated_milestone_properties.name.unwrap()
    );
    assert_eq!(
        updated_milestone.description,
        updated_milestone_properties.description.unwrap()
    );
    assert_eq!(
        updated_milestone
            .start_date
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::milestones::milestone_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/milestones/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/milestones/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_authentication_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/milestones/{}", milestone.id))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!({
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/milestones/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
-> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_milestone_action =
        Action::get_by_name("milestones.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_milestone_action.id,
            &PermissionLevel::User,
        )
        .await?;
