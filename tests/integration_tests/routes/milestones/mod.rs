/**
 *
 * Any test cases for /milestones should be handled here.
 *
 * Programmers:
...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        milestone::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Milestone},
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    // Grant access to the "milestones.get" action to the user.
    ...
    // Grant access to the "milestones.list" action to the user.
    ...
    let router = slashstep_server::routes::milestones::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/milestones"))
    ...
    for actual_milestone in actual_milestones {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|milestone| milestone.id == actual_milestone.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::milestones::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let query = format!("id = {}", quote_literal(&dummy_milestone.id.to_string()));
    let response = test_server
        .get(&format!("/milestones"))
        .add_cookie(Cookie::new(
            "session_access_token",
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/milestones")).add_query_param(
            "query",
            format!(
                "id ~ {}",
                quote_literal(&get_milestones_action.id.to_string())
            ),
        ),
        test_server
            .get(&format!("/milestones"))
        ...
        test_server.get(&format!("/milestones")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM milestones WHERE id = {}",
                quote_literal(&get_milestones_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::milestones::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/milestones")).await;
