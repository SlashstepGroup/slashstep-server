...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, workspace::{EditableWorkspaceProperties, Workspace}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::workspaces::workspace_id::get_router(state.clone())
        .with_state(state)
    ...
    test_environment
        .create_server_access_policy(&user.id, &get_workspaces_action.id, &PermissionLevel::User)
        .await?;

    let workspace = test_environment.create_random_workspace().await?;

    let response = test_server
        .get(&format!("/workspaces/{}", workspace.id))
    ...
    let get_workspace_response_body = response.json::<GetResourceResponseBody<Workspace>>();
    let response_workspace = get_workspace_response_body.data;
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/workspaces/not-a-uuid").await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/workspaces/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_workspaces_action =
        Action::get_by_name("workspaces.delete", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/workspaces/{}", workspace.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    match Workspace::get_by_id(&workspace.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an workspace not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/workspaces/not-a-uuid").await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/workspaces/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let update_fields_action =
        Action::get_by_name("workspaces.update", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/workspaces/{}", original_workspace.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_workspace_response_body: PatchResourceResponseBody<Workspace> = response.json();
    let updated_workspace = patch_workspace_response_body.data;
    assert_eq!(original_workspace.id, updated_workspace.id);
    assert_eq!(
        updated_workspace_properties
            .name
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::workspaces::workspace_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/workspaces/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/workspaces/{}", Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_authentication_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/workspaces/{}", workspace.id))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
async fn verify_workspace_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError>
{
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_workspaces_action =
        Action::get_by_name("workspaces.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_workspaces_action.id,
            &PermissionLevel::User,
        )
        .await?;

    // Set up the server and send the request.
    let maximum_workspace_name_length_configuration = Configuration::get_by_name(
        "workspaces.maximumNameLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_workspace_name_length_configuration
        .update(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/workspaces/{}", dummy_workspace.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_workspace_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let workspace_name_regex_configuration = Configuration::get_by_name(
        "workspaces.allowedNameRegex",
        &test_environment.database_pool,
    )
    .await?;
    workspace_name_regex_configuration
        .update(
            &EditableConfigurationProperties {
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let maximum_workspace_display_name_length_configuration = Configuration::get_by_name(
        "workspaces.maximumDisplayNameLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_workspace_display_name_length_configuration
        .update(
            &EditableConfigurationProperties {
                number_value: Some(Decimal::from(0 as i64)),
                ..Default::default()
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let maximum_workspace_description_length_configuration = Configuration::get_by_name(
        "workspaces.maximumDescriptionLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_workspace_description_length_configuration
        .update(
            &EditableConfigurationProperties {
                number_value: Some(Decimal::from(0 as i64)),
                ..Default::default()
