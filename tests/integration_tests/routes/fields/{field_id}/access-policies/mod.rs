...
async fn create_field_access_policy(
    database_pool: &deadpool_postgres::Pool,
    scoped_field_id: &Uuid,
    user_id: &Uuid,
    action_id: &Uuid,
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::fields::field_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/fields/{}/access-policies", dummy_field.id))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_field = test_environment.create_random_field(None).await?;
    let shown_access_policy = create_field_access_policy(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/fields/{}/access-policies", &dummy_field.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let query = format!(
        ...
        quote_literal(&dummy_field.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "scoped_resource_type = 'Field' AND scoped_field_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_field.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
