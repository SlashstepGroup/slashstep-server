...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        field_choice::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, FieldChoice,
            FieldChoiceType,
        },
    },
    routes::{
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_field_choices_action =
        Action::get_by_name("fieldChoices.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &list_field_choices_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let router =
        slashstep_server::routes::fields::field_id::field_choices::get_router(state.clone())
            .with_state(state)
    ...
    let query = format!(
        "field_id = {}",
        quote_literal(&dummy_field_choice.field_id.to_string())
    );
    let actual_field_choice_count = FieldChoice::count(
        &query,
    ...
    let actual_field_choices = FieldChoice::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let field_choice_count = FieldChoice::count(
        format!("field_id = {}", quote_literal(&dummy_field.id.to_string())).as_str(),
        &test_environment.database_pool,
        None,
        None,
    )
    ...
    for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - field_choice_count + 1) {
        test_environment
            .create_random_field_choice(Some(&dummy_field.id))
            .await?;
    }
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/fields/{}/field-choices", &dummy_field.id))
        .add_cookie(Cookie::new("session_access_token", &session_token))
        .await;
...
async fn verify_query_when_listing_resources() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM field_choices WHERE id = {}",
                    get_field_choices_action.id
                ),
            ),
    ];
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("app_ied = {}", get_field_choices_action.id),
            ),
        test_server
            .get(&format!("/fields/{}/field-choices", &dummy_field.id))
...
-> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/fields/{}/field-choices", &Uuid::now_v7()))
        .await;
...
async fn verify_successful_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/fields/{}/field-choices", dummy_field.id))
        .add_cookie(Cookie::new("session_access_token", &session_token))
    ...
    assert_eq!(
        initial_field_choice_properties.description,
        response_field_choice.description
    );
    assert_eq!(
        initial_field_choice_properties.value_type,
        response_field_choice.value_type
    );
    assert_eq!(
        initial_field_choice_properties.text_value,
        response_field_choice.text_value
    );
    assert_eq!(
        initial_field_choice_properties.number_value,
        response_field_choice.number_value
    );
    assert_eq!(
        initial_field_choice_properties.timestamp_value,
        response_field_choice.timestamp_value
    );
    assert_eq!(
        initial_field_choice_properties.stakeholder_type,
        response_field_choice.stakeholder_type
    );
    assert_eq!(
        initial_field_choice_properties.stakeholder_user_id,
        response_field_choice.stakeholder_user_id
    );
    assert_eq!(
        initial_field_choice_properties.stakeholder_group_id,
        response_field_choice.stakeholder_group_id
    );
    assert_eq!(
        initial_field_choice_properties.stakeholder_app_id,
        response_field_choice.stakeholder_app_id
    );
...
async fn verify_not_found_when_creating_resource() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .post(&format!("/fields/{}/field-choices", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!(initial_field_choice_properties))
        .await;
