...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        }, action::Action, app::App, configuration::{Configuration, EditableConfigurationProperties}, field::{EditableFieldProperties, Field}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::fields::field_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_field_response_body = response.json::<GetResourceResponseBody<Field>>();
    ...
    assert_eq!(
        ...
        field.minimum_choice_count
    );
    assert_eq!(
        response_field.maximum_choice_count,
        field.maximum_choice_count
    );
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/fields/not-a-uuid").await;
...
async fn verify_authentication_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError>
{
    ...
    let response = test_server.get(&format!("/fields/{}", field.id)).await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/fields/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_fields_action =
        Action::get_by_name("fields.delete", &test_environment.database_pool).await?;
    AccessPolicy::create(
    ...
    match App::get_by_id(&field.id, &test_environment.database_pool)
        .await
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/fields/not-a-uuid").await;
...
async fn verify_authentication_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::fields::field_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.delete(&format!("/fields/{}", field.id)).await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/fields/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let update_fields_action =
        Action::get_by_name("fields.update", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/fields/{}", original_field.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_app_response_body: PatchResourceResponseBody<Field> = response.json();
    let updated_app = patch_app_response_body.data;
    assert_eq!(original_field.id, updated_app.id);
    assert_eq!(
        updated_app.name,
        updated_field_properties
            .name
            .expect("Expected an updated field name to compare.")
    );
    assert_eq!(
        updated_app.display_name,
        updated_field_properties
            .display_name
            .expect("Expected an updated field display name to compare.")
    );
    assert_eq!(
        updated_app.description,
        updated_field_properties
            .description
            .expect("Expected an updated field description to compare.")
    );
    assert_eq!(
        updated_app.field_value_type,
        original_field.field_value_type
    );
    ...
    assert_eq!(
        updated_app.minimum_choice_count,
        original_field.minimum_choice_count
    );
    assert_eq!(
        updated_app.maximum_choice_count,
        original_field.maximum_choice_count
    );
    assert_eq!(
        updated_app.parent_project_id,
        original_field.parent_project_id
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::fields::field_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/fields/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/fields/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/fields/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
async fn verify_field_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/fields/{}", dummy_field.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
