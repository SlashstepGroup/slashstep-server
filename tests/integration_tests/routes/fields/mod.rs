...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        field::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Field},
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::fields::get_router(state.clone())
        .with_state(state)
    ...
    .await?;
    assert_eq!(response_json.data.len(), actual_fields.len());

    for actual_field in actual_fields {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|field| field.id == actual_field.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/fields")).add_query_param(
            "query",
            format!("id ~ {}", quote_literal(&get_fields_action.id.to_string())),
        ),
        test_server
            .get(&format!("/fields"))
        ...
        test_server.get(&format!("/fields")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM fields WHERE id = {}",
                quote_literal(&get_fields_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::fields::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/fields")).await;
