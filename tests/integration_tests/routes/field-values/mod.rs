/*
 *
 * Any test cases for /field-values should be handled here.
 *
 * Programmers:
...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        field_value::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, FieldValue,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    // Grant access to the "fieldValues.get" action to the user.
    ...
    // Grant access to the "fieldValues.list" action to the user.
    let list_field_values_action =
        Action::get_by_name("fieldValues.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &list_field_values_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let router = slashstep_server::routes::field_values::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/field-values"))
    ...
    let response_json: ListResourcesResponseBody<FieldValue> = response.json();
    assert!(response_json.total_count > 0);
    assert!(response_json.data.len() > 0);

    let actual_field_value_count = FieldValue::count(
        "",
    ...
    let actual_field_values = FieldValue::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    .await?;
    assert_eq!(response_json.data.len(), actual_field_values.len());

    for actual_field_value in actual_field_values {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|field_value| field_value.id == actual_field_value.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::field_values::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let query = format!("id = {}", quote_literal(&dummy_field_value.id.to_string()));
    let response = test_server
        .get(&format!("/field-values"))
        .add_cookie(Cookie::new("session_access_token", &session_token))
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/field-values")).add_query_param(
            "query",
            format!(
                "id ~ {}",
                quote_literal(&get_field_values_action.id.to_string())
            ),
        ),
        test_server
            .get(&format!("/field-values"))
        ...
        test_server.get(&format!("/field-values")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM field_values WHERE id = {}",
                quote_literal(&get_field_values_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::field_values::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/field-values")).await;
