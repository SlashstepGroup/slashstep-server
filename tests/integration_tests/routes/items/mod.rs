...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        item::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Item},
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_items_action =
        Action::get_by_name("items.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_items_action.id, &PermissionLevel::User)
        .await?;
    ...
    let router = slashstep_server::routes::items::get_router(state.clone())
        .with_state(state)
    ...
    let actual_item_count = Item::count(
        "",
    ...
    for actual_item in actual_items {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|item| item.id == actual_item.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/items")).add_query_param(
            "query",
            format!("id ~ {}", quote_literal(&get_items_action.id.to_string())),
        ),
        test_server
            .get(&format!("/items"))
        ...
        test_server.get(&format!("/items")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM items WHERE id = {}",
                quote_literal(&get_items_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::items::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/items")).await;
