...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        item_connection::{
            DEFAULT_RESOURCE_LIST_LIMIT, InitialItemConnectionProperties,
...
async fn create_item_connection(
    test_environment: &IntegrationTestEnvironment,
    outward_item_id: &Uuid,
    inward_item_id: &Uuid,
    ...
    let item_connection_type = test_environment
        ...
        .await?;
    let dummy_item_connection = ItemConnection::create(
...
async fn verify_successful_item_connection_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let create_item_connections_action =
        Action::get_by_name("itemConnections.create", &test_environment.database_pool).await?;
    test_environment
    ...
    let dummy_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    ...
    let initial_item_connection_properties =
        InitialItemConnectionPropertiesWithPredefinedOutwardItem {
    ...
    let router = slashstep_server::routes::items::item_id::item_connections::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/items/{}/item-connections", outward_item.id))
    ...
    assert_eq!(
        response_item_connection.item_connection_type_id,
        dummy_item_connection_type.id
    );
...
async fn verify_returned_item_connection_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let get_item_connections_action =
        Action::get_by_name("itemConnections.get", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    let list_item_connections_action =
        Action::get_by_name("itemConnections.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &list_item_connections_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let outward_item = test_environment.create_random_item(None).await?;
    let inward_item = test_environment.create_random_item(None).await?;
    let shown_item_connection =
        create_item_connection(&test_environment, &outward_item.id, &inward_item.id).await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/items/{}/item-connections", &outward_item.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let query = format!(
        ...
        quote_literal(&outward_item.id.to_string()),
        quote_literal(&outward_item.id.to_string())
    );
    let actual_item_connection_count = ItemConnection::count(
        &query,
        &test_environment.database_pool,
    ...
    .await?;
    assert_eq!(
        response_item_connections.total_count,
        actual_item_connection_count
    );

    let actual_item_connections = ItemConnection::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    .await?;
    assert_eq!(
        response_item_connections.data.len(),
        actual_item_connections.len()
    );
    assert_eq!(
        response_item_connections.data[0].id,
        actual_item_connections[0].id
    );
    assert_eq!(
        response_item_connections.data[0].id,
        shown_item_connection.id
    );
...
async fn verify_returned_resource_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "(outward_item_id = {} OR inward_item_id = {}) AND ({})",
        quote_literal(&outward_item.id.to_string()),
        quote_literal(&outward_item.id.to_string()),
        additional_query
    );
...
async fn verify_query_when_listing_item_connections() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM item_connections WHERE action_id = {}",
                    get_item_connections_action.id
                ),
            ),
    ];
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("action_ied = {}", get_item_connections_action.id),
            ),
        test_server
            .get(&format!("/items/{}/item-connections", &outward_item.id))
...
#[tokio::test]
async fn verify_authentication_when_listing_item_connections()
