...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, role::{EditableRoleProperties, Role}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::roles::role_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_role_response_body = response.json::<GetResourceResponseBody<Role>>();
    let response_role = get_role_response_body.data;
    ...
    assert_eq!(
        response_role.predefined_role_type,
        role.predefined_role_type
    );
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/roles/not-a-uuid").await;
...
async fn verify_authentication_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError>
{
    ...
    let response = test_server.get(&format!("/roles/{}", role.id)).await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/roles/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_roles_action =
        Action::get_by_name("roles.delete", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &delete_roles_action.id, &PermissionLevel::User)
        .await?;
    ...
    match Role::get_by_id(&role.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an role not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/roles/not-a-uuid").await;
...
async fn verify_authentication_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::roles::role_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.delete(&format!("/roles/{}", role.id)).await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/roles/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_roles_action =
        Action::get_by_name("roles.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &update_roles_action.id, &PermissionLevel::User)
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/roles/{}", original_role.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_role_response_body: PatchResourceResponseBody<Role> = response.json();
    let updated_role = patch_role_response_body.data;
    assert_eq!(original_role.id, updated_role.id);
    assert_eq!(updated_role.name, updated_role_properties.name.unwrap());
    assert_eq!(
        updated_role.display_name,
        updated_role_properties.display_name.unwrap()
    );
    assert_eq!(
        updated_role.description,
        updated_role_properties.description.unwrap()
    );
    assert_eq!(
        original_role.parent_resource_type,
        updated_role.parent_resource_type
    );
    assert_eq!(
        original_role.parent_workspace_id,
        updated_role.parent_workspace_id
    );
    assert_eq!(
        original_role.parent_project_id,
        updated_role.parent_project_id
    );
    assert_eq!(
        original_role.predefined_role_type,
        updated_role.predefined_role_type
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::roles::role_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/roles/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/roles/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/roles/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
-> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_role_action =
        Action::get_by_name("roles.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &update_role_action.id, &PermissionLevel::User)
        .await?;
...
#[tokio::test]
async fn verify_role_descriptions_are_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
