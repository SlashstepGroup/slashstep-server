...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        item_connection::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, ItemConnection,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connections::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/item-connections"))
    ...
    let actual_delegation_policies = ItemConnection::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_item_connection in actual_delegation_policies {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|item_connection| item_connection.id == actual_item_connection.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "id = {}",
        quote_literal(&dummy_item_connection.id.to_string())
    );
    let response = test_server
        .get(&format!("/item-connections"))
        .add_cookie(Cookie::new("session_access_token", &session_token))
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connections::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/item-connections")).await;
