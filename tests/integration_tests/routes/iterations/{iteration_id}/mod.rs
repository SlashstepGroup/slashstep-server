...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, iteration::{EditableIterationProperties, Iteration}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::iterations::iteration_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_iterations_action =
        Action::get_by_name("iterations.get", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &get_iterations_action.id, &PermissionLevel::User)
        .await?;

    let iteration = test_environment.create_random_iteration(None).await?;

    let response = test_server
        .get(&format!("/iterations/{}", iteration.id))
    ...
    let get_iteration_response_body = response.json::<GetResourceResponseBody<Iteration>>();
    let response_iteration = get_iteration_response_body.data;
    ...
    assert_eq!(
        response_iteration.actual_start_date,
        iteration.actual_start_date
    );
    assert_eq!(
        response_iteration.actual_end_date,
        iteration.actual_end_date
    );
    assert_eq!(
        response_iteration.parent_project_id,
        iteration.parent_project_id
    );
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/iterations/not-a-uuid").await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/iterations/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_iterations_action =
        Action::get_by_name("iterations.delete", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/iterations/{}", iteration.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    match Iteration::get_by_id(&iteration.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an iteration not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/iterations/not-a-uuid").await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/iterations/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let update_fields_action =
        Action::get_by_name("iterations.update", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/iterations/{}", original_iteration.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_iteration_response_body: PatchResourceResponseBody<Iteration> = response.json();
    let updated_iteration = patch_iteration_response_body.data;
    assert_eq!(original_iteration.id, updated_iteration.id);
    assert_eq!(
        updated_iteration_properties
            .display_name
            .expect("Expected an updated display name."),
        updated_iteration.display_name
    );
    assert_eq!(
        updated_iteration_properties
            .start_date
            .and_then(|date| DateTime::from_timestamp_millis(date.timestamp_millis()))
            .expect("Expected a valid start date timestamp."),
        DateTime::from_timestamp_millis(updated_iteration.start_date.timestamp_millis())
            .expect("Expected a valid start date timestamp.")
    );
    assert_eq!(
        updated_iteration_properties
            .end_date
            .and_then(|date| DateTime::from_timestamp_millis(date.timestamp_millis()))
            .expect("Expected a valid end date timestamp."),
        DateTime::from_timestamp_millis(updated_iteration.end_date.timestamp_millis())
            .expect("Expected a valid end date timestamp.")
    );
    assert_eq!(
        updated_iteration_properties
            .actual_start_date
            .expect("Expected an updated actual start date.")
            .and_then(|date| DateTime::from_timestamp_millis(date.timestamp_millis())),
        updated_iteration
            .actual_start_date
            .and_then(|date| DateTime::from_timestamp_millis(date.timestamp_millis()))
    ...
    assert_eq!(
        updated_iteration_properties
            .actual_end_date
            .expect("Expected an updated actual end date.")
            .and_then(|date| DateTime::from_timestamp_millis(date.timestamp_millis())),
        updated_iteration
            .actual_end_date
            .and_then(|date| DateTime::from_timestamp_millis(date.timestamp_millis()))
    ...
    assert_eq!(
        original_iteration.parent_project_id,
        updated_iteration.parent_project_id
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::iterations::iteration_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/iterations/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/iterations/{}", Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_authentication_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/iterations/{}", iteration.id))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
-> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_iterations_action =
        Action::get_by_name("iterations.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_iterations_action.id,
            &PermissionLevel::User,
        )
        .await?;

    // Set up the server and send the request.
    let maximum_iteration_display_name_length_configuration = Configuration::get_by_name(
        "iterations.maximumDisplayNameLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_iteration_display_name_length_configuration
        .update(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/iterations/{}", dummy_iteration.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
