...
async fn create_view_access_policy(
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::views::view_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/views/{}/access-policies", dummy_view.id))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_view = test_environment.create_random_view(None, None).await?;
    let shown_access_policy = create_view_access_policy(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/views/{}/access-policies", &dummy_view.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let query = format!(
        ...
        quote_literal(&dummy_view.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "scoped_resource_type = 'View' AND scoped_view_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_view.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
