...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, view::{EditableViewProperties, View}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::views::view_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_views_action =
        Action::get_by_name("views.get", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &get_views_action.id, &PermissionLevel::User)
        .await?;

    let view = test_environment.create_random_view(None, None).await?;

    let response = test_server
        .get(&format!("/views/{}", view.id))
    ...
    let get_view_response_body = response.json::<GetResourceResponseBody<View>>();
    let response_view = get_view_response_body.data;
    ...
    assert_eq!(response_view.description, view.description);
    assert_eq!(
        response_view.default_filter_query,
        view.default_filter_query
    );
    assert_eq!(
        response_view.parent_resource_type,
        view.parent_resource_type
    );
    assert_eq!(response_view.parent_project_id, view.parent_project_id);
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/views/not-a-uuid").await;
...
async fn verify_authentication_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError>
{
    ...
    let response = test_server.get(&format!("/views/{}", view.id)).await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/views/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_views_action =
        Action::get_by_name("views.delete", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &delete_views_action.id, &PermissionLevel::User)
        .await?;
    ...
    match View::get_by_id(&view.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an view not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/views/not-a-uuid").await;
...
async fn verify_authentication_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::views::view_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.delete(&format!("/views/{}", view.id)).await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/views/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_views_action =
        Action::get_by_name("views.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &update_views_action.id, &PermissionLevel::User)
        .await?;

    // Set up the server and send the request.
    let original_view = test_environment.create_random_view(None, None).await?;
    let updated_view_properties = EditableViewProperties {
        name: Some(Uuid::now_v7().to_string()),
        display_name: Some(Uuid::now_v7().to_string()),
        description: Some(Some(Uuid::now_v7().to_string())),
        default_filter_query: Some(Some("".to_string())),
        ..Default::default()
    };
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/views/{}", original_view.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
        .json(&serde_json::json!(updated_view_properties))
        .await;

    // Verify the response.
    assert_eq!(response.status_code(), StatusCode::OK);

    let patch_view_response_body: PatchResourceResponseBody<View> = response.json();
    let updated_view = patch_view_response_body.data;
    assert_eq!(original_view.id, updated_view.id);
    assert_eq!(
        updated_view.display_name,
        updated_view_properties.display_name.unwrap()
    );
    assert_eq!(updated_view.name, updated_view_properties.name.unwrap());
    assert_eq!(
        updated_view.description,
        updated_view_properties.description.unwrap()
    );
    assert_eq!(
        updated_view.default_filter_query,
        updated_view_properties.default_filter_query.unwrap()
    );
    assert_eq!(
        updated_view.parent_resource_type,
        original_view.parent_resource_type
    );
    assert_eq!(
        updated_view.parent_project_id,
        original_view.parent_project_id
    );
    assert_eq!(
        updated_view.parent_workspace_id,
        original_view.parent_workspace_id
    );
    assert_eq!(updated_view.r#type, original_view.r#type);
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::views::view_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/views/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/views/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/views/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
#[tokio::test]
async fn verify_view_display_name_is_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_view_action =
        Action::get_by_name("views.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &update_view_action.id, &PermissionLevel::User)
        .await?;

    // Set up the server and send the request.
    let original_view = test_environment.create_random_view(None, None).await?;
    let maximum_view_display_name_length_configuration = Configuration::get_by_name(
        "views.maximumDisplayNameLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_view_display_name_length_configuration
        .update(
...
/// Verifies that the server returns a 422 status code when view descriptions are over the maximum length.
#[tokio::test]
async fn verify_view_description_is_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
