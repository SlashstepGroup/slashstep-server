...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        view::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, View},
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::views::get_router(state.clone())
        .with_state(state)
    ...
    for actual_view in actual_views {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|view| view.id == actual_view.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/views")).add_query_param(
            "query",
            format!("id ~ {}", quote_literal(&get_views_action.id.to_string())),
        ),
        test_server
            .get(&format!("/views"))
        ...
        test_server.get(&format!("/views")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM views WHERE id = {}",
                quote_literal(&get_views_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::views::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/views")).await;
