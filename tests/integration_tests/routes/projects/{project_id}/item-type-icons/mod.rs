...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        item_type_icon::{
            DEFAULT_RESOURCE_LIST_LIMIT, ItemTypeIcon, ItemTypeIconParentResourceType,
        },
    },
...
async fn verify_successful_item_type_icon_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let create_item_type_icons_action =
        Action::get_by_name("itemTypeIcons.create", &test_environment.database_pool).await?;
    test_environment
    ...
    let router = slashstep_server::routes::projects::project_id::item_type_icons::get_router(state.clone())
        .with_state(state)
    ...
    for test_image_metadata in test_image_metadata_list {
      ...
      None => {
        ...
        let response = test_server
            .post(&format!("/projects/{}/item-type-icons", dummy_project.id))
...
async fn verify_returned_resource_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_project = test_environment.create_random_project(None).await?;
    let shown_item_type_icon = test_environment
        .create_random_item_type_icon(Some(&dummy_project.id))
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/projects/{}/item-type-icons", &dummy_project.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    .await?;
    assert_eq!(
        response_item_type_icons.total_count,
        actual_item_type_icon_count
    );

    let actual_item_type_icons = ItemTypeIcon::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    .await?;
    assert_eq!(
        response_item_type_icons.data.len(),
        actual_item_type_icons.len()
    );
    assert_eq!(
        response_item_type_icons.data[0].id,
        actual_item_type_icons[0].id
    );
...
async fn verify_query_when_listing_item_type_icons() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM item_type_icons WHERE action_id = {}",
                    get_item_type_icons_action.id
                ),
            ),
    ];
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("action_ied = {}", get_item_type_icons_action.id),
            ),
        test_server
            .get(&format!("/projects/{}/item-type-icons", &dummy_project.id))
