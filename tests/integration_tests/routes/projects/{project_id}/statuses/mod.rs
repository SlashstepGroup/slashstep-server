...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        status::{
            DEFAULT_RESOURCE_LIST_LIMIT, InitialStatusPropertiesWithPredefinedParent, Status,
            StatusType,
        },
    },
...
async fn verify_successful_status_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::projects::project_id::statuses::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/projects/{}/statuses", dummy_project.id))
    ...
    assert_eq!(
        response_status.display_name,
        initial_status_properties.display_name
    );
    assert_eq!(
        response_status.description,
        initial_status_properties.description
    );
    ...
    assert_eq!(
        response_status.decimal_color,
        initial_status_properties.decimal_color
    );
    assert_eq!(
        response_status.next_status_id,
        initial_status_properties.next_status_id
    );
...
async fn verify_status_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/projects/{}/statuses", project.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_status_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let project = test_environment.create_random_project(None).await?;
    let allowed_status_name_regex_configuration =
        Configuration::get_by_name("statuses.allowedNameRegex", &test_environment.database_pool)
            .await?;
    allowed_status_name_regex_configuration
        .update(
            &EditableConfigurationProperties {
...
async fn verify_returned_status_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_statuses_action =
        Action::get_by_name("statuses.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_statuses_action.id, &PermissionLevel::User)
        .await?;

    // Create dummy resources.
    let dummy_project = test_environment.create_random_project(None).await?;
    let shown_status = test_environment
        .create_random_status(Some(&dummy_project.id))
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/projects/{}/statuses", &dummy_project.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let actual_status_count = Status::count(
        &query,
        &test_environment.database_pool,
    ...
    let actual_statuses = Status::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM statuses WHERE action_id = {}",
