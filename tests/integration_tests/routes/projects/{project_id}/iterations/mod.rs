...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        iteration::{
            DEFAULT_RESOURCE_LIST_LIMIT, InitialIterationPropertiesWithPredefinedParent, Iteration,
        },
    },
...
async fn verify_successful_iteration_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let create_iterations_action =
        Action::get_by_name("iterations.create", &test_environment.database_pool).await?;
    test_environment
    ...
    let router = slashstep_server::routes::projects::project_id::iterations::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/projects/{}/iterations", dummy_project.id))
    ...
    assert_eq!(
        response_iteration.display_name,
        initial_iteration_properties.display_name
    );
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/projects/{}/iterations", project.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_returned_iteration_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_iterations_action =
        Action::get_by_name("iterations.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_iterations_action.id, &PermissionLevel::User)
        .await?;

    // Create dummy resources.
    let dummy_project = test_environment.create_random_project(None).await?;
    let shown_iteration = test_environment
        .create_random_iteration(Some(&dummy_project.id))
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/projects/{}/iterations", &dummy_project.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let actual_iteration_count = Iteration::count(
        &query,
        &test_environment.database_pool,
    ...
    let actual_iterations = Iteration::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
...
async fn verify_query_when_listing_iterations() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM iterations WHERE action_id = {}",
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("action_ied = {}", get_iterations_action.id),
            ),
        test_server
            .get(&format!("/projects/{}/iterations", &dummy_project.id))
