...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        item_type::{
            DEFAULT_RESOURCE_LIST_LIMIT, InitialItemTypePropertiesWithPredefinedParent, ItemType,
        },
    },
...
async fn verify_successful_item_type_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::projects::project_id::item_types::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/projects/{}/item-types", dummy_project.id))
    ...
    assert_eq!(
        response_item_type.display_name,
        initial_item_type_properties.display_name
    );
    assert_eq!(
        response_item_type.description,
        initial_item_type_properties.description
    );
    assert_eq!(
        response_item_type.item_type_icon_id,
        initial_item_type_properties.item_type_icon_id
    );
...
async fn verify_returned_item_type_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_item_types_action =
        Action::get_by_name("itemTypes.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_item_types_action.id, &PermissionLevel::User)
        .await?;

    // Create dummy resources.
    let dummy_project = test_environment.create_random_project(None).await?;
    let shown_item_type = test_environment
        .create_random_item_type(Some(&dummy_project.id))
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/projects/{}/item-types", &dummy_project.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let actual_item_types = ItemType::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
...
async fn verify_query_when_listing_item_types() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM item_types WHERE action_id = {}",
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("action_ied = {}", get_item_types_action.id),
            ),
        test_server
            .get(&format!("/projects/{}/item-types", &dummy_project.id))
