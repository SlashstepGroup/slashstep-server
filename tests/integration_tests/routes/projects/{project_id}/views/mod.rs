...
use slashstep_server::{
    ...
    resources::{
        ...
        view::{
            DEFAULT_RESOURCE_LIST_LIMIT, InitialViewPropertiesWithPredefinedParent, View,
            ViewParentResourceType, ViewType,
        },
    },
...
async fn verify_successful_view_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let create_views_action =
        Action::get_by_name("views.create", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &create_views_action.id, &PermissionLevel::User)
        .await?;
    ...
    let router = slashstep_server::routes::projects::project_id::views::get_router(state.clone())
        .with_state(state)
    ...
    assert_eq!(
        response_view.display_name,
        initial_view_properties.display_name
    );
    assert_eq!(
        response_view.default_filter_query,
        initial_view_properties.default_filter_query
    );
    assert_eq!(
        response_view.description,
        initial_view_properties.description
    );
    assert_eq!(
        response_view.parent_resource_type,
        ViewParentResourceType::Project
    );
...
async fn verify_view_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
    ...
    let maximum_view_name_length_configuration =
        Configuration::get_by_name("views.maximumNameLength", &test_environment.database_pool)
            .await?;
    maximum_view_name_length_configuration
        .update(
...
-> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let project = test_environment.create_random_project(None).await?;
    let maximum_view_description_length_configuration = Configuration::get_by_name(
        "views.maximumDescriptionLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_view_description_length_configuration
        .update(
            &EditableConfigurationProperties {
                number_value: Some(Decimal::from(0 as i64)),
                ..Default::default()
...
async fn verify_view_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let project = test_environment.create_random_project(None).await?;
    let allowed_view_name_regex_configuration =
        Configuration::get_by_name("views.allowedNameRegex", &test_environment.database_pool)
            .await?;
    allowed_view_name_regex_configuration
        .update(
            &EditableConfigurationProperties {
...
async fn verify_returned_view_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_views_action =
        Action::get_by_name("views.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_views_action.id, &PermissionLevel::User)
        .await?;

    // Create dummy resources.
    let dummy_project = test_environment.create_random_project(None).await?;
    let shown_view = test_environment
        .create_random_view(
            Some(&ViewParentResourceType::Project),
    ...
    let actual_view_count = View::count(
        &query,
        &test_environment.database_pool,
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "SELECT * FROM views WHERE action_id = {}",
                    get_views_action.id
                ),
            ),
    ];
