...
async fn create_app_access_policy(
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let shown_access_policy = create_app_access_policy(
    ...
    let router = slashstep_server::routes::apps::app_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/apps/{}/access-policies", &dummy_app.id))
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "scoped_resource_type = 'App' AND scoped_app_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_app.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
...
-> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/apps/{}/access-policies", &Uuid::now_v7()))
        .await;
...
async fn verify_successful_resource_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/apps/{}/access-policies", dummy_app.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    assert_eq!(
        dummy_app.id,
        response_access_policy
            .scoped_app_id
            .expect("App ID is not set.")
    );
...
async fn verify_not_found_when_creating_resource() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .post(&format!("/apps/{}/access-policies", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!(initial_access_policy_properties))
        .await;
