...
use slashstep_server::{
    ...
    resources::{
        ...
        action::{
            Action, ActionParentResourceType, DEFAULT_ACTION_LIST_LIMIT,
...
async fn verify_successful_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::apps::app_id::actions::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/apps/{}/actions", dummy_app.id))
    ...
    assert_eq!(
        initial_action_properties.display_name,
        response_action.display_name
    );
    assert_eq!(
        initial_action_properties.description,
        response_action.description
    );
    assert_eq!(
        dummy_app.id,
        response_action
            .parent_app_id
            .expect("Parent app ID is not set.")
    );
    assert_eq!(
        response_action.parent_resource_type,
...
async fn verify_not_found_when_creating_resource() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .post(&format!("/apps/{}/actions", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!(initial_action_properties))
        .await;
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    // Give the user access to the "actions.get" action.
    let plain_text_password = Uuid::now_v7().to_string();
    let user = test_environment
        .create_random_user(Some(&plain_text_password))
        .await?;
    ...
    let list_actions_action =
        Action::get_by_name("actions.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_actions_action.id, &PermissionLevel::User)
        .await?;

    // Create dummy resources.
    let dummy_app = test_environment.create_random_app(None, None).await?;
    let dummy_action = test_environment
        .create_random_action(Some(&dummy_app.id))
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/apps/{}/actions", &dummy_app.id))
        .add_cookie(Cookie::new("session_access_token", &session_token))
        .await;
    ...
    let query = format!(
        "parent_app_id = {}",
        quote_literal(&dummy_app.id.to_string())
    );
    let actual_action_count = Action::count(
        &query,
    ...
    let actual_access_policies = Action::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let actual_actions = Action::list(
        &query,
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
...
async fn verify_query_when_listing_resources() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!("SELECT * FROM actions WHERE id = {}", get_actions_action.id),
            ),
    ];
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("parenet_app_id = {}", get_actions_action.id),
            ),
        test_server
            .get(&format!("/apps/{}/actions", &dummy_app.id))
...
-> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/apps/{}/actions", &Uuid::now_v7()))
        .await;
