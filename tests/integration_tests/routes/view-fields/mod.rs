...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        view_field::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, ViewField},
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_delegation_policies_action =
        Action::get_by_name("viewFields.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    let router = slashstep_server::routes::view_fields::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/view-fields"))
    ...
    let actual_view_field_count = ViewField::count(
        "",
    ...
    let actual_delegation_policies = ViewField::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_view_field in actual_delegation_policies {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|view_field| view_field.id == actual_view_field.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - view_field_count + 1) {
        test_environment
            .create_random_view_field(Some(&parent_view.id), None)
            .await?;
    }
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/view-fields")).add_query_param(
            "query",
            format!(
                "id ~ {}",
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::view_fields::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/view-fields")).await;
