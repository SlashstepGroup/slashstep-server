...
async fn create_user_access_policy(
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::users::user_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/users/{}/access-policies", dummy_user.id))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_user = test_environment.create_random_user(None).await?;
    let shown_access_policy = create_user_access_policy(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/users/{}/access-policies", &dummy_user.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let query = format!(
        ...
        quote_literal(&dummy_user.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "scoped_resource_type = 'User' AND scoped_user_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_user.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
