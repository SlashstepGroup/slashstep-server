...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::groups::group_id::roles::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/groups/{}/roles", &dummy_group.id))
    ...
    .await?;
    assert_eq!(response_body.data.len(), actual_roles.len());
    assert_eq!(response_body.data[0].id, actual_roles[0].id);
    assert_eq!(response_body.data[0].id, dummy_role.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .get(&format!(
            "/groups/{}/roles",
            &dummy_role.parent_group_id.unwrap()
        ))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
...
async fn verify_query_when_listing_resources() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!("SELECT * FROM roles WHERE id = {}", get_roles_action.id),
            ),
    ];
...
-> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/groups/{}/roles", &Uuid::now_v7()))
        .await;
...
async fn verify_successful_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!("/groups/{}/roles", dummy_group.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_not_found_when_creating_resource() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .post(&format!("/groups/{}/roles", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!(initial_role_properties))
        .await;
