/**
 *
 * Any test cases for /delegation-policies/{delegation policy_id} should be handled here.
 *
 * Programmers:
...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, delegation_policy::DelegationPolicy
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::delegation_policies::delegation_policy_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_delegation_policies_action =
        Action::get_by_name("delegationPolicies.get", &test_environment.database_pool).await?;
    test_environment
    ...
    let response = test_server
        .get(&format!("/delegation-policies/{}", delegation_policy.id))
    ...
    let get_delegation_policy_response_body =
        response.json::<GetResourceResponseBody<DelegationPolicy>>();
    let response_delegation_policy = get_delegation_policy_response_body.data;
    assert_eq!(response_delegation_policy.id, delegation_policy.id);
    assert_eq!(
        response_delegation_policy.action_id,
        delegation_policy.action_id
    );
    assert_eq!(
        response_delegation_policy.maximum_permission_level,
        delegation_policy.maximum_permission_level
    );
    assert_eq!(
        response_delegation_policy.delegate_app_authorization_id,
        delegation_policy.delegate_app_authorization_id
    );
    assert_eq!(
        response_delegation_policy.principal_user_id,
        delegation_policy.principal_user_id
    );
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/delegation-policies/not-a-uuid").await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/delegation-policies/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_delegation_policies_action =
        Action::get_by_name("delegationPolicies.delete", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/delegation-policies/{}", delegation_policy.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    match DelegationPolicy::get_by_id(&delegation_policy.id, &test_environment.database_pool)
        .await
        .expect_err("Expected a delegation policy not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/delegation-policies/not-a-uuid").await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/delegation-policies/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_delegation_policies_action =
        Action::get_by_name("delegationPolicies.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_delegation_policies_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let response = test_server
        ...
        .json(&serde_json::json!({
          "maximum_permission_level": new_maximum_permission_level.clone()
        }))
        .await;
    ...
    let patch_delegation_policy_response_body: PatchResourceResponseBody<DelegationPolicy> =
        response.json();
    let updated_delegation_policy = patch_delegation_policy_response_body.data;
    assert_eq!(updated_delegation_policy.id, original_delegation_policy.id);
    assert_eq!(
        updated_delegation_policy.action_id,
        original_delegation_policy.action_id
    );
    assert_eq!(
        updated_delegation_policy.maximum_permission_level,
        new_maximum_permission_level
    );
    assert_eq!(
        updated_delegation_policy.delegate_app_authorization_id,
        original_delegation_policy.delegate_app_authorization_id
    );
    assert_eq!(
        updated_delegation_policy.principal_user_id,
        original_delegation_policy.principal_user_id
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::delegation_policies::delegation_policy_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/delegation-policies/not-a-uuid").await;
...
async fn verify_request_body_exists_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch("/delegation-policies/not-a-uuid")
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/delegation-policies/{}", delegation_policy.id))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!({
          "maximum_permission_level": "Super Admin" // This is not a valid permission level, so the router should return a 400 status code.
        }))
        .await;
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/delegation-policies/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
