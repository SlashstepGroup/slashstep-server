...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        app_credential::{AppCredential, DEFAULT_RESOURCE_LIST_LIMIT},
    },
...
async fn verify_returned_resource_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::app_credentials::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/app-credentials"))
    ...
    for actual_app_credential in actual_app_credentials {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|app_credential| app_credential.id == actual_app_credential.id);
...
async fn verify_authentication_when_listing_resources() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::app_credentials::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/app-credentials")).await;
