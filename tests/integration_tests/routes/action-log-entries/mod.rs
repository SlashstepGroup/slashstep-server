...
-> Result<(), TestSlashstepServerError> {
    ...
    let list_action_log_entries_action =
        Action::get_by_name("actionLogEntries.list", &test_environment.database_pool).await?;
    AccessPolicy::create(
        &InitialAccessPolicyProperties {
    ...
    let actual_action_count = ActionLogEntry::count(
        "",
        &test_environment.database_pool,
    ...
    let actual_action_log_entries = ActionLogEntry::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    .await?;
    assert_eq!(
        response_json.data.len() + LIST_OFFSET as usize,
        actual_action_log_entries.len()
    );
    ...
    for actual_action_log_entry in actual_action_log_entries {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|action_log_entry| action_log_entry.id == actual_action_log_entry.id);
        ...
        if !found_access_policy.is_some() {
            if remaining_list_offset > 0 {
                ...
                panic!(
                    "Couldn't find action log entry with ID {}.",
                    actual_action_log_entry.id
                );
            }
        }
    }
...
async fn verify_returned_action_log_entry_list_with_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let query = format!(
        "action_id = {}",
        quote_literal(&dummy_action_log_entry.action_id.to_string())
    );
    let response = test_server
        .get(&format!("/action-log-entries"))
        .add_cookie(Cookie::new(
    ...
    .await?;
    assert_eq!(response_json.data.len(), actual_action_log_entries.len());
...
async fn verify_default_action_log_entry_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let action_log_entry_count =
        ActionLogEntry::count("", &test_environment.database_pool, None, None).await?;
    ...
    assert_eq!(
        ...
        DEFAULT_ACTION_LOG_ENTRY_LIST_LIMIT as usize
    );
...
async fn verify_maximum_action_log_entry_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        ...
        .add_query_param(
            "query",
            format!("limit {}", DEFAULT_ACTION_LOG_ENTRY_LIST_LIMIT + 1),
        )
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
...
async fn verify_query_when_listing_action_log_entries() -> Result<(), TestSlashstepServerError> {
    ...
    let unprocessable_entity_requests = vec![
        test_server
            ...
            .add_query_param(
                "query",
                format!("action_ied = {}", get_action_log_entries_action.id),
            ),
        test_server
            .get(&format!("/action-log-entries"))
...
-> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::action_log_entries::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/action-log-entries")).await;
