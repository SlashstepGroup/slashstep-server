...
#[tokio::test]
async fn verify_successful_creation_for_public_client_with_authorization_code()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;

    // Create dummy resources.
    let json_web_token_private_key = get_json_web_token_private_key().await?;
    let dummy_oauth_authorization = test_environment
        .create_random_oauth_authorization(None, None)
        .await?;
    let authorization_code = dummy_oauth_authorization
        .generate_authorization_code(json_web_token_private_key.as_ref())?;
...
/// Verifies that the router can return a StatusCode::CREATED status code and the created resource.
#[tokio::test]
async fn verify_successful_creation_for_public_client_with_authorization_code_and_code_challenge()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    ...
    let dummy_oauth_authorization = test_environment
        .create_random_oauth_authorization(None, Some(&base64_hashed_code_challenge))
        .await?;
    let authorization_code = dummy_oauth_authorization
        .generate_authorization_code(json_web_token_private_key.as_ref())?;
    let create_oauth_access_token_query_parameters = CreateOAuthAccessTokenQueryParameters {
...
/// Verifies that the router can return a StatusCode::CREATED status code and the created resource.
#[tokio::test]
async fn verify_successful_creation_for_confidential_client_with_authorization_code()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
/// Verifies that the router can return a StatusCode::CREATED status code and the created resource.
#[tokio::test]
async fn verify_successful_creation_for_confidential_client_with_authorization_code_and_code_challenge()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
#[tokio::test]
async fn verify_client_secret_is_provided_for_confidential_client()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
#[tokio::test]
async fn verify_code_verifier_is_provided_when_code_challenge_is_present()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
#[tokio::test]
async fn verify_code_verifier_is_correct_when_code_challenge_is_present()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
/// Verifies that the router can return a StatusCode::CREATED status code and the created resource.
#[tokio::test]
async fn verify_successful_creation_for_public_client_with_refresh_token()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
/// Verifies that the router can return a StatusCode::CREATED status code and the created resource.
#[tokio::test]
async fn verify_successful_creation_for_confidential_client_with_refresh_token()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    ...
    let dummy_app_authorization = test_environment
        ...
        .await?;
    let app = App::get_by_id(
        &dummy_app_authorization.app_id,
        &test_environment.database_pool,
    )
    .await
    .expect("Expected a dummy app to exist.");
...
/// Verifies that the router can return a StatusCode::CREATED status code and the created resource.
#[tokio::test]
async fn verify_active_refresh_token_for_confidential_client()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
