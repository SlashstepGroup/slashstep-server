...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        item_type_icon::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, ItemTypeIcon,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_delegation_policies_action =
        Action::get_by_name("itemTypeIcons.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    let router = slashstep_server::routes::item_type_icons::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/item-type-icons"))
    ...
    let actual_item_type_icon_count = ItemTypeIcon::count(
        "",
    ...
    let actual_delegation_policies = ItemTypeIcon::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_item_type_icon in actual_delegation_policies {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|item_type_icon| item_type_icon.id == actual_item_type_icon.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "id = {}",
        quote_literal(&dummy_item_type_icon.id.to_string())
    );
    let response = test_server
        .get(&format!("/item-type-icons"))
        .add_cookie(Cookie::new(
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let item_type_icon_count =
        ItemTypeIcon::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_type_icons::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/item-type-icons")).await;
