...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::Action,
        http_transaction::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, HTTPTransaction,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_http_transactions_action =
        Action::get_by_name("httpTransactions.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &list_http_transactions_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let router = slashstep_server::routes::http_transactions::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/http-transactions"))
    ...
    let actual_http_transaction_count = HTTPTransaction::count(
        "",
    ...
    let actual_http_transactions = HTTPTransaction::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_http_transaction in actual_http_transactions {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|http_transaction| http_transaction.id == actual_http_transaction.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "id = {}",
        quote_literal(&dummy_http_transaction.id.to_string())
    );
    let response = test_server
        .get(&format!("/http-transactions"))
        .add_cookie(Cookie::new(
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let http_transaction_count =
        HTTPTransaction::count("", &test_environment.database_pool, None, None).await?;
    ...
    let bad_requests = vec![
        test_server
            ...
            .add_query_param(
                ...
                format!(
                    "id ~ {}",
                    quote_literal(&get_http_transactions_action.id.to_string())
                ),
            ),
        test_server
            .get(&format!("/http-transactions"))
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "id = null; SELECT * FROM http_transactions WHERE id = {}",
                    quote_literal(&get_http_transactions_action.id.to_string())
                ),
            ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::http_transactions::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/http-transactions")).await;
