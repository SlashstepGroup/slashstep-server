...
async fn create_http_transaction_access_policy(
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::http_transactions::http_transaction_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!(
            "/http-transactions/{}/access-policies",
            dummy_http_transaction.id
        ))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_http_transaction = test_environment.create_random_http_transaction().await?;
    let shown_access_policy = create_http_transaction_access_policy(
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_http_transaction = test_environment.create_random_http_transaction().await?;
    create_http_transaction_access_policy(
        &test_environment.database_pool,
        &dummy_http_transaction.id,
        &user.id,
        &list_access_policies_action.id,
    ...
    let query = format!(
        "scoped_resource_type = 'HTTPTransaction' AND scoped_http_transaction_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_http_transaction.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
