...
use slashstep_server::{
    ...
    resources::{
        ...
        membership_invitation::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, MembershipInvitation,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::membership_invitations::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/membership-invitations"))
    ...
    for actual_membership_invitation in actual_membership_invitations {
        let found_access_policy = response_json.data.iter().find(|membership_invitation| {
            membership_invitation.id == actual_membership_invitation.id
        });
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    // Create a dummy delegation policy.
    let dummy_membership_invitation = test_environment
        .create_random_membership_invitation()
        .await?;
    ...
    let query = format!(
        "id = {}",
        quote_literal(&dummy_membership_invitation.id.to_string())
    );
    let response = test_server
        .get(&format!("/membership-invitations"))
        .add_cookie(Cookie::new("session_access_token", &session_token))
    ...
    for actual_membership_invitation in actual_membership_invitations {
        let found_action = response_json.data.iter().find(|membership_invitation| {
            membership_invitation.id == actual_membership_invitation.id
        });
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let membership_invitation_count =
    ...
    let bad_requests = vec![
        test_server
            ...
            .add_query_param(
                ...
                format!(
                    "id ~ {}",
                    quote_literal(&get_membership_invitations_action.id.to_string())
                ),
            ),
        test_server
            .get(&format!("/membership-invitations"))
        ...
        test_server
            ...
            .add_query_param(
                "query",
                format!(
                    "id = null; SELECT * FROM membership_invitations WHERE id = {}",
                    quote_literal(&get_membership_invitations_action.id.to_string())
                ),
            ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::membership_invitations::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/membership-invitations")).await;
