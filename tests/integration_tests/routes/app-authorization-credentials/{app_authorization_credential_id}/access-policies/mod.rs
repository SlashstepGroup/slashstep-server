...
async fn create_app_authorization_credential_access_policy(
    ...
    let access_policy = AccessPolicy::create(
        &InitialAccessPolicyProperties {
            ...
            scoped_app_authorization_credential_id: Some(
                scoped_app_authorization_credential_id.clone(),
            ),
            ..Default::default()
        },
        database_pool,
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::app_authorization_credentials::app_authorization_credential_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!(
            "/app-authorization-credentials/{}/access-policies",
            dummy_app_authorization_credential.id
        ))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_app_authorization_credential = test_environment
        .create_random_app_authorization_credential(None)
        .await?;
    let shown_access_policy = create_app_authorization_credential_access_policy(
    ...
    let query = format!(
        ...
        quote_literal(&dummy_app_authorization_credential.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_app_authorization_credential = test_environment
        .create_random_app_authorization_credential(None)
        .await?;
    create_app_authorization_credential_access_policy(
        &test_environment.database_pool,
        &dummy_app_authorization_credential.id,
        &user.id,
        &list_access_policies_action.id,
    ...
    let query = format!(
        "scoped_resource_type = 'AppAuthorizationCredential' AND scoped_app_authorization_credential_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_app_authorization_credential.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
