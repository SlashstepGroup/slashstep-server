...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, status::{EditableStatusProperties, Status, StatusType}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::statuses::status_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_statuses_action =
        Action::get_by_name("statuses.get", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &get_statuses_action.id, &PermissionLevel::User)
        .await?;

    let status = test_environment.create_random_status(None).await?;

    let response = test_server
        .get(&format!("/statuses/{}", status.id))
    ...
    let get_status_response_body = response.json::<GetResourceResponseBody<Status>>();
    let response_status = get_status_response_body.data;
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/statuses/not-a-uuid").await;
...
async fn verify_authentication_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError>
{
    ...
    let response = test_server.get(&format!("/statuses/{}", status.id)).await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/statuses/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_statuses_action =
        Action::get_by_name("statuses.delete", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &delete_statuses_action.id, &PermissionLevel::User)
        .await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/statuses/{}", status.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    match Status::get_by_id(&status.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an status not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/statuses/not-a-uuid").await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/statuses/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let update_fields_action =
        Action::get_by_name("statuses.update", &test_environment.database_pool).await?;
    test_environment
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/statuses/{}", original_status.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_status_response_body: PatchResourceResponseBody<Status> = response.json();
    let updated_status = patch_status_response_body.data;
    ...
    assert_eq!(
        ...
        updated_status.display_name
    );
    assert_eq!(
        updated_status_properties
            .description
            .expect("Expected an updated description."),
        updated_status.description
    );
    assert_eq!(
        original_status.parent_project_id,
        updated_status.parent_project_id
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::statuses::status_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/statuses/not-a-uuid").await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/statuses/{}", Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_authentication_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/statuses/{}", status.id))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
async fn verify_status_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_statuses_action =
        Action::get_by_name("statuses.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &update_statuses_action.id, &PermissionLevel::User)
        .await?;

    // Set up the server and send the request.
    let maximum_status_name_length_configuration = Configuration::get_by_name(
        "statuses.maximumNameLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_status_name_length_configuration
        .update(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/statuses/{}", dummy_status.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_status_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    ...
    let create_statuses_action =
        Action::get_by_name("statuses.create", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &create_statuses_action.id, &PermissionLevel::User)
        .await?;

    // Set up the server and send the request.
    let status_name_regex_configuration =
        Configuration::get_by_name("statuses.allowedNameRegex", &test_environment.database_pool)
            .await?;
    status_name_regex_configuration
        .update(
            &EditableConfigurationProperties {
...
#[tokio::test]
async fn verify_status_display_name_is_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    ...
    // Set up the server and send the request.
    let maximum_status_display_name_length_configuration = Configuration::get_by_name(
        "statuses.maximumDisplayNameLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_status_display_name_length_configuration
        .update(
            &EditableConfigurationProperties {
                number_value: Some(Decimal::from(0 as i64)),
                ..Default::default()
...
#[tokio::test]
async fn verify_status_description_is_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    ...
    // Set up the server and send the request.
    let maximum_status_description_length_configuration = Configuration::get_by_name(
        "statuses.maximumDescriptionLength",
        &test_environment.database_pool,
    )
    .await?;
    maximum_status_description_length_configuration
        .update(
            &EditableConfigurationProperties {
                number_value: Some(Decimal::from(0 as i64)),
                ..Default::default()
