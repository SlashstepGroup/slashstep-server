...
async fn verify_returned_access_policy_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let get_access_policy_response_body: GetResourceResponseBody<AccessPolicy> = response.json();
    let response_access_policy = get_access_policy_response_body.data;
    ...
    assert_eq!(
        response_access_policy.principal_group_id,
        access_policy.principal_group_id
    );
    assert_eq!(
        response_access_policy.principal_role_id,
        access_policy.principal_role_id
    );
    assert_eq!(
        response_access_policy.principal_app_id,
        access_policy.principal_app_id
    );
    assert_eq!(
        response_access_policy.scoped_resource_type,
        access_policy.scoped_resource_type
    );
    assert_eq!(
        response_access_policy.scoped_action_id,
        access_policy.scoped_action_id
    );
    assert_eq!(
        response_access_policy.scoped_app_id,
        access_policy.scoped_app_id
    );
    assert_eq!(
        response_access_policy.scoped_group_id,
        access_policy.scoped_group_id
    );
    assert_eq!(
        response_access_policy.scoped_item_id,
        access_policy.scoped_item_id
    );
    assert_eq!(
        response_access_policy.scoped_milestone_id,
        access_policy.scoped_milestone_id
    );
    assert_eq!(
        response_access_policy.scoped_project_id,
        access_policy.scoped_project_id
    );
    assert_eq!(
        response_access_policy.scoped_role_id,
        access_policy.scoped_role_id
    );
    assert_eq!(
        response_access_policy.scoped_user_id,
        access_policy.scoped_user_id
    );
    assert_eq!(
        response_access_policy.scoped_workspace_id,
        access_policy.scoped_workspace_id
    );
...
-> Result<(), TestSlashstepServerError> {
    ...
    match AccessPolicy::get_by_id(&access_policy.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an access policy not found error.")
    {
...
async fn verify_successful_patch_access_policy_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let get_access_policy_response_body =
        response.json::<PatchResourceResponseBody<AccessPolicy>>();
    let response_access_policy = get_access_policy_response_body.data;
    assert_eq!(response_access_policy.id, access_policy.id);
    assert_eq!(response_access_policy.action_id, access_policy.action_id);
    assert_eq!(
...
-> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .patch("/access-policies/not-a-uuid")
...
-> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .patch(&format!("/access-policies/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "permission_level": "User",
          "is_inheritance_enabled": false
        }))
