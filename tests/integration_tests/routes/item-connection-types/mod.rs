...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        item_connection_type::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, ItemConnectionType,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connection_types::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/item-connection-types"))
    ...
    let actual_delegation_policies = ItemConnectionType::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_item_connection_type in actual_delegation_policies {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|item_connection_type| item_connection_type.id == actual_item_connection_type.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let query = format!(
        "id = {}",
        quote_literal(&dummy_item_connection_type.id.to_string())
    );
    let response = test_server
        .get(&format!("/item-connection-types"))
        .add_cookie(Cookie::new(
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
    ...
    let item_connection_type_count =
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connection_types::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/item-connection-types")).await;
