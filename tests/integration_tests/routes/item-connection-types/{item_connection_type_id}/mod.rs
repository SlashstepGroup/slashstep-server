...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ResourceError, access_policy::PermissionLevel, action::Action, configuration::{Configuration, EditableConfigurationProperties}, item_connection_type::{EditableItemConnectionTypeProperties, ItemConnectionType}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connection_types::item_connection_type_id::get_router(state.clone())
        .with_state(state)
    ...
    let item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    ...
    let get_item_connection_type_response_body =
        response.json::<GetResourceResponseBody<ItemConnectionType>>();
    let response_item_connection_type = get_item_connection_type_response_body.data;
    assert_eq!(response_item_connection_type.id, item_connection_type.id);
    assert_eq!(
        response_item_connection_type.display_name,
        item_connection_type.display_name
    );
    assert_eq!(
        response_item_connection_type.inward_description,
        item_connection_type.inward_description
    );
    assert_eq!(
        response_item_connection_type.outward_description,
        item_connection_type.outward_description
    );
    assert_eq!(
        response_item_connection_type.parent_resource_type,
        item_connection_type.parent_resource_type
    );
    assert_eq!(
        response_item_connection_type.parent_project_id,
        item_connection_type.parent_project_id
    );
    assert_eq!(
        response_item_connection_type.parent_workspace_id,
        item_connection_type.parent_workspace_id
    );
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/item-connection-types/not-a-uuid").await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/item-connection-types/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_item_connection_types_action = Action::get_by_name(
    ...
    match ItemConnectionType::get_by_id(&item_connection_type.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an item connection type not found error.")
    {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server
        .delete("/item-connection-types/not-a-uuid")
        .await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/item-connection-types/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_item_connection_types_action = Action::get_by_name(
    ...
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_item_connection_types_action.id,
            &PermissionLevel::User,
        )
        .await?;

    // Set up the server and send the request.
    let original_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    ...
    let response = test_server
        .patch(&format!(
            "/item-connection-types/{}",
            original_item_connection_type.id
        ))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
    ...
    let patch_item_connection_type_response_body: PatchResourceResponseBody<ItemConnectionType> =
        response.json();
    let updated_item_connection_type = patch_item_connection_type_response_body.data;
    assert_eq!(
        original_item_connection_type.id,
        updated_item_connection_type.id
    );
    assert_eq!(
        updated_item_connection_type.display_name,
        updated_item_connection_type_properties
            .display_name
            .unwrap()
    );
    assert_eq!(
        updated_item_connection_type.inward_description,
        updated_item_connection_type_properties
            .inward_description
            .unwrap()
    );
    assert_eq!(
        updated_item_connection_type.outward_description,
        updated_item_connection_type_properties
            .outward_description
            .unwrap()
    );
    assert_eq!(
        original_item_connection_type.parent_resource_type,
        updated_item_connection_type.parent_resource_type
    );
    assert_eq!(
        original_item_connection_type.parent_workspace_id,
        updated_item_connection_type.parent_workspace_id
    );
    assert_eq!(
        original_item_connection_type.parent_project_id,
        updated_item_connection_type.parent_project_id
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connection_types::item_connection_type_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/item-connection-types/not-a-uuid").await;
...
async fn verify_request_body_exists_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch("/item-connection-types/not-a-uuid")
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/item-connection-types/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/item-connection-types/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
#[tokio::test]
async fn verify_item_connection_type_display_name_is_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_item_connection_type_action = Action::get_by_name(
        "itemConnectionTypes.update",
        &test_environment.database_pool,
    )
    .await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_item_connection_type_action.id,
            &PermissionLevel::User,
        )
        .await?;
    ...
    let original_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    let maximum_item_connection_type_display_name_length_configuration =
        Configuration::get_by_name(
            "itemConnectionTypes.maximumDisplayNameLength",
            &test_environment.database_pool,
        )
        .await?;
    maximum_item_connection_type_display_name_length_configuration
        .update(
...
#[tokio::test]
async fn verify_item_connection_type_descriptions_are_at_most_at_maximum_length()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
