...
async fn create_item_connection_type_access_policy(
    database_pool: &deadpool_postgres::Pool,
    scoped_item_connection_type_id: &Uuid,
    user_id: &Uuid,
    action_id: &Uuid,
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::item_connection_types::item_connection_type_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!(
            "/item-connection-types/{}/access-policies",
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    let shown_access_policy = create_item_connection_type_access_policy(
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_item_connection_type = test_environment
        .create_random_item_connection_type()
        .await?;
    create_item_connection_type_access_policy(
        &test_environment.database_pool,
        &dummy_item_connection_type.id,
        &user.id,
        &list_access_policies_action.id,
    ...
    let query = format!(
        "scoped_resource_type = 'ItemConnectionType' AND scoped_item_connection_type_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_item_connection_type.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
