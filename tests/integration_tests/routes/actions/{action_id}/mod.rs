...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        action::{Action, EditableActionProperties},
...
async fn verify_returned_action_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::actions::action_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_action_response_body = response.json::<GetResourceResponseBody<Action>>();
    let response_action = get_action_response_body.data;
...
async fn verify_uuid_when_getting_action_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/actions/not-a-uuid").await;
...
async fn verify_authentication_when_getting_action_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get(&format!("/actions/{}", action.id)).await;
...
#[tokio::test]
async fn verify_successful_deletion_when_deleting_action_by_id()
-> Result<(), TestSlashstepServerError> {
    ...
    let delete_actions_action =
        Action::get_by_name("actions.delete", &test_environment.database_pool).await?;
    AccessPolicy::create(
    ...
    match Action::get_by_id(&action.id, &test_environment.database_pool)
        .await
...
async fn verify_uuid_when_deleting_action_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/actions/not-a-uuid").await;
...
async fn verify_authentication_when_deleting_action_by_id() -> Result<(), TestSlashstepServerError>
{
    ...
    let router = slashstep_server::routes::actions::action_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.delete(&format!("/actions/{}", action.id)).await;
...
async fn verify_successful_patch_action_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        ...
        .await?;
    let get_actions_action =
        Action::get_by_name("actions.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &get_actions_action.id, &PermissionLevel::User)
        .await?;
    ...
    let patch_action_response_body: PatchResourceResponseBody<Action> = response.json();
    let updated_action = patch_action_response_body.data;
    ...
    assert_eq!(
        original_action.parent_resource_type,
        updated_action.parent_resource_type
    );
...
async fn verify_content_type_when_patching_action_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::actions::action_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/actions/not-a-uuid").await;
...
#[tokio::test]
async fn verify_request_body_exists_when_patching_action_by_id()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
...
#[tokio::test]
async fn verify_request_body_json_when_patching_action_by_id()
-> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/actions/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
        .json(&serde_json::json!({
...
async fn verify_action_exists_when_patching_action() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/actions/{}", Uuid::now_v7()))
        .json(&serde_json::json!({
          "display_name": Uuid::now_v7().to_string()
        }))
        .await;
...
async fn verify_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_actions_action =
        Action::get_by_name("actions.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &update_actions_action.id, &PermissionLevel::User)
        .await?;

    // Set up the server and send the request.
    let maximum_name_length_configuration =
        Configuration::get_by_name("actions.maximumNameLength", &test_environment.database_pool)
            .await?;
...
#[tokio::test]
async fn verify_action_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    ...
    let create_actions_action =
        Action::get_by_name("actions.create", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &create_actions_action.id, &PermissionLevel::User)
        .await?;

    // Set up the server and send the request.
    let action_name_regex_configuration =
        Configuration::get_by_name("actions.allowedNameRegex", &test_environment.database_pool)
            .await?;
    action_name_regex_configuration
        .update(
            &EditableConfigurationProperties {
                text_value: Some("^$".to_string()), // This regex pattern doesn't allow any action names, so this should cause a validation error.
                ..Default::default()
            },
            &test_environment.database_pool,
        )
    ...
    let dummy_action = test_environment.create_random_action(None).await?;
    let editable_action_properties = EditableActionProperties {
...
#[tokio::test]
async fn verify_action_display_name_matches_regex() -> Result<(), TestSlashstepServerError> {
    let test_environment = IntegrationTestEnvironment::new().await?;
    ...
    // Set up the server and send the request.
    let action_display_name_regex_configuration = Configuration::get_by_name(
        "actions.allowedDisplayNameRegex",
        &test_environment.database_pool,
    )
    .await?;
    action_display_name_regex_configuration
        .update(
            &EditableConfigurationProperties {
                text_value: Some("^$".to_string()), // This regex pattern doesn't allow any action display names, so this should cause a validation error.
                ..Default::default()
            },
            &test_environment.database_pool,
        )
