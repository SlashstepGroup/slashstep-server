...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::field_choices::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/field-choices"))
    ...
    for actual_field_choice in actual_field_choices {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|field_choice| field_choice.id == actual_field_choice.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/field-choices")).add_query_param(
            "query",
            format!(
                "id ~ {}",
                quote_literal(&get_field_choices_action.id.to_string())
            ),
        ),
        test_server
            .get(&format!("/field-choices"))
        ...
        test_server.get(&format!("/field-choices")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM field_choices WHERE id = {}",
                quote_literal(&get_field_choices_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::field_choices::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/field-choices")).await;
