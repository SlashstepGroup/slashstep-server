...
use slashstep_server::{
    AppState, get_json_web_token_private_key, resources::{
        ...
        }, action::Action, app::App, configuration::{Configuration, EditableConfigurationProperties}, field_choice::{EditableFieldChoiceProperties, FieldChoice}
    },
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::field_choices::field_choice_id::get_router(state.clone())
        .with_state(state)
    ...
    let get_field_choices_action =
        Action::get_by_name("fieldChoices.get", &test_environment.database_pool).await?;
    test_environment
    ...
    let response = test_server
        .get(&format!("/field-choices/{}", field_choice.id))
    ...
    let get_field_response_body = response.json::<GetResourceResponseBody<FieldChoice>>();
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.get("/field-choices/not-a-uuid").await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/field-choices/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_fields_action =
        Action::get_by_name("fieldChoices.delete", &test_environment.database_pool).await?;
    AccessPolicy::create(
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/field-choices/{}", field_choice.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    match App::get_by_id(&field_choice.id, &test_environment.database_pool)
        .await
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let response = test_server.delete("/field-choices/not-a-uuid").await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .delete(&format!("/field-choices/{}", uuid::Uuid::now_v7()))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let session_token = session
        .generate_access_token(&json_web_token_private_key, session.expiration_date)
        .await?;
    let update_field_choices_action =
        Action::get_by_name("fieldChoices.update", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &update_field_choices_action.id,
            &PermissionLevel::User,
        )
        .await?;

    // Set up the server and send the request.
    let original_field_choice = test_environment.create_random_field_choice(None).await?;
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/field-choices/{}", original_field_choice.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
    ...
    let patch_field_choice_response_body: PatchResourceResponseBody<FieldChoice> = response.json();
    let updated_field_choice = patch_field_choice_response_body.data;
    assert_eq!(original_field_choice.id, updated_field_choice.id);
    assert_eq!(
        updated_field_choice.description,
        updated_field_choice_properties
            .description
            .expect("Expected an updated field choice description to compare.")
    );
    assert_eq!(
        updated_field_choice.text_value,
        updated_field_choice_properties
            .text_value
            .expect("Expected an updated field choice text value to compare.")
    );
    assert_eq!(
        updated_field_choice.number_value,
        original_field_choice.number_value
    );
    assert_eq!(
        updated_field_choice.timestamp_value,
        original_field_choice.timestamp_value
    );
    assert_eq!(
        updated_field_choice.stakeholder_type,
        original_field_choice.stakeholder_type
    );
    assert_eq!(
        updated_field_choice.stakeholder_user_id,
        original_field_choice.stakeholder_user_id
    );
    assert_eq!(
        updated_field_choice.stakeholder_group_id,
        original_field_choice.stakeholder_group_id
    );
    assert_eq!(
        updated_field_choice.stakeholder_app_id,
        original_field_choice.stakeholder_app_id
    );
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::field_choices::field_choice_id::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.patch("/field-choices/not-a-uuid").await;
...
async fn verify_request_body_exists_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch("/field-choices/not-a-uuid")
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/field-choices/{}", uuid::Uuid::now_v7()))
        .add_header("Content-Type", "application/json")
...
async fn verify_authentication_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/field-choices/{}", field_choice.id))
        .json(&serde_json::json!({}))
        .await;
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/field-choices/{}", Uuid::now_v7()))
        .json(&serde_json::json!({}))
        .await;
...
/// Verifies that the server returns a 422 status code when the field choice text is over the maximum length.
#[tokio::test]
async fn verify_text_value_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
    ...
    let test_server = TestServer::new(router);
    let response = test_server
        .patch(&format!("/field-choices/{}", dummy_field_choice.id))
        .add_cookie(Cookie::new(
            "session_access_token",
            &session_token,
        ))
