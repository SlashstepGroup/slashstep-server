...
async fn create_membership_access_policy(
    database_pool: &deadpool_postgres::Pool,
    scoped_membership_id: &Uuid,
    user_id: &Uuid,
    action_id: &Uuid,
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::memberships::membership_id::access_policies::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .post(&format!(
            "/memberships/{}/access-policies",
            dummy_membership.id
        ))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_membership = test_environment.create_random_membership().await?;
    let shown_access_policy = create_membership_access_policy(
    ...
    let query = format!(
        ...
        quote_literal(&dummy_membership.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_membership = test_environment.create_random_membership().await?;
    create_membership_access_policy(
        &test_environment.database_pool,
        &dummy_membership.id,
        &user.id,
        &list_access_policies_action.id,
    ...
    let query = format!(
        "scoped_resource_type = 'Membership' AND scoped_membership_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_membership.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
