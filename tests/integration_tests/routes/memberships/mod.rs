...
use slashstep_server::{
    ...
    resources::{
        ...
        action::Action,
        membership::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Membership,
        },
    },
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_memberships_action =
        Action::get_by_name("memberships.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
            &list_memberships_action.id,
    ...
    let router = slashstep_server::routes::memberships::get_router(state.clone())
        .with_state(state)
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server
        .get(&format!("/memberships"))
    ...
    let actual_membership_count = Membership::count(
        "",
    ...
    let actual_memberships = Membership::list(
        "",
        &test_environment.database_pool,
        Some(&AccessPolicyPrincipalType::User),
        Some(&user.id),
    ...
    for actual_membership in actual_memberships {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|membership| membership.id == actual_membership.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/memberships")).add_query_param(
            "query",
            format!(
                "id ~ {}",
                quote_literal(&get_memberships_action.id.to_string())
            ),
        ),
        test_server
            .get(&format!("/memberships"))
        ...
        test_server.get(&format!("/memberships")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM memberships WHERE id = {}",
                quote_literal(&get_memberships_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = slashstep_server::routes::memberships::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/memberships")).await;
