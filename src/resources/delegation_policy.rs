...
impl DelegationPolicy {
    ...
    pub async fn get_by_id(
        ...
        let database_client = database_pool.get().await?;
        let query =
            include_str!("../queries/delegation_policies/get_delegation_policy_row_by_id.sql");
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "An delegation policy with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableDelegationPolicyProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "maximum_permission_level",
            properties.maximum_permission_level.as_ref(),
        );
