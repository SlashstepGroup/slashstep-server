...
fn assert_app_credentials_are_equal(
    ...
    app_credential_2: &AppCredential,
) {
    ...
    assert_eq!(
        app_credential_1.expiration_date,
        app_credential_2.expiration_date
    );
    assert_eq!(
        app_credential_1.creation_ip_address,
        app_credential_2.creation_ip_address
    );
    assert_eq!(app_credential_1.public_key, app_credential_2.public_key);
}

fn assert_app_credential_is_equal_to_initial_properties(
    app_credential: &AppCredential,
    initial_properties: &InitialAppCredentialProperties,
) {
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let app_credential =
        AppCredential::create(&app_credential_properties, &test_environment.database_pool).await?;

    // Ensure that all the properties were set correctly.
    assert_app_credential_is_equal_to_initial_properties(
        &app_credential,
        &app_credential_properties,
    );
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    created_app_credential
        .delete(&test_environment.database_pool)
        .await?;

    // Ensure that the access policy is no longer in the database.
    match AppCredential::get_by_id(&created_app_credential.id, &test_environment.database_pool)
        .await
    {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_app_credential = test_environment.create_random_app_credential(None).await?;
    let retrieved_app_credential =
        AppCredential::get_by_id(&created_app_credential.id, &test_environment.database_pool)
            .await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let app_credential_with_same_app_id = test_environment
        .create_random_app_credential(Some(&created_app_credentials[0].app_id))
        .await?;
    created_app_credentials.push(app_credential_with_same_app_id);

    let query = format!("app_id = \"{}\"", created_app_credentials[0].app_id);
    let retrieved_app_credentials =
        AppCredential::list(&query, &test_environment.database_pool, None, None).await?;

    let created_app_credentials_with_specific_app_id: Vec<&AppCredential> = created_app_credentials
        .iter()
        .filter(|app_credential| app_credential.app_id == created_app_credentials[0].app_id)
        .collect();
    assert_eq!(
        created_app_credentials_with_specific_app_id.len(),
        retrieved_app_credentials.len()
    );
...
-> Result<(), TestSlashstepServerError> {
    ...
    let mut current_app_credentials =
        AppCredential::list("", &test_environment.database_pool, None, None).await?;
    ...
    // Get the "appCredentials.get" action one time.
    ...
    // Grant access to the "appCredentials.get" action to the user for half of the actions.
    ...
    let retrieved_app_credentials = AppCredential::list(
        "",
        &test_environment.database_pool,
    ...
    assert_eq!(
        allowed_app_credentials.len(),
        retrieved_app_credentials.len()
    );
    for allowed_app_credential in allowed_app_credentials {
        let retrieved_app_credential = &retrieved_app_credentials
            .iter()
            .find(|action| action.id == allowed_app_credential.id)
            .unwrap();
