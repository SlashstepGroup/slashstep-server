...
impl FieldChoice {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A field choice with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableFieldChoiceProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "timestamp_value",
            properties.timestamp_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "stakeholder_type",
            properties.stakeholder_type.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "stakeholder_user_id",
            properties.stakeholder_user_id.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "stakeholder_group_id",
            properties.stakeholder_group_id.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "stakeholder_app_id",
            properties.stakeholder_app_id.as_ref(),
        );
