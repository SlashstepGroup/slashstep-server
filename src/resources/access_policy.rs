...
pub const GET_RESOURCE_ACTION_NAME: &str = "accessPolicies.get";

#[derive(
    Debug, PartialEq, Eq, ToSql, FromSql, Clone, Copy, Serialize, Deserialize, Default, PartialOrd,
)]
...
impl FromStr for PermissionLevel {
    ...
    fn from_str(string: &str) -> Result<Self, Self::Err> {
        match string {
            ...
            _ => Err(ResourceError::UnexpectedEnumVariantError(
                string.to_string(),
            )),
        }
    }
...
impl AccessPolicy {
                ...
                .map_err(|error| match error.as_db_error() {
                    Some(db_error) => match db_error.code() {
                        &SqlState::UNIQUE_VIOLATION => ResourceError::ConflictError(
                            "An access policy with the same scope and action ID already exists."
                                .to_string(),
                        ),
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "An access policy with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    fn convert_from_row(row: &postgres::Row) -> Self {
        AccessPolicy {
            ...
            scoped_app_authorization_credential_id: row
                .get("scoped_app_authorization_credential_id"),
            ...
            scoped_password_reset_authorization_id: row
                .get("scoped_password_reset_authorization_id"),
    ...
    pub async fn update(
        &self,
        properties: &EditableAccessPolicyProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "permission_level",
            Some(&properties.permission_level),
        );
        let (mut parameter_boxes, mut query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "is_inheritance_enabled",
            Some(&properties.is_inheritance_enabled),
        );
