...
pub const ALLOWED_QUERY_KEYS: &[&str] = &["id", "user_id", "expiration_date"];
...
impl PasswordResetAuthorization {
    ...
    pub async fn create(
        ...
        let query = include_str!(
            "../../queries/password_reset_authorizations/insert_password_reset_authorization_row.sql"
        );
    ...
    pub async fn get_by_id(
        ...
        let database_client = database_pool.get().await?;
        let query = include_str!(
            "../../queries/password_reset_authorizations/get_password_reset_authorization_row_by_id.sql"
        );
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A password reset token with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
