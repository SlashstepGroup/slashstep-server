...
pub const UUID_QUERY_KEYS: &[&str] = &["id", "parent_project_id", "parent_workspace_id"];
...
impl ItemConnectionType {
    ...
    pub async fn create(
        ...
        let query =
            include_str!("../queries/item_connection_types/insert_item_connection_type_row.sql");
    ...
    fn parse_string_slashstepql_parameters<'a>(
        ...
        match key {
            "parent_resource_type" => {
                let scoped_resource_type =
                    match ItemConnectionTypeParentResourceType::from_str(value) {
    ...
    pub async fn update(
        &self,
        properties: &EditableItemConnectionTypeProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "inward_description",
            properties.inward_description.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "outward_description",
            properties.outward_description.as_ref(),
        );
