/**
 *
 * This module defines the implementation and types of an app.
 *
 * Programmers:
...
pub const UUID_QUERY_KEYS: &[&str] = &["id", "parent_workspace_id", "parent_user_id"];
...
impl App {
    ...
    /// Creates a new app.
    pub async fn create(
        initial_properties: &InitialAppProperties,
        database_pool: &deadpool_postgres::Pool,
                ...
                .map_err(|error| match error.as_db_error() {
                    Some(db_error) => match db_error.code() {
                        &SqlState::UNIQUE_VIOLATION => {
                            ResourceError::ConflictError(initial_properties.name.to_string())
                        }
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "An app with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableAppProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "client_type",
            properties.client_type.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "client_secret_hash",
            properties.client_secret_hash.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "parent_resource_type",
            properties.parent_resource_type.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "parent_workspace_id",
            properties.parent_workspace_id.as_ref(),
        );
        let (mut parameter_boxes, mut query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "parent_user_id",
            properties.parent_user_id.as_ref(),
        );
