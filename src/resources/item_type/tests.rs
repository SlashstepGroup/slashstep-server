...
use super::{DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, ItemType, InitialItemTypeProperties};

fn assert_item_types_are_equal(item_type_1: &ItemType, item_type_2: &ItemType) {

  assert_eq!(item_type_1.id, item_type_2.id);
  assert_eq!(item_type_1.name, item_type_2.name);
  assert_eq!(item_type_1.display_name, item_type_2.display_name);
  assert_eq!(item_type_1.parent_project_id, item_type_2.parent_project_id);
  assert_eq!(item_type_1.item_type_icon_id, item_type_2.item_type_icon_id);
  assert_eq!(item_type_1.description, item_type_2.description);

}

fn assert_item_type_is_equal_to_initial_properties(item_type: &ItemType, initial_properties: &InitialItemTypeProperties) {

  assert_eq!(item_type.name, initial_properties.name);
  assert_eq!(item_type.display_name, initial_properties.display_name);
  assert_eq!(item_type.parent_project_id, initial_properties.parent_project_id);
  assert_eq!(item_type.item_type_icon_id, initial_properties.item_type_icon_id);
  assert_eq!(item_type.description, initial_properties.description);
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resource_count = ItemType::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let item_type = ItemType::create(&item_type_properties, &test_environment.database_pool).await?;
  assert_item_type_is_equal_to_initial_properties(&item_type, &item_type_properties);
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
  ...
  match ItemType::get_by_id(&created_item_type.id, &test_environment.database_pool).await {
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = ItemType::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = ItemType::list(&query, &test_environment.database_pool, None, None).await?;

  let created_resources_with_specific_id: Vec<&ItemType> = created_resources.iter().filter(|item_type| item_type.id == created_resources[0].id).collect();
  ...
  for i in 0..created_resources_with_specific_id.len() {
    ...
    assert_item_types_are_equal(created_resource, retrieved_resource);
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  for i in 0..created_resources.len() {
    ...
    let created_item_type = &created_resources[i];
    let retrieved_resource = &retrieved_resources[i];
...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  let mut current_resources = ItemType::list("", &test_environment.database_pool, None, None).await?;
  ...
  // Get the "item_types.get" action one time.
  let user = test_environment.create_random_user().await?;
  let get_item_types_action = Action::get_by_name(GET_RESOURCE_ACTION_NAME, &test_environment.database_pool).await?;

  // Grant access to the "item_types.get" action to the user for half of the actions.
  let allowed_resource_count = current_resources.len() / 2;
  let mut allowed_resources = Vec::new();
  for index in 0..allowed_resource_count {

    let scoped_item_type = &current_resources[index];

    AccessPolicy::create(&InitialAccessPolicyProperties {
      action_id: get_item_types_action.id.clone(),
      ...
      scoped_resource_type: crate::resources::access_policy::ResourceType::ItemType,
      scoped_item_type_id: Some(scoped_item_type.id.clone()),
      ..Default::default()
    }, &test_environment.database_pool).await?;

    allowed_resources.push(scoped_item_type.clone());

  }

  // Make sure the user only sees the allowed actions.
  let retrieved_resources = ItemType::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  ...
  for allowed_resource in allowed_resources {
    ...
    assert_item_types_are_equal(&allowed_resource, retrieved_resource);
