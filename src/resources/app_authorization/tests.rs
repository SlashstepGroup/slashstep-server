...
fn assert_app_authorizations_are_equal(
    app_authorization_1: &AppAuthorization,
    app_authorization_2: &AppAuthorization,
) {
    assert_eq!(app_authorization_1.id, app_authorization_2.id);
    assert_eq!(app_authorization_1.app_id, app_authorization_2.app_id);
    assert_eq!(
        app_authorization_1.authorizing_resource_type,
        app_authorization_2.authorizing_resource_type
    );
    assert_eq!(
        app_authorization_1.authorizing_project_id,
        app_authorization_2.authorizing_project_id
    );
    assert_eq!(
        app_authorization_1.authorizing_workspace_id,
        app_authorization_2.authorizing_workspace_id
    );
    assert_eq!(
        app_authorization_1.authorizing_user_id,
        app_authorization_2.authorizing_user_id
    );
}
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_app_authorization_count =
        AppAuthorization::count("", &test_environment.database_pool, None, None).await?;

    assert_eq!(
        retrieved_app_authorization_count,
        MAXIMUM_APP_AUTHORIZATION_COUNT
    );
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
    ...
    let app_authorization = AppAuthorization::create(
        &app_authorization_properties,
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    match AppAuthorization::get_by_id(
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let created_app_authorization = test_environment
        .create_random_app_authorization(None)
        .await?;
    let retrieved_app_authorization = AppAuthorization::get_by_id(
        &created_app_authorization.id,
        &test_environment.database_pool,
    )
    .await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
    ...
    assert_eq!(
        retrieved_app_authorizations.len(),
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let app_authorization_with_same_app_id = test_environment
        .create_random_app_authorization(Some(&created_app_authorizations[0].app_id))
        .await?;
    ...
    let retrieved_app_authorizations =
        AppAuthorization::list(&query, &test_environment.database_pool, None, None).await?;

    let created_app_authorizations_with_specific_app_id: Vec<&AppAuthorization> =
        created_app_authorizations
            .iter()
            .filter(|app_authorization| {
                app_authorization.app_id == created_app_authorizations[0].app_id
            })
            .collect();
    assert_eq!(
        created_app_authorizations_with_specific_app_id.len(),
        retrieved_app_authorizations.len()
    );
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_app_authorizations =
        AppAuthorization::list("", &test_environment.database_pool, None, None).await?;
    assert_eq!(
        created_app_authorizations.len(),
        retrieved_app_authorizations.len()
    );
...
-> Result<(), TestSlashstepServerError> {
    ...
    let mut current_app_authorizations =
        AppAuthorization::list("", &test_environment.database_pool, None, None).await?;
    ...
    let retrieved_app_authorizations = AppAuthorization::list(
        "",
        &test_environment.database_pool,
    ...
    assert_eq!(
        allowed_app_authorizations.len(),
        retrieved_app_authorizations.len()
    );
    for allowed_app_authorization in allowed_app_authorizations {
        let retrieved_app_authorization = &retrieved_app_authorizations
            .iter()
            .find(|action| action.id == allowed_app_authorization.id)
            .unwrap();

        assert_app_authorizations_are_equal(
