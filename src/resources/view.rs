/*
 *
 * This module defines the implementation and types of a view.
 *
 * Programmers:
...
pub const UUID_QUERY_KEYS: &[&str] = &["id", "parent_workspace_id", "parent_project_id"];
...
impl View {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A view with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableViewProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "default_filter_query",
            properties.default_filter_query.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
