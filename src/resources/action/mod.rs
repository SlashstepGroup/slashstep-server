...
pub const ALLOWED_QUERY_KEYS: &[&str] =
    &["id", "name", "display_name", "description", "parent_app_id"];
pub const UUID_QUERY_KEYS: &[&str] = &["id", "parent_app_id"];
...
impl Action {
                ...
                .map_err(|error| match error.as_db_error() {
                    Some(db_error) => match db_error.code() {
                        &SqlState::UNIQUE_VIOLATION => ResourceError::ConflictError(
                            "An action with the same name already exists.".to_string(),
                        ),
    ...
    pub async fn initialize_resource_table(
        ...
        let database_client = database_pool.get().await?;
        let table_initialization_query =
            include_str!("../../queries/actions/initialize_actions_table.sql");
        database_client
    ...
    pub async fn update(
        &self,
        properties: &EditableActionProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
