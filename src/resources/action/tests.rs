...
fn assert_action_is_equal_to_initial_properties(
    action: &Action,
    initial_properties: &InitialActionProperties,
) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_action_count =
        Action::count("", &test_environment.database_pool, None, None).await?;

    assert_eq!(
        retrieved_action_count,
        previous_action_count + MAXIMUM_ACTION_COUNT
    );
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_action_result =
        Action::get_by_id(&created_action.id, &test_environment.database_pool).await;
...
async fn list_actions_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let action_with_same_display_name = Action::create(
        &InitialActionProperties {
            name: Uuid::now_v7().to_string(),
    ...
    let retrieved_actions =
        Action::list(&query, &test_environment.database_pool, None, None).await?;

    let created_actions_with_specific_display_name: Vec<&Action> = created_actions
        .iter()
        .filter(|action| action.display_name == created_actions[0].display_name)
        .collect();
    assert_eq!(
        created_actions_with_specific_display_name.len(),
        retrieved_actions.len()
    );
...
async fn list_actions_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    for created_action in created_actions {
        let retrieved_action = &retrieved_actions
            .iter()
            .find(|action| action.id == created_action.id)
            .unwrap();
...
-> Result<(), TestSlashstepServerError> {
    ...
    let retrieved_actions = Action::list(
        "",
        &test_environment.database_pool,
    ...
    for allowed_action in allowed_actions {
        let retrieved_action = &retrieved_actions
            .iter()
            .find(|action| action.id == allowed_action.id)
            .unwrap();
...
async fn update_action() -> Result<(), TestSlashstepServerError> {
    ...
    let updated_action = original_action
        .update(
