...
impl Role {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A role with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn get_by_predefined_role_type(
        ...
        predefined_role_type: &PredefinedRoleType,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        let database_client = database_pool.get().await?;
            ...
            .await
        {
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A protected server role with the type \"{}\" does not exist.",
                        predefined_role_type
                    )));
                }
            },
                ...
                .map_err(|error| match error.as_db_error() {
                    Some(db_error) => match db_error.code() {
                        &SqlState::UNIQUE_VIOLATION => ResourceError::ConflictError(
                            "A role with the same name and parent resource type already exists."
                                .to_string(),
                        ),
        ...
        match key {
            "predefined_role_type" => {
                let predefined_role_type = match value {
                    ...
                    _ => {
                        return Err(SlashstepQLError::StringParserError(format!(
                            "Failed to parse predefined role type from \"{}\" for key \"{}\".",
                            value, key
                        )));
                    }
                };
            ...
            "parent_resource_type" => {
                let parent_resource_type = match value {
                    ...
                    _ => {
                        return Err(SlashstepQLError::StringParserError(format!(
                            "Failed to parse role parent resource type from \"{}\" for key \"{}\".",
                            value, key
                        )));
                    }
                };
    ...
    pub async fn update(
        &self,
        properties: &EditableRoleProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
