...
use crate::{
    ...
    utilities::slashstepql::{
        ...
        SlashstepQLError, SlashstepQLFilterSanitizer, SlashstepQLParameterType,
        SlashstepQLParsedParameter, SlashstepQLSanitizeFunctionOptions,
    },
};
use chrono::DateTime;
...
pub const ALLOWED_QUERY_KEYS: &[&str] = &["id", "summary", "parent_project_id", "number"];
pub const UUID_QUERY_KEYS: &[&str] = &["id", "parent_project_id"];
...
impl Item {
        ...
        match identifier_parts[0] {
            "fields" => {
                ...
                if let Some(string_value) = assignment_properties.string_value {
                    ...
                    if let Ok(uuid_value) = Uuid::parse_str(&string_value) {
                        ...
                        for index in 0..uuid_column_name_map.len() {
                            ...
                            assignment_properties.where_clause.push_str(&format!(
                                "(field_values.value_type = {} AND field_values.{} {} ${})",
                                quote_literal(value_type),
                                column_name,
                                &assignment_properties.operator,
                                assignment_properties.parameters.len() + 1
                            ));
                            assignment_properties.parameters.push((
                                assignment_properties.key.clone(),
                        ...
                        assignment_properties.parameters.push((
                            assignment_properties.key.clone(),
                            SlashstepQLParameterType::Timestamp(datetime_value),
                        ));
                        assignment_properties.where_clause.push_str(" OR ")
                    }
                    ...
                    assignment_properties.where_clause.push_str(&format!(
                        "(field_values.value_type = 'Text' AND field_values.text_value {} ${})",
                        &assignment_properties.operator,
                        assignment_properties.parameters.len() + 1
                    ));
                    ...
                    assignment_properties.parameters.push((
                        assignment_properties.key.clone(),
                        SlashstepQLParameterType::String(string_value.clone()),
                    ));
    ...
    pub async fn update(
        &self,
        properties: &EditableItemProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
