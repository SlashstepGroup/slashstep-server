...
pub const UUID_QUERY_KEYS: &[&str] = &["id", "next_status_id", "parent_project_id"];
...
impl Status {
    ...
    pub async fn initialize_resource_table(
        ...
        let query = include_str!(
            "../queries/statuses/create_function_verify_next_status_parent_project_id.sql"
        );
        database_client.execute(query, &[]).await?;
            ...
            Err(error) => match error.as_db_error() {
                Some(db_error) => match db_error.code() {
                    &SqlState::RAISE_EXCEPTION => match db_error.message() {
                        "Next statuses must belong to the same parent project." => {
                            return Err(ResourceError::DifferentParentError(
                                "next_status_id".to_string(),
                            ));
                        }
    ...
    pub async fn update(
        &self,
        properties: &EditableStatusProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "type",
            properties.r#type.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "decimal_color",
            properties.decimal_color.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "next_status_id",
            properties.next_status_id.as_ref(),
        );
