/*
 *
 * This module defines the implementation and types of a workspace.
 *
 * Programmers:
...
pub const ALLOWED_QUERY_KEYS: &[&str] = &["id", "name", "display_name", "description"];
pub const UUID_QUERY_KEYS: &[&str] = &["id"];
pub const RESOURCE_NAME: &str = "Workspace";
pub const DATABASE_TABLE_NAME: &str = "workspaces";
pub const GET_RESOURCE_ACTION_NAME: &str = "workspaces.get";
...
impl Workspace {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A workspace with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableWorkspaceProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
