/*
 *
 * This module defines the implementation and types of a configuration.
 *
 * Programmers:
...
pub const ALLOWED_QUERY_KEYS: &[&str] = &["id", "name", "description", "value_type"];
...
#[derive(
    Debug, Clone, PartialEq, Eq, Serialize, Deserialize, Default, ToSql, FromSql, EnumIter,
)]
...
impl Configuration {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A configuration with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn get_by_name(
        ...
        let row = match database_client.query_opt(query, &[&name]).await {
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A configuration with the name \"{}\" does not exist.",
                        name
                    )));
                }
            },
    ...
    pub async fn create(
        initial_properties: &InitialConfigurationProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
                ...
                .map_err(|error| match error.as_db_error() {
                    Some(db_error) => match db_error.code() {
                        &SqlState::UNIQUE_VIOLATION => ResourceError::ConflictError(
                            "A configuration with the same name already exists.".to_string(),
                        ),
    ...
    pub async fn update(
        &self,
        properties: &EditableConfigurationProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "text_value",
            properties.text_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "number_value",
            properties.number_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "boolean_value",
            properties.boolean_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "default_text_value",
            properties.default_text_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "default_number_value",
            properties.default_number_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "default_boolean_value",
            properties.default_boolean_value.as_ref(),
        );
