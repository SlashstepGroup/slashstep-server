...
use super::{DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, Session, InitialSessionProperties};

fn assert_server_log_entries_are_equal(session_1: &Session, session_2: &Session) {

  assert_eq!(session_1.id, session_2.id);
  assert_eq!(session_1.user_id, session_2.user_id);
  assert_eq!(session_1.expiration_date, session_2.expiration_date);
  assert_eq!(session_1.creation_ip_address, session_2.creation_ip_address);

}

fn assert_session_is_equal_to_initial_properties(session: &Session, initial_properties: &InitialSessionProperties) {

  assert_eq!(session.user_id, initial_properties.user_id);
  assert_eq!(session.expiration_date, DateTime::from_timestamp_millis(initial_properties.expiration_date.timestamp_millis()).expect("Expected the expiration date to be a valid DateTime."));
  assert_eq!(session.creation_ip_address, initial_properties.creation_ip_address);
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
  ...
  let mut created_resources: Vec<Session> = Vec::new();
  for _ in 0..MAXIMUM_RESOURCE_COUNT {

    let resource = test_environment.create_random_session(None).await?;
    created_resources.push(resource);

  }

  let retrieved_resource_count = Session::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let session = Session::create(&session_properties, &test_environment.database_pool).await?;
  assert_session_is_equal_to_initial_properties(&session, &session_properties);
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
  ...
  let test_environment = TestEnvironment::new().await?;
  initialize_required_tables(&test_environment.database_pool).await?;
  let created_session = test_environment.create_random_session(None).await?;
  
  created_session.delete(&test_environment.database_pool).await?;

  // Ensure that the access policy is no longer in the database.
  match Session::get_by_id(&created_session.id, &test_environment.database_pool).await {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let created_session = test_environment.create_random_session(None).await?;
  let retrieved_resource = Session::get_by_id(&created_session.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
  ...
  initialize_required_tables(&test_environment.database_pool).await?;
  const MAXIMUM_RESOURCE_COUNT: i64 = DEFAULT_RESOURCE_LIST_LIMIT + 1;
  let mut server_log_entries: Vec<Session> = Vec::new();
  for _ in 0..MAXIMUM_RESOURCE_COUNT {

    let session = test_environment.create_random_session(None).await?;
    server_log_entries.push(session);

  }

  let retrieved_resources = Session::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = Session::list(&query, &test_environment.database_pool, None, None).await?;

  let created_resources_with_specific_id: Vec<&Session> = created_resources.iter().filter(|session| session.id == created_resources[0].id).collect();
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..MAXIMUM_RESOURCE_COUNT {
    ...
    let session = test_environment.create_random_session(None).await?;
    created_resources.push(session);
...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  let mut current_resources = Session::list("", &test_environment.database_pool, None, None).await?;
  if current_resources.len() < MINIMUM_RESOURCE_COUNT as usize {
    ...
    for _ in 0..remaining_action_count {
      ...
      let session = test_environment.create_random_session(None).await?;
      current_resources.push(session);
  ...
  for index in 0..allowed_resource_count {
    ...
    let scoped_session = &current_resources[index];
    ...
    AccessPolicy::create(&InitialAccessPolicyProperties {
      ...
      scoped_resource_type: crate::resources::access_policy::ResourceType::Session,
      scoped_session_id: Some(scoped_session.id.clone()),
      ..Default::default()
    }, &test_environment.database_pool).await?;

    allowed_resources.push(scoped_session);

  }

  // Make sure the user only sees the allowed actions.
  let retrieved_resources = Session::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
