...
use super::{DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, Iteration, InitialIterationProperties};

fn assert_iterations_are_equal(iteration_1: &Iteration, iteration_2: &Iteration) {
...
fn assert_iteration_is_equal_to_initial_properties(iteration: &Iteration, initial_properties: &InitialIterationProperties) {
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..MAXIMUM_RESOURCE_COUNT {
    ...
    let resource = test_environment.create_random_iteration(None).await?;
    created_resources.push(resource);

  }

  let retrieved_resource_count = Iteration::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let iteration = Iteration::create(&iteration_properties, &test_environment.database_pool).await?;
  assert_iteration_is_equal_to_initial_properties(&iteration, &iteration_properties);
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
  ...
  match Iteration::get_by_id(&created_iteration.id, &test_environment.database_pool).await {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let created_iteration = test_environment.create_random_iteration(None).await?;
  let retrieved_resource = Iteration::get_by_id(&created_iteration.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = Iteration::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = Iteration::list(&query, &test_environment.database_pool, None, None).await?;

  let created_resources_with_specific_id: Vec<&Iteration> = created_resources.iter().filter(|iteration| iteration.id == created_resources[0].id).collect();
  ...
  for i in 0..created_resources_with_specific_id.len() {
    ...
    assert_iterations_are_equal(created_resource, retrieved_resource);
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..MAXIMUM_RESOURCE_COUNT {
    ...
    let iteration = test_environment.create_random_iteration(None).await?;
    created_resources.push(iteration);
...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  let mut current_resources = Iteration::list("", &test_environment.database_pool, None, None).await?;
  if current_resources.len() < MINIMUM_RESOURCE_COUNT as usize {
    ...
    for _ in 0..remaining_action_count {
      ...
      let iteration = test_environment.create_random_iteration(None).await?;
      current_resources.push(iteration);

    }

  }

  // Get the "iterations.get" action one time.
  let user = test_environment.create_random_user().await?;
  let get_iterations_action = Action::get_by_name(GET_RESOURCE_ACTION_NAME, &test_environment.database_pool).await?;

  // Grant access to the "iterations.get" action to the user for half of the actions.
  let allowed_resource_count = current_resources.len() / 2;
  let mut allowed_resources = Vec::new();
  for index in 0..allowed_resource_count {

    let scoped_iteration = &current_resources[index];

    AccessPolicy::create(&InitialAccessPolicyProperties {
      action_id: get_iterations_action.id.clone(),
      ...
      scoped_resource_type: crate::resources::access_policy::ResourceType::Iteration,
      scoped_iteration_id: Some(scoped_iteration.id.clone()),
      ..Default::default()
    }, &test_environment.database_pool).await?;

    allowed_resources.push(scoped_iteration.clone());

  }

  // Make sure the user only sees the allowed actions.
  let retrieved_resources = Iteration::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  ...
  for allowed_resource in allowed_resources {
    ...
    assert_iterations_are_equal(&allowed_resource, retrieved_resource);
