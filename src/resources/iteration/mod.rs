...
impl Iteration {
    ...
    pub async fn create(
        ...
        let parameters: &[&(dyn ToSql + Sync)] = &[
            ...
            &initial_properties
                .actual_start_date
                .map(|date| DateTime::from_timestamp_millis(date.timestamp_millis())),
            &initial_properties
                .actual_end_date
                .map(|date| DateTime::from_timestamp_millis(date.timestamp_millis())),
        ];
    ...
    pub async fn update(
        &self,
        properties: &EditableIterationProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "actual_start_date",
            properties.actual_start_date.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "actual_end_date",
            properties.actual_end_date.as_ref(),
        );
