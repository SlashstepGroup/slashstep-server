/**
 *
 * This module defines the implementation and types of a field.
 *
 * Programmers:
...
impl Field {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "An field with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableFieldProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        ...
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            ...
            query,
            "minimum_value",
            properties.minimum_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "maximum_value",
            properties.maximum_value.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "minimum_choice_count",
            properties.minimum_choice_count.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "maximum_choice_count",
            properties.maximum_choice_count.as_ref(),
        );
        let (parameter_boxes, query) = slashstepql::add_parameter_to_query(
            parameter_boxes,
            query,
            "is_deadline",
            properties.is_deadline.as_ref(),
        );
