/*
 *
 * This module defines the implementation and types of a session.
 *
 * Programmers:
...
impl Session {
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A session with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn create(
        ...
        let expiration_date = match DateTime::from_timestamp_millis(
            initial_properties.expiration_date.timestamp_millis(),
        ) {
