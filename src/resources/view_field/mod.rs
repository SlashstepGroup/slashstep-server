...
impl ViewField {
  ...
  /// Creates a new field.
  pub async fn create(initial_properties: &InitialViewFieldProperties, database_pool: &deadpool_postgres::Pool) -> Result<Self, ResourceError> {
  ...
  pub async fn update(&self, properties: &EditableViewFieldProperties, database_pool: &deadpool_postgres::Pool) -> Result<Self, ResourceError> {
    ...
    let (parameter_boxes, query) = slashstepql::add_parameter_to_query(parameter_boxes, query, "next_view_field_id", properties.next_view_field_id.as_ref());
