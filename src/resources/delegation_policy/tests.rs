use crate::{
  initialize_required_tables, predefinitions::initialize_predefined_actions, resources::{
    ResourceError, access_policy::{AccessPolicy, ActionPermissionLevel, InitialAccessPolicyProperties, AccessPolicyPrincipalType}, action::{
      Action, DEFAULT_ACTION_LIST_LIMIT
    }
  }, tests::{TestEnvironment, TestSlashstepServerError}
};
use super::{DEFAULT_RESOURCE_LIST_LIMIT, GET_RESOURCE_ACTION_NAME, DelegationPolicy, InitialDelegationPolicyProperties};

fn assert_delegation_policies_are_equal(delegation_policy_1: &DelegationPolicy, delegation_policy_2: &DelegationPolicy) {

  assert_eq!(delegation_policy_1.id, delegation_policy_2.id);
  assert_eq!(delegation_policy_1.action_id, delegation_policy_2.action_id);
  assert_eq!(delegation_policy_1.maximum_permission_level, delegation_policy_2.maximum_permission_level);
  assert_eq!(delegation_policy_1.delegate_app_authorization_id, delegation_policy_2.delegate_app_authorization_id);
  assert_eq!(delegation_policy_1.principal_user_id, delegation_policy_2.principal_user_id);

}

fn assert_delegation_policy_is_equal_to_initial_properties(delegation_policy: &DelegationPolicy, initial_properties: &InitialDelegationPolicyProperties) {

  assert_eq!(delegation_policy.action_id, initial_properties.action_id);
  assert_eq!(delegation_policy.maximum_permission_level, initial_properties.maximum_permission_level);
  assert_eq!(delegation_policy.delegate_app_authorization_id, initial_properties.delegate_app_authorization_id);
  assert_eq!(delegation_policy.principal_user_id, initial_properties.principal_user_id);
...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..MAXIMUM_RESOURCE_COUNT {
    ...
    let resource = test_environment.create_random_delegation_policy().await?;
    created_resources.push(resource);

  }

  let retrieved_resource_count = DelegationPolicy::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let delegation_policy = DelegationPolicy::create(&delegation_policy_properties, &test_environment.database_pool).await?;

  // Ensure that all the properties were set correctly.
  assert_delegation_policy_is_equal_to_initial_properties(&delegation_policy, &delegation_policy_properties);
...
async fn verify_deletion() -> Result<(), TestSlashstepServerError> {
  ...
  initialize_required_tables(&test_environment.database_pool).await?;
  initialize_predefined_actions(&test_environment.database_pool).await?;
  let created_delegation_policy = test_environment.create_random_delegation_policy().await?;
  created_delegation_policy.delete(&test_environment.database_pool).await?;

  // Ensure that the access policy is no longer in the database.
  match DelegationPolicy::get_by_id(&created_delegation_policy.id, &test_environment.database_pool).await {
...
async fn verify_get_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let created_delegation_policy = test_environment.create_random_delegation_policy().await?;
  let retrieved_resource = DelegationPolicy::get_by_id(&created_delegation_policy.id, &test_environment.database_pool).await?;
...
async fn verify_list_resources_with_default_limit() -> Result<(), TestSlashstepServerError> {
  ...
  initialize_predefined_actions(&test_environment.database_pool).await?;
  const MAXIMUM_RESOURCE_COUNT: i64 = DEFAULT_RESOURCE_LIST_LIMIT + 1;
  let mut delegation_policies: Vec<DelegationPolicy> = Vec::new();
  for _ in 0..MAXIMUM_RESOURCE_COUNT {

    let delegation_policy = test_environment.create_random_delegation_policy().await?;
    delegation_policies.push(delegation_policy);

  }

  let retrieved_resources = DelegationPolicy::list("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = DelegationPolicy::list(&query, &test_environment.database_pool, None, None).await?;

  let created_resources_with_specific_id: Vec<&DelegationPolicy> = created_resources.iter().filter(|delegation_policy| delegation_policy.id == created_resources[0].id).collect();
  ...
  for i in 0..created_resources_with_specific_id.len() {
    ...
    assert_delegation_policies_are_equal(created_resource, retrieved_resource);
...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  let mut current_resources = DelegationPolicy::list("", &test_environment.database_pool, None, None).await?;
  if current_resources.len() < MINIMUM_RESOURCE_COUNT as usize {
    ...
    for _ in 0..remaining_action_count {
      ...
      let delegation_policy = test_environment.create_random_delegation_policy().await?;
      current_resources.push(delegation_policy);
  ...
  for index in 0..allowed_resource_count {
    ...
    let scoped_delegation_policy = &current_resources[index];
    ...
    AccessPolicy::create(&InitialAccessPolicyProperties {
      ...
      scoped_resource_type: crate::resources::access_policy::ResourceType::DelegationPolicy,
      scoped_delegation_policy_id: Some(scoped_delegation_policy.id.clone()),
      ..Default::default()
    }, &test_environment.database_pool).await?;

    allowed_resources.push(scoped_delegation_policy.clone());

  }

  // Make sure the user only sees the allowed actions.
  let retrieved_resources = DelegationPolicy::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  ...
  for allowed_resource in allowed_resources {
    ...
    assert_delegation_policies_are_equal(&allowed_resource, retrieved_resource);
