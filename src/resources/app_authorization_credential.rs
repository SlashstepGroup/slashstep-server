/**
 *
 * This module defines the implementation and types of an app authorization credential.
 *
 * Programmers:
...
pub const GET_RESOURCE_ACTION_NAME: &str = "appAuthorizationCredentials.get";
...
impl AppAuthorizationCredential {
    ...
    pub async fn get_by_id(
        ...
        let database_client = database_pool.get().await?;
        let query = include_str!(
            "../queries/app_authorization_credentials/get_app_authorization_credential_row_by_id.sql"
        );
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "An app authorization credential with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
