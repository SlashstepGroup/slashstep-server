...
impl OAuthAuthorization {
    ...
    pub async fn create(
        initial_properties: &InitialOAuthAuthorizationProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
        let query =
            include_str!("../queries/oauth_authorizations/insert_oauth_authorization_row.sql");
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "An oauth authorization with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
    ...
    pub async fn update(
        &self,
        properties: &EditableOAuthAuthorizationProperties,
        database_pool: &deadpool_postgres::Pool,
    ) -> Result<Self, ResourceError> {
