/*
 *
 * This module defines the implementation and types of a server log entry.
 *
 * Programmers:
...
pub const ALLOWED_QUERY_KEYS: &[&str] = &["id", "message", "http_transaction_id", "level"];
pub const UUID_QUERY_KEYS: &[&str] = &["id", "http_transaction_id"];
...
impl ServerLogEntry {
    ...
    pub async fn critical(
        message: &str,
        http_transaction_id: Option<&Uuid>,
        database_pool: &deadpool_postgres::Pool,
    ...
    pub async fn get_by_id(
        ...
        let database_client = database_pool.get().await?;
        let query =
            include_str!("../queries/server_log_entries/get_server_log_entry_row_by_id.sql");
            ...
            Ok(row) => match row {
                ...
                None => {
                    return Err(ResourceError::NotFoundError(format!(
                        "A server log entry with the ID \"{}\" does not exist.",
                        id
                    )));
                }
            },
