...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resource_count = Group::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = Group::list(&query, &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  let mut current_resources = Group::list("", &test_environment.database_pool, None, None).await?;
  ...
  for index in 0..allowed_resource_count {
    ...
    AccessPolicy::create(&InitialAccessPolicyProperties {
      ...
      scoped_resource_type: crate::resources::access_policy::ResourceType::Group,
  ...
  let retrieved_resources = Group::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
