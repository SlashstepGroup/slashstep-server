...
async fn verify_count() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resource_count = Item::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_list_resources_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  for created_resource in created_resources {
    ...
    let retrieved_resource = retrieved_resources.iter().find(|retrieved_resource| retrieved_resource.id == created_resource.id).expect("Expected a retrieved resource with the same ID.");
    assert_fields_are_equal(&created_resource, retrieved_resource);
...
async fn verify_list_resources_without_query_and_filter_based_on_requestor_permissions() -> Result<(), TestSlashstepServerError> {
  ...
  let retrieved_resources = Item::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
