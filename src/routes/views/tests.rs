/**
 * 
 * Any test cases for /views should be handled here.
 * 
 * Programmers: 
...
use crate::{
  AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, action::Action, view::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, View},
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "views.get" action to the user.
  ...
  // Grant access to the "views.list" action to the user.
  let list_views_action = Action::get_by_name("views.list", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &list_views_action.id, &PermissionLevel::User).await?;

  // Create a dummy delegation policy.
  test_environment.create_random_view(None, None).await?;
  ...
  let response = test_server.get(&format!("/views"))
  ...
  let response_json: ListResourcesResponseBody::<View> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_view_count = View::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_view_count);

  let actual_views = View::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_views.len());

  for actual_view in actual_views {

    let found_access_policy = response_json.resources.iter().find(|view| view.id == actual_view.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_view.id.to_string()));
  let response = test_server.get(&format!("/views"))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", session_token)))
  ...
  for actual_view in actual_views {
    ...
    let found_action = response_json.resources.iter().find(|view| view.id == actual_view.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let view_count = View::count("", &test_environment.database_pool, None, None).await?;
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - view_count + 1) {
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/views"))
      .add_query_param("query", format!("id ~ {}", quote_literal(&get_views_action.id.to_string()))),
    test_server.get(&format!("/views"))
      .add_query_param("query", format!("SELECT * FROM views")),
    test_server.get(&format!("/views"))
      .add_query_param("query", format!("SELECT PG_SLEEP(10)")),
    test_server.get(&format!("/views"))
      .add_query_param("query", format!("id = null; SELECT * FROM views WHERE id = {}", quote_literal(&get_views_action.id.to_string())))
  ];
