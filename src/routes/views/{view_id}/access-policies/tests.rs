/**
 * 
 * Any test cases for /views/{view_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
async fn create_view_access_policy(database_pool: &deadpool_postgres::Pool, scoped_view_id: &Uuid, user_id: &Uuid, action_id: &Uuid, permission_level: &PermissionLevel) -> Result<AccessPolicy, TestSlashstepServerError> {
  ...
  let access_policy = AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_view_id: Some(scoped_view_id.clone()),
    ..Default::default()
  }, database_pool).await?;
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_view = test_environment.create_random_view(None, None).await?;
  ...
  let response = test_server.post(&format!("/views/{}/access-policies", dummy_view.id))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_view = test_environment.create_random_view(None, None).await?;
  let shown_access_policy = create_view_access_policy(&test_environment.database_pool, &dummy_view.id, &user.id, &list_access_policies_action.id, &PermissionLevel::User).await?;
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.get(&format!("/views/{}/access-policies", &dummy_view.id))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", &session_token)))
    .await;
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_view = test_environment.create_random_view(None, None).await?;
  create_view_access_policy(&test_environment.database_pool, &dummy_view.id, &user.id, &list_access_policies_action.id, &PermissionLevel::User).await?;

  let shown_access_policy = create_view_access_policy(&test_environment.database_pool, &dummy_view.id, &user.id, &get_access_policies_action.id, &PermissionLevel::Editor).await?;
...
async fn verify_default_resource_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT + 1) {
    ...
    create_view_access_policy(&test_environment.database_pool, &dummy_view.id, &random_user.id, &random_action.id, &PermissionLevel::User).await?;
...
async fn verify_query_when_listing_access_policies() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/views/{}/access-policies", &dummy_view.id))
