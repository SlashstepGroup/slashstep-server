...
async fn handle_list_http_transactions_request(
        ...
        Err(error) => {
            let http_error = match error {
                ...
                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list HTTP transactions: {:?}",
                    error
                ))),
            };
    ...
    info!("Successfully returned {} {}.", queried_http_transaction_list_length, if queried_http_transaction_list_length == 1 { "HTTP transaction" } else { "HTTP transactions" });
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/http-transactions",
            axum::routing::get(handle_list_http_transactions_request),
        )
