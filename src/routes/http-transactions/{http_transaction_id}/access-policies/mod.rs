use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_http_transaction_by_id, get_principal_type_and_id_from_principal,
...
async fn handle_list_access_policies_request(
    ...
    .await?;
    let http_transaction = get_http_transaction_by_id(
        &http_transaction_id,
        &http_transaction,
        &state.database_pool,
    )
...
async fn handle_create_access_policy_request(
    ...
    trace!("Creating access policy for HTTP transaction {}...", http_transaction_id);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/http-transactions/{http_transaction_id}/access-policies",
