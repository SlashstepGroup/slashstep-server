/**
 * 
 * Any functionality for /http-transactions/{http_transaction_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
use crate::{AppState, HTTPError, middleware::{authentication_middleware, http_transaction_middleware}, resources::{ResourceType, ResourceError, access_policy::{AccessPolicy, ActionPermissionLevel, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, InitialAccessPolicyProperties, InitialAccessPolicyPropertiesForPredefinedScope}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, InitialActionLogEntryProperties}, app::App, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User}, routes::{ListResourcesResponseBody, ResourceListQueryParameters}, utilities::route_handler_utilities::{get_action_by_id, get_action_by_name, get_action_log_entry_expiration_timestamp, get_http_transaction_by_id, get_principal_type_and_id_from_principal, get_request_body_without_json_rejection, get_uuid_from_string, is_authenticated_user_anonymous, match_db_error, match_slashstepql_error, verify_delegate_permissions, verify_principal_permissions}};

#[cfg(test)]
mod tests;

/// GET /http-transactions/{http_transaction_id}/access-policies
/// 
/// Lists access policies for a HTTP transaction.
#[axum::debug_handler]
async fn handle_list_access_policies_request(
  ...
  let http_transaction = get_http_transaction_by_id(&http_transaction_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::HTTPTransaction, Some(&http_transaction.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  let query = format!(
    "scoped_resource_type = 'HTTPTransaction' AND scoped_http_transaction_id = {}{}", 
    quote_literal(&http_transaction_id.to_string()), 
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_resource_type: ResourceType::HTTPTransaction,
    target_http_transaction_id: Some(http_transaction_id),
    ..Default::default()
...
/// POST /http-transactions/{http_transaction_id}/access-policies
/// 
/// Creates an access policy for a HTTP transaction.
#[axum::debug_handler]
async fn handle_create_access_policy_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::HTTPTransaction, Some(&target_http_transaction.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::HTTPTransaction, Some(&target_http_transaction.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for HTTP transaction {}...", http_transaction_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_resource_type: ResourceType::HTTPTransaction,
    scoped_http_transaction_id: Some(target_http_transaction.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/http-transactions/{http_transaction_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/http-transactions/{http_transaction_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
