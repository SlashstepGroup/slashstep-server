...
use crate::{
  Action, AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, app::{App, AppClientType}, http_transaction::HTTPTransaction,
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_instance_access_policy(&user.id, &get_http_transactions_action.id, &ActionPermissionLevel::User).await?;
...
//   match App::get_by_id(&http_transaction.id, &test_environment.database_pool).await.expect_err("Expected an app not found error.") {
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.patch(&format!("/http-transactions/{}", original_http_transaction.id))
//     .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
...
//   let updated_app: Http_transaction = response.json();
//   assert_eq!(original_http_transaction.id, updated_http_transaction.id);
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.patch(&format!("/http-transactions/{}", uuid::Uuid::now_v7()))
//     .add_header("Content-Type", "application/json")
//     .json(&serde_json::json!({
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.patch(&format!("/http-transactions/{}", http_transaction.id))
//     .json(&serde_json::json!({
//       "display_name": Uuid::now_v7().to_string()
//     }))
//     .await;
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.patch(&format!("/http-transactions/{}", Uuid::now_v7()))
//     .json(&serde_json::json!({
//       "display_name": Uuid::now_v7().to_string()
//     }))
//     .await;
