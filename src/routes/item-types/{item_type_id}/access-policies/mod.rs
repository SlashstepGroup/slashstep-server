use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_item_type_by_id, get_principal_type_and_id_from_principal,
...
async fn handle_list_access_policies_request(
    ...
    let item_type = get_item_type_by_id(&item_type_id, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
    ...
    trace!("Creating access policy for item type {}...", item_type_id);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/item-types/{item_type_id}/access-policies",
