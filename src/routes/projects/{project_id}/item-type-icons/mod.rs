...
use crate::{
    ...
    resources::{
        ...
        access_policy::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, PermissionLevel},
        action_log_entry::{
...
async fn handle_list_item_type_icons_request(
        ...
        Err(error) => {
            let http_error = match error {
                ...
                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list item type icons: {:?}",
                    error
                ))),
            };
    ...
    ServerLogEntry::trace(
        &format!("Counting item type icons..."),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
...
impl From<TypedMultipartError> for HTTPError {
    fn from(error: TypedMultipartError) -> Self {
        match error {
            TypedMultipartError::FieldTooLarge {
                field_name,
                limit_bytes,
            } => HTTPError::PayloadTooLarge(Some(format!(
                "The file provided for {} exceeds the maximum allowed size of {} bytes.",
                field_name, limit_bytes
            ))),

            TypedMultipartError::InvalidRequest { source: _ } => HTTPError::BadRequest(Some(
                format!("Failed to parse request body. Ensure the request body is valid JSON."),
            )),

            TypedMultipartError::InvalidRequestBody { source: _ } => HTTPError::BadRequest(Some(
                format!("Failed to parse request body. Ensure the request body is valid JSON."),
            )),

            TypedMultipartError::DuplicateField { field_name } => {
                HTTPError::BadRequest(Some(format!(
                    "The \"{}\" field is duplicated. There should only be one.",
                    field_name
                )))
            }
            ...
            TypedMultipartError::MissingField { field_name } => {
                HTTPError::BadRequest(Some(format!(
                    "The \"{}\" field is required. Provide it, then try again.",
                    field_name
                )))
            }

            TypedMultipartError::NamelessField => HTTPError::BadRequest(Some(format!(
                "One of the fields in the multipart form data is missing a name. Ensure all fields have names, then try again."
            ))),
...
async fn handle_create_item_type_icon_request(
    ...
    async fn validate_content_type(
        ...
        let allowed_content_types = vec!["image/png", "image/jpeg", "image/gif", "image/svg+xml"];

        if !allowed_content_types.contains(&content_type) {
            let http_error = HTTPError::UnsupportedMediaType(Some(format!(
    ...
    async fn sanitize_contents(
        ...
        if content_type == "image/svg+xml" {
            ...
            let mut cleaned_bytes = Vec::new();
            {
                ...
                svg_filter_result =
                    svg_filter.filter(&mut svg_string.as_bytes(), &mut cleaned_bytes)
            }

            if svg_filter_result.is_err() {
                let http_error =
                    HTTPError::BadRequest(Some(format!("The SVG file could not be parsed.")));
                ServerLogEntry::from_http_error(
                    &http_error,
                    Some(&http_transaction.id),
                    &database_pool,
    ...
    async fn get_item_type_icon_storage_directory_path(
        ...
        ServerLogEntry::trace(
            &format!("Getting configuration to know where to store item type icons..."),
            Some(&http_transaction.id),
            &database_pool,
        )
        .await
        ...
        let item_type_icon_storage_directory_path =
            match item_type_icon_storage_directory_path_configuration
                .text_value
                .or(item_type_icon_storage_directory_path_configuration.default_text_value)
            {
                Some(storage_directory_path) => storage_directory_path,

                None => {
                    let http_error = HTTPError::InternalServerError(Some(format!(
                        "The configuration for the item type icon storage directory does not have a value."
                    )));
                    ServerLogEntry::from_http_error(
                        &http_error,
                        Some(&http_transaction.id),
    ...
    let content_type = match &body.icon_data.metadata.content_type {
        ...
        None => {
            let http_error = HTTPError::BadRequest(Some(format!(
                "The field \"icon_data\" must have a content type."
            )));
            ServerLogEntry::from_http_error(
                &http_error,
                Some(&http_transaction.id),
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        .route(
            ...
            axum::routing::get(handle_list_item_type_icons_request),
        )
        .route(
            "/projects/{project_id}/item-type-icons",
            axum::routing::post(handle_create_item_type_icon_request),
        )
