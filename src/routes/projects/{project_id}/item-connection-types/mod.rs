...
async fn handle_list_item_connection_types_request(
    ...
    (
        StatusCode,
        Json<ListResourcesResponseBody<ItemConnectionType>>,
    ),
    HTTPError,
> {
        ...
        Err(error) => {
            let http_error = match error {
                ...
                ResourceError::PostgresError(error) => {
                    match_db_error(&error, "item connection types")
                }

                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list item connection types: {:?}",
                    error
                ))),
            };
    ...
    ServerLogEntry::trace(
        &format!("Counting item connection types..."),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
...
async fn handle_create_item_connection_type_request(
    ...
    .await?;
    validate_field_length(
        &item_connection_type_properties_json.inward_description,
    ...
    .await?;
    validate_field_length(
        &item_connection_type_properties_json.outward_description,
        "itemConnectionTypes.maximumDescriptionLength",
        "outward_description",
        &http_transaction,
        &state.database_pool,
    ...
    verify_principal_permissions(
        ...
        &ResourceType::Project,
        Some(&target_project.id),
        &create_item_connection_types_action,
        &http_transaction,
        &PermissionLevel::User,
        &state.database_pool,
    )
    ...
    let item_connection_type = match ItemConnectionType::create(
        &InitialItemConnectionTypeProperties {
            ...
            inward_description: item_connection_type_properties_json
                .inward_description
                .clone(),
            outward_description: item_connection_type_properties_json
                .outward_description
                .clone(),
    ...
    .await
    {
        ...
        Err(error) => {
            let http_error = HTTPError::InternalServerError(Some(format!(
                "Failed to create item connection type: {:?}",
                error
            )));
            ServerLogEntry::from_http_error(
                &http_error,
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        .route(
            "/projects/{project_id}/item-connection-types",
            axum::routing::get(handle_list_item_connection_types_request),
        )
        .route(
            "/projects/{project_id}/item-connection-types",
            axum::routing::post(handle_create_item_connection_type_request),
        )
