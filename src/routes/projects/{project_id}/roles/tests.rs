/**
 * 
 * Any test cases for /projects/{project_id}/roles should be handled here.
 * 
 * Programmers: 
...
async fn verify_successful_role_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.post(&format!("/projects/{}/roles", dummy_project.id))
  ...
  assert_eq!(response_role.parent_resource_type, RoleParentResourceType::Project);
  assert_eq!(response_role.parent_project_id, Some(dummy_project.id));
...
async fn verify_role_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
  ...
  let maximum_role_name_length_configuration = Configuration::get_by_name("roles.maximumNameLength", &test_environment.database_pool).await?;
  maximum_role_name_length_configuration.update(&EditableConfigurationProperties {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.post(&format!("/projects/{}/roles", project.id))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", session_token)))
    .json(&serde_json::json!(initial_role_properties))
    .await;
...
async fn verify_role_description_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
  ...
  // Set up the server and send the request.
  let project = test_environment.create_random_project(None).await?;
  let maximum_role_description_length_configuration = Configuration::get_by_name("roles.maximumDescriptionLength", &test_environment.database_pool).await?;
  maximum_role_description_length_configuration.update(&EditableConfigurationProperties {
    number_value: Some(Decimal::from(0 as i64)),
    ..Default::default()
  }, &test_environment.database_pool).await?;
...
async fn verify_role_name_matches_regex() -> Result<(), TestSlashstepServerError> {
  ...
  // Set up the server and send the request.
  let project = test_environment.create_random_project(None).await?;
  let allowed_role_name_regex_configuration = Configuration::get_by_name("roles.allowedNameRegex", &test_environment.database_pool).await?;
  allowed_role_name_regex_configuration.update(&EditableConfigurationProperties {
...
async fn verify_returned_role_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_server_access_policy(&user.id, &list_roles_action.id, &PermissionLevel::User).await?;

  // Create dummy resources.
  let dummy_project = test_environment.create_random_project(None).await?;
  let shown_role = test_environment.create_random_role(Some(&RoleParentResourceType::Project), Some(&dummy_project.id)).await?;
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.get(&format!("/projects/{}/roles", &dummy_project.id))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", &session_token)))
    .await;
...
async fn verify_default_resource_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT + 1) {
    ...
    let _ = test_environment.create_random_role(Some(&RoleParentResourceType::Project), Some(&dummy_project.id)).await?;
...
async fn verify_query_when_listing_roles() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/projects/{}/roles", &dummy_project.id))
      .add_query_param("query", format!("SELECT * FROM roles")),
    test_server.get(&format!("/projects/{}/roles", &dummy_project.id))
      .add_query_param("query", format!("SELECT PG_SLEEP(10)")),
    test_server.get(&format!("/projects/{}/roles", &dummy_project.id))
      .add_query_param("query", format!("SELECT * FROM roles WHERE action_id = {}", get_roles_action.id))
  ];
