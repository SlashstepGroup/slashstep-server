...
async fn handle_create_item_type_request(
    ...
    ServerLogEntry::trace(
        &format!("Creating item type for project {}...", project_id),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
    .ok();
    let item_type = match ItemType::create(
        &InitialItemTypeProperties {
            ...
            item_type_icon_id: item_type_properties_json.item_type_icon_id,
    ...
    .ok();
    ServerLogEntry::success(
        &format!("Successfully created item type {}.", item_type.id),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
    ...
    Ok((StatusCode::CREATED, Json(item_type)))
}
...
pub fn get_router(state: AppState) -> Router<AppState> {
    ...
    Router::<AppState>::new()
        .route(
            "/projects/{project_id}/item-types",
            axum::routing::get(handle_list_item_types_request),
        )
        .route(
            "/projects/{project_id}/item-types",
            axum::routing::post(handle_create_item_type_request),
        )
