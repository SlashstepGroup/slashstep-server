...
use crate::{
    ...
    resources::{
        ...
        view::{
            ...
            InitialViewPropertiesWithPredefinedParent, View, ViewParentResourceType,
        },
    },
        ...
        Err(error) => {
            let http_error = match error {
                ResourceError::SlashstepQLError(error) => {
                    match_slashstepql_error(&error, &DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, "views")
                }
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        .route(
            ...
            axum::routing::get(handle_list_views_request),
        )
        .route(
            "/projects/{project_id}/views",
            axum::routing::post(handle_create_view_request),
        )
