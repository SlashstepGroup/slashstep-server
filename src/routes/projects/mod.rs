...
async fn handle_list_projects_request(
  ...
  ServerLogEntry::trace("Listing projects...", Some(&http_transaction.id), &state.database_pool).await.ok();
  ...
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_project_list_length, if queried_project_list_length == 1 { "project" } else { "projects" }), Some(&http_transaction.id), &state.database_pool).await.ok();
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/projects", axum::routing::get(handle_list_projects_request))
