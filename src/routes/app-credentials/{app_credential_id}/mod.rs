...
use crate::{
    ...
    utilities::route_handler_utilities::{
        get_action_by_name, get_action_log_entry_expiration_timestamp, get_app_credential_by_id,
...
async fn handle_delete_app_credential_request(
    ...
    if let Err(error) = target_app_credential.delete(&state.database_pool).await {
        let http_error = HTTPError::InternalServerError(Some(format!(
            "Failed to delete app credential: {:?}",
            error
        )));
        http_error.log();
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            ...
            axum::routing::get(handle_get_app_credential_request),
        )
        .route(
            "/app-credentials/{app_credential_id}",
            axum::routing::delete(handle_delete_app_credential_request),
        )
