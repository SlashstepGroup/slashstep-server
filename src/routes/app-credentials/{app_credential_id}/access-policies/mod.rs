/**
 * 
 * Any functionality for /app-credentials/{app_credential_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
async fn handle_list_access_policies_request(
  ...
  let app_credential = get_app_credential_by_id(&app_credential_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppCredential, Some(&app_credential.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  let query = format!(
    "scoped_resource_type = 'AppCredential' AND scoped_app_credential_id = {}{}", 
    quote_literal(&app_credential_id.to_string()), 
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_app_credential_id: Some(app_credential_id),
    ..Default::default()
...
/// POST /app-credentials/{app_credential_id}/access-policies
/// 
/// Creates an access policy for an app credential.
#[axum::debug_handler]
async fn handle_create_access_policy_request(
  ...
  // Make sure the authenticated_user can create access policies for the target app credential.
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppCredential, Some(&target_app_credential.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppCredential, Some(&target_app_credential.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for app credential {}...", app_credential_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_app_credential_id: Some(target_app_credential.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/app-credentials/{app_credential_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/app-credentials/{app_credential_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
