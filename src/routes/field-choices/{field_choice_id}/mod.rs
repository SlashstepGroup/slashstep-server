...
async fn handle_delete_field_choice_request(
  ...
  verify_principal_permissions(&authenticated_principal, &delete_field_choices_action, &resource_hierarchy, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
...
async fn handle_patch_field_choice_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&original_target_field_choice, &ResourceType::FieldChoice, &original_target_field_choice.id, &http_transaction, &state.database_pool).await?;
