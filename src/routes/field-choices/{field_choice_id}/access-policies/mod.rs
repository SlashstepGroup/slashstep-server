...
use crate::{AppState, HTTPError, middleware::{authentication_middleware, http_transaction_middleware}, resources::{ResourceError, access_policy::{AccessPolicy, ResourceType, ActionPermissionLevel, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, InitialAccessPolicyProperties, InitialAccessPolicyPropertiesForPredefinedScope}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, ActionLogEntryTargetResourceType, InitialActionLogEntryProperties}, app::App, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User}, routes::{ListResourcesResponseBody, ResourceListQueryParameters}, utilities::{route_handler_utilities::{get_action_by_id, get_action_by_name, get_action_log_entry_expiration_timestamp, get_field_choice_by_id, get_request_body_without_json_rejection, get_uuid_from_string, match_db_error, match_slashstepql_error, verify_delegate_permissions, verify_principal_permissions}}};
...
async fn handle_list_access_policies_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&field_choice, &ResourceType::FieldChoice, &field_choice_id, &http_transaction, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&target_field_choice, &ResourceType::FieldChoice, &target_field_choice.id, &http_transaction, &state.database_pool).await?;
