/**
 * 
 * Any functionality for /field-choices/{field_choice_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
async fn handle_list_access_policies_request(
  ...
  let field_choice = get_field_choice_by_id(&field_choice_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::FieldChoice, Some(&field_choice.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  let query = format!(
    "scoped_resource_type = 'FieldChoice' AND scoped_field_choice_id = {}{}", 
    quote_literal(&field_choice_id.to_string()), 
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_field_choice_id: Some(field_choice_id),
    ..Default::default()
...
/// POST /field-choices/{field_choice_id}/access-policies
/// 
...
async fn handle_create_access_policy_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::FieldChoice, Some(&target_field_choice.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::FieldChoice, Some(&target_field_choice.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for field choice {}...", field_choice_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_field_choice_id: Some(target_field_choice.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/field-choices/{field_choice_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/field-choices/{field_choice_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
