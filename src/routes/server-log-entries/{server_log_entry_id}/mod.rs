...
use crate::{
  ...
  utilities::route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_action_log_entry_expiration_timestamp, get_authenticated_principal, get_resource_hierarchy, get_server_log_entry_by_id, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}
};
...
async fn handle_get_server_log_entry_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&target_server_log_entry, &ResourceType::ServerLogEntry, &target_server_log_entry.id, &http_transaction, &state.database_pool).await?;
