...
use crate::{
    ...
    resources::{
        ...
        milestone::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, Milestone},
...
async fn handle_list_milestones_request(
    ...
    let list_resources_action =
        get_action_by_name("milestones.list", &http_transaction, &state.database_pool).await?;
    ...
    trace!("Listing milestones...");
    let query = query_parameters.query.unwrap_or("".to_string());
    let queried_resources = match Milestone::list(
        &query,
        &state.database_pool,
        Some(&principal_type),
        Some(&principal_id),
    ...
    trace!("Counting milestones...");
    let resource_count = match Milestone::count(
        &query,
        &state.database_pool,
        Some(&principal_type),
        Some(&principal_id),
    ...
    info!("Successfully returned {} {}.", queried_milestone_list_length, if queried_milestone_list_length == 1 { "milestone" } else { "milestones" });
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/milestones",
            axum::routing::get(handle_list_milestones_request),
        )
