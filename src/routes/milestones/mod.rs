/**
 * 
 * Any functionality for /milestones should be handled here.
 * 
 * Programmers: 
...
use crate::{AppState, HTTPError, middleware::{authentication_middleware, http_transaction_middleware, rate_limit_middleware}, resources::{ResourceType, ResourceError, access_policy::{PermissionLevel}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, InitialActionLogEntryProperties}, app::App, app_authorization::AppAuthorization, milestone::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, Milestone}, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User}, routes::{ListResourcesResponseBody, ResourceListQueryParameters}, utilities::route_handler_utilities::{get_action_by_name, get_action_log_entry_expiration_timestamp, get_principal_type_and_id_from_principal, is_authenticated_user_anonymous, match_db_error, match_slashstepql_error, verify_delegate_permissions, verify_principal_permissions}};
...
async fn handle_list_milestones_request(
  ...
  ServerLogEntry::trace("Listing milestones...", Some(&http_transaction.id), &state.database_pool).await.ok();
  ...
  let queried_milestone_list_length = queried_resources.len();
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_milestone_list_length, if queried_milestone_list_length == 1 { "milestone" } else { "milestones" }), Some(&http_transaction.id), &state.database_pool).await.ok();
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/milestones", axum::routing::get(handle_list_milestones_request))
    ...
    .merge(milestone_id::get_router(state.clone()));
  return router;
