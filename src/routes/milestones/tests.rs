...
use crate::{
  AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, action::Action, milestone::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Milestone},
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.get(&format!("/milestones"))
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/milestones"))
      .add_query_param("query", format!("id ~ {}", quote_literal(&get_milestones_action.id.to_string()))),
    test_server.get(&format!("/milestones"))
      .add_query_param("query", format!("SELECT * FROM milestones")),
    test_server.get(&format!("/milestones"))
      .add_query_param("query", format!("SELECT PG_SLEEP(10)")),
    test_server.get(&format!("/milestones"))
      .add_query_param("query", format!("id = null; SELECT * FROM milestones WHERE id = {}", quote_literal(&get_milestones_action.id.to_string())))
  ];
