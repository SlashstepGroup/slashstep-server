/**
 * 
 * Any test cases for /item-type-icons should be handled here.
 * 
 * Programmers: 
...
use crate::{
  AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, action::Action, item_type_icon::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, ItemTypeIcon},
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "itemTypeIcons.get" action to the user.
  let user = test_environment.create_random_user().await?;
  let session = test_environment.create_random_session(Some(&user.id)).await?;
  let json_web_token_private_key = get_json_web_token_private_key().await?;
  let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
  let get_delegation_policies_action = Action::get_by_name("itemTypeIcons.get", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &get_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Grant access to the "itemTypeIcons.list" action to the user.
  let list_delegation_policies_action = Action::get_by_name("itemTypeIcons.list", &test_environment.database_pool).await?;
  ...
  test_environment.create_random_item_type_icon(None).await?;
  ...
  let response = test_server.get(&format!("/item-type-icons"))
  ...
  let response_json: ListResourcesResponseBody::<ItemTypeIcon> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_item_type_icon_count = ItemTypeIcon::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_item_type_icon_count);

  let actual_delegation_policies = ItemTypeIcon::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_item_type_icon in actual_delegation_policies {

    let found_access_policy = response_json.resources.iter().find(|item_type_icon| item_type_icon.id == actual_item_type_icon.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_item_type_icon.id.to_string()));
  let response = test_server.get(&format!("/item-type-icons"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let actual_delegation_policies = ItemTypeIcon::list(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_item_type_icon in actual_delegation_policies {

    let found_action = response_json.resources.iter().find(|item_type_icon| item_type_icon.id == actual_item_type_icon.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let item_type_icon_count = ItemTypeIcon::count("", &test_environment.database_pool, None, None).await?;
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - item_type_icon_count + 1) {
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/item-type-icons"))
