/**
 * 
 * Any functionality for /item-type-icons/{item_type_icon_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
/// 
/// Lists access policies for a item type icon.
#[axum::debug_handler]
async fn handle_list_access_policies_request(
  ...
  let item_type_icon = get_item_type_icon_by_id(&item_type_icon_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemTypeIcon, Some(&item_type_icon.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  let query = format!(
    "scoped_resource_type = 'ItemTypeIcon' AND scoped_item_type_icon_id = {}{}", 
    quote_literal(&item_type_icon_id.to_string()), 
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_item_type_icon_id: Some(item_type_icon_id),
    ..Default::default()
...
/// POST /item-type-icons/{item_type_icon_id}/access-policies
/// 
/// Creates an access policy for a item type icon.
#[axum::debug_handler]
async fn handle_create_access_policy_request(
  ...
  // Make sure the authenticated_user can create access policies for the target item type icon.
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemTypeIcon, Some(&target_item_type_icon.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemTypeIcon, Some(&target_item_type_icon.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for item type icon {}...", item_type_icon_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_resource_type: ResourceType::ItemTypeIcon,
    scoped_item_type_icon_id: Some(target_item_type_icon.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/item-type-icons/{item_type_icon_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/item-type-icons/{item_type_icon_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
