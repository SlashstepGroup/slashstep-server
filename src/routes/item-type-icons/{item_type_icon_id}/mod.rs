use crate::{
    ...
    utilities::route_handler_utilities::{
        get_action_by_name, get_action_log_entry_expiration_timestamp, get_item_type_icon_by_id,
...
async fn handle_delete_item_type_icon_request(
    ...
    if let Err(error) = target_item_type_icon.delete(&state.database_pool).await {
        let http_error = HTTPError::InternalServerError(Some(format!(
            "Failed to delete item type icon: {:?}",
            error
        )));
...
async fn handle_patch_item_type_icon_request(
        ...
        .await
    {
        ...
        Err(error) => {
            let http_error = HTTPError::InternalServerError(Some(format!(
                "Failed to update item type icon: {:?}",
                error
            )));
            ServerLogEntry::from_http_error(
                &http_error,
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        ...
        .route(
            "/item-type-icons/{item_type_icon_id}",
            axum::routing::delete(handle_delete_item_type_icon_request),
        )
        .route(
            "/item-type-icons/{item_type_icon_id}",
            axum::routing::patch(handle_patch_item_type_icon_request),
        )
