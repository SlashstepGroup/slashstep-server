...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.get(&format!("/item-type-icons/{}", item_type_icon.id))
  ...
  assert_eq!(response_item_type_icon.display_name, item_type_icon.display_name);
  assert_eq!(response_item_type_icon.parent_resource_type, item_type_icon.parent_resource_type);
  assert_eq!(response_item_type_icon.parent_project_id, item_type_icon.parent_project_id);
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.get("/item-type-icons/not-a-uuid")
    .await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.get(&format!("/item-type-icons/{}", uuid::Uuid::now_v7()))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .await;
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.delete(&format!("/item-type-icons/{}", item_type_icon.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .await;
  
  assert_eq!(response.status_code(), StatusCode::NO_CONTENT);

  match ItemTypeIcon::get_by_id(&item_type_icon.id, &test_environment.database_pool).await.expect_err("Expected an item type icon not found error.") {
...
async fn verify_uuid_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.delete("/item-type-icons/not-a-uuid")
    .await;
...
async fn verify_resource_exists_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.delete(&format!("/item-type-icons/{}", uuid::Uuid::now_v7()))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .await;
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.patch(&format!("/item-type-icons/{}", original_item_type_icon.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let updated_item_type_icon: ItemTypeIcon = response.json();
  assert_eq!(original_item_type_icon.id, updated_item_type_icon.id);
  assert_eq!(updated_item_type_icon_properties.display_name.expect("Expected an updated display name."), updated_item_type_icon.display_name);
  assert_eq!(original_item_type_icon.parent_resource_type, updated_item_type_icon.parent_resource_type);
  assert_eq!(original_item_type_icon.parent_project_id, updated_item_type_icon.parent_project_id);
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.patch("/item-type-icons/not-a-uuid")
    .await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.patch(&format!("/item-type-icons/{}", Uuid::now_v7()))
    .add_header("Content-Type", "application/json")
...
async fn verify_authentication_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.patch(&format!("/item-type-icons/{}", item_type_icon.id))
    .json(&serde_json::json!({
      "display_name": Uuid::now_v7().to_string()
    }))
    .await;
...
#[tokio::test]
async fn verify_item_type_icon_display_name_is_at_most_at_maximum_length() -> Result<(), TestSlashstepServerError> {
  ...
  maximum_item_type_icon_display_name_length_configuration.update(&EditableConfigurationProperties {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.patch(&format!("/item-type-icons/{}", dummy_item_type_icon.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .json(&serde_json::json!(updated_item_type_icon_properties))
    .await;
