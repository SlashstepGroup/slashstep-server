...
async fn handle_list_access_policies_request(
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Session, Some(&session.id), &list_resources_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Session, Some(&target_session.id), &create_access_policies_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
