/**
 * 
 * Any functionality for /sessions/{session_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
use crate::{AppState, HTTPError, middleware::{authentication_middleware, http_transaction_middleware}, resources::{ResourceType, ResourceError, access_policy::{AccessPolicy, ActionPermissionLevel, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, InitialAccessPolicyProperties, InitialAccessPolicyPropertiesForPredefinedScope}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, InitialActionLogEntryProperties}, app::App, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User}, routes::{ListResourcesResponseBody, ResourceListQueryParameters}, utilities::route_handler_utilities::{get_action_by_id, get_action_by_name, get_action_log_entry_expiration_timestamp, get_session_by_id, get_principal_type_and_id_from_principal, get_request_body_without_json_rejection, get_uuid_from_string, is_authenticated_user_anonymous, match_db_error, match_slashstepql_error, verify_delegate_permissions, verify_principal_permissions}};

#[cfg(test)]
mod tests;

/// GET /sessions/{session_id}/access-policies
/// 
/// Lists access policies for a session.
#[axum::debug_handler]
async fn handle_list_access_policies_request(
  ...
  let session = get_session_by_id(&session_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Session, Some(&session.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  let query = format!(
    "scoped_resource_type = 'Session' AND scoped_session_id = {}{}", 
    quote_literal(&session_id.to_string()), 
...
/// POST /sessions/{session_id}/access-policies
/// 
/// Creates an access policy for a session.
#[axum::debug_handler]
async fn handle_create_access_policy_request(
  ...
  // Make sure the authenticated_user can create access policies for the target session.
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Session, Some(&target_session.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Session, Some(&target_session.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for session {}...", session_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_session_id: Some(target_session.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/sessions/{session_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/sessions/{session_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
