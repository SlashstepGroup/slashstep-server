...
async fn handle_list_sessions_request(
  ...
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_session_list_length, if queried_session_list_length == 1 { "session" } else { "sessions" }), Some(&http_transaction.id), &state.database_pool).await.ok();
