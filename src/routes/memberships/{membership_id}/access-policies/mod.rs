/**
 * 
 * Any functionality for /memberships/{membership_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
async fn handle_list_access_policies_request(
  ...
  let membership = get_membership_by_id(&membership_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Membership, Some(&membership.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
...
/// POST /memberships/{membership_id}/access-policies
/// 
/// Creates an access policy for an membership.
#[axum::debug_handler]
async fn handle_create_access_policy_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Membership, Some(&target_membership.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Membership, Some(&target_membership.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for membership {}...", membership_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_membership_id: Some(target_membership.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/memberships/{membership_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/memberships/{membership_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
