...
async fn create_membership_access_policy(database_pool: &deadpool_postgres::Pool, scoped_membership_id: &Uuid, user_id: &Uuid, action_id: &Uuid, permission_level: &PermissionLevel) -> Result<AccessPolicy, TestSlashstepServerError> {
  ...
  let access_policy = AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_resource_type: ResourceType::Membership,
    scoped_membership_id: Some(scoped_membership_id.clone()),
    ..Default::default()
  }, database_pool).await?;
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_membership = test_environment.create_random_membership().await?;
  ...
  let response = test_server.post(&format!("/memberships/{}/access-policies", dummy_membership.id))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_membership = test_environment.create_random_membership().await?;
  let shown_access_policy = create_membership_access_policy(&test_environment.database_pool, &dummy_membership.id, &user.id, &list_access_policies_action.id, &PermissionLevel::User).await?;
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.get(&format!("/memberships/{}/access-policies", &dummy_membership.id))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", &session_token)))
    .await;
  ...
  let query = format!("scoped_resource_type = 'Membership' AND scoped_membership_id = {}", quote_literal(&dummy_membership.id.to_string()));
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_membership = test_environment.create_random_membership().await?;
  create_membership_access_policy(&test_environment.database_pool, &dummy_membership.id, &user.id, &list_access_policies_action.id, &PermissionLevel::User).await?;

  let shown_access_policy = create_membership_access_policy(&test_environment.database_pool, &dummy_membership.id, &user.id, &get_access_policies_action.id, &PermissionLevel::Editor).await?;
  ...
  let query = format!("scoped_resource_type = 'Membership' AND scoped_membership_id = {} and permission_level = 'Editor'", quote_literal(&dummy_membership.id.to_string()));
  let actual_access_policy_count = AccessPolicy::count(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_access_policies.total_count, actual_access_policy_count);
...
async fn verify_default_resource_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT + 1) {
    ...
    create_membership_access_policy(&test_environment.database_pool, &dummy_membership.id, &random_user.id, &random_action.id, &PermissionLevel::User).await?;
...
async fn verify_query_when_listing_access_policies() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/memberships/{}/access-policies", &dummy_membership.id))
