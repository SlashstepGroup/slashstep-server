...
use crate::{
  ...
  utilities::{route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_authenticated_principal, get_membership_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}}
};
...
async fn handle_get_membership_request(
  ...
  let get_memberships_action = get_action_by_name("slashstep.memberships.get", &http_transaction, &state.database_pool).await?;
...
//   let membership_id = get_uuid_from_string(&membership_id, "app", &http_transaction, &state.database_pool).await?;
...
//     |membership_id, database_pool| Box::new(App::get_by_id(membership_id, database_pool))
//   ).await;
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    ...
    // .route("/memberships/{membership_id}", axum::routing::delete(handle_delete_app_request))
    // .route("/memberships/{membership_id}", axum::routing::patch(handle_patch_app_request))
