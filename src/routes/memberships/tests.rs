...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_random_membership().await?;
  ...
  let response = test_server.get(&format!("/memberships"))
  ...
  let response_json: ListResourcesResponseBody::<Membership> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_membership_count = Membership::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_membership_count);

  let actual_memberships = Membership::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_memberships.len());

  for actual_membership in actual_memberships {

    let found_access_policy = response_json.resources.iter().find(|membership| membership.id == actual_membership.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_membership.id.to_string()));
  let response = test_server.get(&format!("/memberships"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  for actual_membership in actual_memberships {
    ...
    let found_action = response_json.resources.iter().find(|membership| membership.id == actual_membership.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let membership_count = Membership::count("", &test_environment.database_pool, None, None).await?;
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/memberships"))
      .add_query_param("query", format!("id ~ {}", quote_literal(&get_memberships_action.id.to_string()))),
    test_server.get(&format!("/memberships"))
      .add_query_param("query", format!("SELECT * FROM memberships")),
    test_server.get(&format!("/memberships"))
      .add_query_param("query", format!("SELECT PG_SLEEP(10)")),
    test_server.get(&format!("/memberships"))
      .add_query_param("query", format!("id = null; SELECT * FROM memberships WHERE id = {}", quote_literal(&get_memberships_action.id.to_string())))
  ];
