...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  assert_eq!(response_json.data.len(), actual_membership_invitations.len());

  for actual_membership_invitation in actual_membership_invitations {

    let found_access_policy = response_json.data.iter().find(|membership_invitation| membership_invitation.id == actual_membership_invitation.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  for actual_membership_invitation in actual_membership_invitations {
    ...
    let found_action = response_json.data.iter().find(|membership_invitation| membership_invitation.id == actual_membership_invitation.id);
