/**
 * 
 * Any test cases for /membership-invitations should be handled here.
 * 
 * Programmers: 
...
use crate::{
  AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, action::Action, membership_invitation::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, MembershipInvitation},
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "membershipInvitations.get" action to the user.
  ...
  // Grant access to the "membershipInvitations.list" action to the user.
  ...
  test_environment.create_random_membership_invitation().await?;
  ...
  let response = test_server.get(&format!("/membership-invitations"))
  ...
  let response_json: ListResourcesResponseBody::<MembershipInvitation> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_membership_invitation_count = MembershipInvitation::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_membership_invitation_count);

  let actual_membership_invitations = MembershipInvitation::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_membership_invitations.len());

  for actual_membership_invitation in actual_membership_invitations {

    let found_access_policy = response_json.resources.iter().find(|membership_invitation| membership_invitation.id == actual_membership_invitation.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_membership_invitation.id.to_string()));
  let response = test_server.get(&format!("/membership-invitations"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  for actual_membership_invitation in actual_membership_invitations {
    ...
    let found_action = response_json.resources.iter().find(|membership_invitation| membership_invitation.id == actual_membership_invitation.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let membership_invitation_count = MembershipInvitation::count("", &test_environment.database_pool, None, None).await?;
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - membership_invitation_count + 1) {
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/membership-invitations"))
      .add_query_param("query", format!("id ~ {}", quote_literal(&get_membership_invitations_action.id.to_string()))),
    test_server.get(&format!("/membership-invitations"))
      .add_query_param("query", format!("SELECT * FROM membership_invitations")),
    test_server.get(&format!("/membership-invitations"))
      .add_query_param("query", format!("SELECT PG_SLEEP(10)")),
    test_server.get(&format!("/membership-invitations"))
      .add_query_param("query", format!("id = null; SELECT * FROM membership_invitations WHERE id = {}", quote_literal(&get_membership_invitations_action.id.to_string())))
  ];
