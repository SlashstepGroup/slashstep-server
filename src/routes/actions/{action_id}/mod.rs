...
async fn handle_get_action_request(
  ...
  let resource_hierarchy = get_resource_hierarchy_for_action(&action, &http_transaction, &mut postgres_client).await?;
...
async fn handle_patch_action_request(
  ...
  let user = get_user_from_option_user(&user, &http_transaction, &mut postgres_client).await?;
  let resource_hierarchy = get_resource_hierarchy_for_action(&original_action, &http_transaction, &mut postgres_client).await?;
