...
async fn verify_name_is_under_maximum_length() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.patch(&format!("/actions/{}", dummy_action.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .json(&serde_json::json!({
      "name": "a"
    }))
...
async fn verify_action_name_matches_regex() -> Result<(), TestSlashstepServerError> {
  ...
  action_name_regex_configuration.update(&EditableConfigurationProperties {
...
async fn verify_action_display_name_matches_regex() -> Result<(), TestSlashstepServerError> {
  ...
  action_display_name_regex_configuration.update(&EditableConfigurationProperties {
