...
async fn verify_successful_deletion_when_deleting_action_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let delete_actions_action = Action::get_by_name("slashstep.actions.delete", &postgres_client).await?;
  ...
  match Action::get_by_id(&action.id, &postgres_client).await.expect_err("Expected an action not found error.") {
...
async fn verify_successful_patch_action_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let json_web_token_private_key = Session::get_json_web_token_private_key().await?;
  let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
  let get_actions_action = Action::get_by_name("slashstep.actions.update", &postgres_client).await?;
  AccessPolicy::create(&InitialAccessPolicyProperties {
    action_id: get_actions_action.id,
