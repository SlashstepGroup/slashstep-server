...
async fn handle_list_access_policies_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&action, &ResourceType::Action, &action.id, &http_transaction, &state.database_pool).await?;
