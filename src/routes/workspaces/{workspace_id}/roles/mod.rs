/**
 * 
 * Any functionality for /workspaces/{workspace_id}/roles should be handled here.
 * 
 * Programmers: 
...
/// 
/// Creates a role for an workspace.
#[axum::debug_handler]
async fn handle_create_role_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Workspace, Some(&target_workspace.id), &create_roles_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  // Create the role.
  ServerLogEntry::trace(&format!("Creating role for workspace {}...", workspace_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let role = match Role::create(&InitialRoleProperties {
    ...
    parent_workspace_id: Some(target_workspace.id),
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/workspaces/{workspace_id}/roles", axum::routing::get(handle_list_roles_request))
    .route("/workspaces/{workspace_id}/roles", axum::routing::post(handle_create_role_request))
