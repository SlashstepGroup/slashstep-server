...
async fn handle_create_role_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Workspace, Some(&target_workspace.id), &create_roles_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
