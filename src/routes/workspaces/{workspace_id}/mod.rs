...
use crate::{
  ...
  resources::{
    access_policy::{ResourceType, ActionPermissionLevel}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, ActionLogEntryTargetResourceType, InitialActionLogEntryProperties}, app::{App, EditableAppProperties}, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User, workspace::Workspace
  }, 
  utilities::{route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_action_log_entry_expiration_timestamp, get_authenticated_principal, get_resource_hierarchy, get_uuid_from_string, get_workspace_by_id, verify_delegate_permissions, verify_principal_permissions}}
};
...
async fn handle_get_workspace_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&target_workspace, &ResourceType::Workspace, &target_workspace.id, &http_transaction, &state.database_pool).await?;
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_resource_type: ActionLogEntryTargetResourceType::Workspace,
...
//   let workspace_id = get_uuid_from_string(&workspace_id, "app", &http_transaction, &state.database_pool).await?;
...
//     |workspace_id, database_pool| Box::new(App::get_by_id(workspace_id, database_pool))
//   ).await;
...
//   let original_target_field = get_app_by_id(&workspace_id, &http_transaction, &state.database_pool).await?;
...
//     actor_type: if let AuthenticatedPrincipal::User(_) = &authenticated_principal { ActionLogEntryActorType::User } else { ActionLogEntryActorType::App },
//     actor_user_id: if let AuthenticatedPrincipal::User(authenticated_user) = &authenticated_principal { Some(authenticated_user.id.clone()) } else { None },
//     actor_workspace_id: if let AuthenticatedPrincipal::App(authenticated_app) = &authenticated_principal { Some(authenticated_app.id.clone()) } else { None },
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    ...
    // .route("/workspaces/{workspace_id}", axum::routing::delete(handle_delete_app_request))
    // .route("/workspaces/{workspace_id}", axum::routing::patch(handle_patch_app_request))
