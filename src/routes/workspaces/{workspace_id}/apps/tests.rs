...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_random_app(Some(&AppParentResourceType::Workspace), Some(&workspace.id)).await?;
  ...
  let response = test_server.get(&format!("/workspaces/{}/apps", workspace.id))
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Create a dummy app.
  let workspace = test_environment.create_random_workspace().await?;
  let dummy_app = test_environment.create_random_app(Some(&AppParentResourceType::Workspace), Some(&workspace.id)).await?;
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/workspaces/{}/apps", workspace.id))
...
async fn verify_successful_app_creation_with_public_client() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.post(&format!("/workspaces/{}/apps", workspace.id))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", session_token)))
