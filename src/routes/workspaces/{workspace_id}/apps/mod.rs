...
use crate::{AppState, HTTPError, middleware::{authentication_middleware, http_transaction_middleware, rate_limit_middleware}, resources::{ResourceError, ResourceType, access_policy::{AccessPolicy, AccessPolicyPrincipalType, PermissionLevel, InitialAccessPolicyProperties}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, InitialActionLogEntryProperties}, app::{App, AppClientType, AppParentResourceType, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, InitialAppProperties}, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, role::{InitialRoleProperties, PredefinedRoleType, Role, RoleParentResourceType}, server_log_entry::ServerLogEntry, user::User}, routes::{AppWithClientSecret, CreateAppRequestBody, ListResourcesResponseBody, ResourceListQueryParameters}, utilities::route_handler_utilities::{get_action_by_name, get_action_log_entry_expiration_timestamp, get_principal_type_and_id_from_principal, get_request_body_without_json_rejection, get_uuid_from_string, get_workspace_by_id, is_authenticated_user_anonymous, match_db_error, match_slashstepql_error, validate_field_length, validate_resource_display_name, validate_resource_name, verify_delegate_permissions, verify_principal_permissions}};
...
async fn handle_list_apps_request(
  ...
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_resource_list_length, if queried_resource_list_length == 1 { "app" } else { "apps" }), Some(&http_transaction.id), &state.database_pool).await.ok();
...
async fn handle_create_app_request(
  ...
  validate_resource_name(&app_properties_json.name, "apps.allowedNameRegex", "app", &http_transaction, &state.database_pool).await?;
  validate_field_length(&app_properties_json.name, "apps.maximumNameLength", "name", &http_transaction, &state.database_pool).await?;
  validate_resource_display_name(&app_properties_json.display_name, "apps.allowedDisplayNameRegex", "app", &http_transaction, &state.database_pool).await?;
  ...
  let workspace_id = get_uuid_from_string(&workspace_id, "workspace", &http_transaction, &state.database_pool).await?;
  let workspace = get_workspace_by_id(&workspace_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &create_apps_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Server, None, &create_apps_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
  ...
  ServerLogEntry::trace("Creating app admins role for app...", Some(&http_transaction.id), &state.database_pool).await.ok();

  let app_admins_role = match Role::create(&InitialRoleProperties {
    name: "app-admins".to_string(),
    display_name: "App admins".to_string(),
    description: Some("Principals who have administrative privileges for an app.".to_string()),
    ...
    Err(error) => {
      ...
      let http_error = HTTPError::InternalServerError(Some(format!("Failed to create app admins role on app {}: {:?}", workspace.id, error)));
      ServerLogEntry::from_http_error(&http_error, Some(&http_transaction.id), &state.database_pool).await.ok();
      return Err(http_error);
  ...
  ServerLogEntry::trace("Creating access policies for app admins role...", Some(&http_transaction.id), &state.database_pool).await.ok();
  ...
  for action_name in allowed_actions {
    ...
    if let Err(error) = AccessPolicy::create(&InitialAccessPolicyProperties {
      ...
      principal_role_id: Some(app_admins_role.id.clone()),
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/workspaces/{workspace_id}/apps", axum::routing::get(handle_list_apps_request))
    .route("/workspaces/{workspace_id}/apps", axum::routing::post(handle_create_app_request))
