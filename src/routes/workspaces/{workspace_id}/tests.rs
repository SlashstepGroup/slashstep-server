...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let get_workspaces_action = Action::get_by_name("slashstep.workspaces.get", &test_environment.database_pool).await?;
...
//   // Grant access to the "slashstep.workspaces.delete" action to the user.
//   let delete_fields_action = Action::get_by_name("slashstep.workspaces.delete", &test_environment.database_pool).await?;
...
//   match App::get_by_id(&workspace.id, &test_environment.database_pool).await.expect_err("Expected an app not found error.") {
...
//   let json_web_token_private_key = get_json_web_token_private_key().await?;
//   let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
//   let update_fields_action = Action::get_by_name("slashstep.workspaces.update", &test_environment.database_pool).await?;
//   AccessPolicy::create(&InitialAccessPolicyProperties {
