...
use crate::{
    ...
    resources::{
        ...
        field_value::DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT,
    ...
    utilities::route_handler_utilities::{
        get_action_by_name, get_action_log_entry_expiration_timestamp, get_item_by_id,
...
async fn handle_list_item_connections_request(
        ...
        Err(error) => {
            let http_error = match error {
                ...
                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list item connections: {:?}",
                    error
                ))),
            };
    ...
    ServerLogEntry::trace(
        &format!("Counting item connections..."),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
...
async fn handle_create_item_connection_request(
    ...
    if let Err(error) = verify_principal_permissions(
        &principal_type,
        &principal_id,
        is_authenticated_user_anonymous(authenticated_user.as_ref()),
        &ResourceType::Item,
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        .route(
            "/items/{item_id}/item-connections",
            axum::routing::get(handle_list_item_connections_request),
        )
        .route(
            "/items/{item_id}/item-connections",
            axum::routing::post(handle_create_item_connection_request),
        )
