...
use crate::{
    ...
    resources::{
        ...
        field_value::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, FieldValue, FieldValueParentResourceType,
    ...
    utilities::route_handler_utilities::{
        get_action_by_name, get_action_log_entry_expiration_timestamp, get_field_by_id,
        get_item_by_id, get_principal_type_and_id_from_principal,
        ...
        validate_decimal_is_within_range, validate_field_length, verify_delegate_permissions,
...
async fn handle_list_field_values_request(
    ...
    trace!("Counting field values...");
    let resource_count = match FieldValue::count(
        &query,
        &state.database_pool,
        Some(&principal_type),
        Some(&principal_id),
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        ...
        .route(
            "/items/{item_id}/field-values",
            axum::routing::post(handle_create_field_value_request),
        )
