...
use crate::{AppState, HTTPError, middleware::{authentication_middleware, http_transaction_middleware, rate_limit_middleware}, resources::{ResourceType, ResourceError, access_policy::{PermissionLevel}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, InitialActionLogEntryProperties}, app::App, app_authorization::AppAuthorization, field_value::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, FieldValue, FieldValueParentResourceType, InitialFieldValueProperties, InitialFieldValuePropertiesWithPredefinedParent}, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User}, routes::{ListResourcesResponseBody, ResourceListQueryParameters}, utilities::route_handler_utilities::{get_action_by_name, get_action_log_entry_expiration_timestamp, get_field_by_id, get_item_by_id, get_principal_type_and_id_from_principal, get_request_body_without_json_rejection, get_uuid_from_string, is_authenticated_user_anonymous, match_db_error, match_slashstepql_error, validate_decimal_is_within_range, validate_field_length, verify_delegate_permissions, verify_principal_permissions}};

/// GET /items/{item_id}/field-values
/// 
/// Lists field values for an field value.
#[axum::debug_handler]
async fn handle_list_field_values_request(
  ...
  let list_resources_action = get_action_by_name("fieldValues.list", &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Item, Some(&target_item.id), &list_resources_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
  ...
  let queried_resources = match FieldValue::list(&query, &state.database_pool, Some(&principal_type), Some(&principal_id)).await {
    ...
    Err(error) => {
      ...
      let http_error = match error {
        ...
        ResourceError::SlashstepQLError(error) => match_slashstepql_error(&error, &DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, "field values"),

        ResourceError::PostgresError(error) => match_db_error(&error, "field values"),

        _ => HTTPError::InternalServerError(Some(format!("Failed to list field values: {:?}", error)))
  ...
  ServerLogEntry::trace(&format!("Counting field values..."), Some(&http_transaction.id), &state.database_pool).await.ok();
  let resource_count = match FieldValue::count(&query, &state.database_pool, Some(&principal_type), Some(&principal_id)).await {

    Ok(resource_count) => resource_count,

    Err(error) => {

      let http_error = HTTPError::InternalServerError(Some(format!("Failed to count field values: {:?}", error)));
      ServerLogEntry::from_http_error(&http_error, Some(&http_transaction.id), &state.database_pool).await.ok();
      return Err(http_error);
  ...
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_resource_list_length, if queried_resource_list_length == 1 { "field value" } else { "field values" }), Some(&http_transaction.id), &state.database_pool).await.ok();
...
async fn handle_create_field_value_request(
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &create_field_values_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Item, Some(&target_item.id), &create_field_values_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
