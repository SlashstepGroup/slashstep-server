...
use crate::{
  ...
  utilities::{reusable_route_handlers::delete_resource, route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_authenticated_principal, get_item_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}}
};
...
async fn handle_get_item_request(
  ...
  let get_items_action = get_action_by_name("slashstep.items.get", &http_transaction, &state.database_pool).await?;
