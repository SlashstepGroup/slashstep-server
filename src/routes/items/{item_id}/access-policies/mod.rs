...
async fn handle_list_access_policies_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&item, &AccessPolicyResourceType::Item, &item_id, &http_transaction, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
  ...
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_resource_type: AccessPolicyResourceType::Item,
