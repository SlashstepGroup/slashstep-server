...
async fn handle_list_items_request(
  ...
  let queried_resources = match Item::list(&query, &state.database_pool, Some(&individual_principal)).await {
  ...
  let resource_count = match Item::count(&query, &state.database_pool, Some(&individual_principal)).await {
