...
async fn handle_list_items_request(
  ...
  let list_resources_action = get_action_by_name("items.list", &http_transaction, &state.database_pool).await?;
  ...
  ServerLogEntry::trace("Listing items...", Some(&http_transaction.id), &state.database_pool).await.ok();
  let query = query_parameters.query.unwrap_or("".to_string());
  let queried_resources = match Item::list(&query, &state.database_pool, Some(&principal_type), Some(&principal_id)).await {
    ...
    Err(error) => {
      ...
      let http_error = match error {
        ...
        ResourceError::SlashstepQLError(error) => match_slashstepql_error(&error, &DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, "items"),

        ResourceError::PostgresError(error) => match_db_error(&error, "items"),

        _ => HTTPError::InternalServerError(Some(format!("Failed to list items: {:?}", error)))
  ...
  ServerLogEntry::trace(&format!("Counting items..."), Some(&http_transaction.id), &state.database_pool).await.ok();
  let resource_count = match Item::count(&query, &state.database_pool, Some(&principal_type), Some(&principal_id)).await {

    Ok(resource_count) => resource_count,

    Err(error) => {

      let http_error = HTTPError::InternalServerError(Some(format!("Failed to count items: {:?}", error)));
      ServerLogEntry::from_http_error(&http_error, Some(&http_transaction.id), &state.database_pool).await.ok();
      return Err(http_error);
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/items", axum::routing::get(handle_list_items_request))
