...
async fn handle_get_app_authorization_credential_request(
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppAuthorizationCredential, Some(&target_app_authorization_credential.id), &get_app_authorizations_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
...
async fn handle_delete_app_authorization_credential_request(
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &delete_app_authorization_credentials_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppAuthorizationCredential, Some(&target_app_authorization_credential.id), &delete_app_authorization_credentials_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
