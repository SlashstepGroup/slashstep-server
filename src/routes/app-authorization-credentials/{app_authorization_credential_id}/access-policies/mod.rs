...
async fn handle_list_access_policies_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&app_authorization_credential, &ResourceType::AppAuthorizationCredential, &app_authorization_credential_id, &http_transaction, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&target_app_authorization_credential, &ResourceType::AppAuthorizationCredential, &target_app_authorization_credential.id, &http_transaction, &state.database_pool).await?;
