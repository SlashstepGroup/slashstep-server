...
use crate::{
    ...
    resources::{
        ...
        app_authorization_credential::{
            AppAuthorizationCredential, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT,
        },
...
async fn handle_list_app_authorization_credentials_request(
    ...
    (
        StatusCode,
        Json<ListResourcesResponseBody<AppAuthorizationCredential>>,
    ),
    HTTPError,
> {
        ...
        Err(error) => {
            let http_error = match error {
                ...
                ResourceError::PostgresError(error) => {
                    match_db_error(&error, "app authorization credentials")
                }

                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list app authorization credentials: {:?}",
                    error
                ))),
            };
    ...
    info!("Successfully returned {} {}.", queried_app_authorization_credential_list_length, if queried_app_authorization_credential_list_length == 1 { "app authorization credential" } else { "app authorization credentials" });
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/app-authorization-credentials",
            axum::routing::get(handle_list_app_authorization_credentials_request),
        )
