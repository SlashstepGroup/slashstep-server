...
async fn handle_list_membership_invitations_request(
  ...
  let queried_resources = match MembershipInvitation::list(&query, &state.database_pool, Some(&individual_principal)).await {
  ...
  let resource_count = match MembershipInvitation::count(&query, &state.database_pool, Some(&individual_principal)).await {
