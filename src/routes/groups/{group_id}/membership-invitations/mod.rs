...
async fn handle_list_membership_invitations_request(
    ...
    (
        StatusCode,
        Json<ListResourcesResponseBody<MembershipInvitation>>,
    ),
    HTTPError,
> {
        ...
        Err(error) => {
            let http_error = match error {
                ...
                ResourceError::PostgresError(error) => {
                    match_db_error(&error, "membership invitations")
                }

                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list membership invitations: {:?}",
                    error
                ))),
            };
...
async fn handle_create_membership_invitation_request(
    ...
    Extension(authenticated_app_authorization): Extension<Option<Arc<AppAuthorization>>>,
    body: Result<
        Json<InitialMembershipInvitationPropertiesWithPredefinedParentAndInviter>,
        JsonRejection,
    >,
    ...
    let membership_invitation = match MembershipInvitation::create(
        &InitialMembershipInvitationProperties {
            ...
            invitee_principal_user_id: membership_invitation_properties_json
                .invitee_principal_user_id,
            invitee_principal_group_id: membership_invitation_properties_json
                .invitee_principal_group_id,
            invitee_principal_app_id: membership_invitation_properties_json
                .invitee_principal_app_id,
            inviter_principal_type: if authenticated_user.is_some() {
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        ...
        .route(
            "/groups/{group_id}/membership-invitations",
            axum::routing::post(handle_create_membership_invitation_request),
        )
