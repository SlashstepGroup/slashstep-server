...
pub async fn handle_list_roles_request(
  ...
  let queried_resources = match Role::list(&query, &state.database_pool, Some(&individual_principal)).await {
  ...
  let resource_count = match Role::count(&query, &state.database_pool, Some(&individual_principal)).await {
