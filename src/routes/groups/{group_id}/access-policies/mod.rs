...
async fn handle_list_access_policies_request(
  ...
  let resource_hierarchy = get_resource_hierarchy(&group, &AccessPolicyResourceType::Group, &group_id, &http_transaction, &state.database_pool).await?;
