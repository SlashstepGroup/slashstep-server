use crate::{
    ...
    resources::{
        ...
        group::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Group,
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
    ...
    let list_groups_action =
        Action::get_by_name("groups.list", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(&user.id, &list_groups_action.id, &PermissionLevel::User)
        .await?;
    ...
    let actual_group_count = Group::count(
        "",
    ...
    for actual_group in actual_groups {
        let found_access_policy = response_json
            .data
            .iter()
            .find(|group| group.id == actual_group.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
    ...
    let bad_requests = vec![
        test_server.get(&format!("/groups")).add_query_param(
            "query",
            format!("id ~ {}", quote_literal(&get_groups_action.id.to_string())),
        ),
        test_server
            .get(&format!("/groups"))
        ...
        test_server.get(&format!("/groups")).add_query_param(
            "query",
            format!(
                "id = null; SELECT * FROM groups WHERE id = {}",
                quote_literal(&get_groups_action.id.to_string())
            ),
        ),
    ];
...
async fn verify_authentication() -> Result<(), TestSlashstepServerError> {
    ...
    let router = super::get_router(state.clone())
        ...
        .into_make_service_with_connect_info::<SocketAddr>();
    let test_server = TestServer::new(router);
    let response = test_server.get(&format!("/groups")).await;
...
async fn verify_successful_group_creation() -> Result<(), TestSlashstepServerError> {
    ...
    assert_eq!(
        ...
        response_group.display_name
    );
    assert_eq!(
        initial_group_properties.description,
