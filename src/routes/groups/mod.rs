...
use crate::{
    ...
    resources::{
        ...
        group::{
            DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, Group, GroupParentResourceType,
        ...
        Err(error) => {
            let http_error = match error {
                ResourceError::SlashstepQLError(error) => {
                    match_slashstepql_error(&error, &DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, "groups")
                }
...
async fn create_default_child_resources(
    ...
    let group_admins_role = create_role(
        &InitialRoleProperties {
            ...
            description: Some(
                "Group admins can manage the group, including its metadata and its members."
                    .to_string(),
            ),
    ...
    let group_members_role = create_role(
        &InitialRoleProperties {
            ...
            description: Some(
                "Group members can view the group's metadata and its members.".to_string(),
            ),
            parent_resource_type: RoleParentResourceType::Group,
            parent_group_id: Some(group.id),
