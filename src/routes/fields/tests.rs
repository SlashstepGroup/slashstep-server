...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  let actual_field_count = Field::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_field_count);

  let actual_fields = Field::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.data.len(), actual_fields.len());

  for actual_field in actual_fields {

    let found_access_policy = response_json.data.iter().find(|field| field.id == actual_field.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  for actual_field in actual_fields {
    ...
    let found_action = response_json.data.iter().find(|field| field.id == actual_field.id);
