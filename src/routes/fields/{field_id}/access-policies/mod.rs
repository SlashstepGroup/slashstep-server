use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_field_by_id, get_principal_type_and_id_from_principal,
...
async fn handle_list_access_policies_request(
    ...
    let field = get_field_by_id(&field_id, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
    ...
    trace!("Creating access policy for field {}...", field_id);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/fields/{field_id}/access-policies",
