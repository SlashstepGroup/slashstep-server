use crate::{
    ...
    utilities::route_handler_utilities::{
        get_action_by_name, get_action_log_entry_expiration_timestamp, get_item_connection_by_id,
...
async fn handle_get_item_connection_request(
...
) -> Result<Json<GetResourceResponseBody<ItemConnection>>, HTTPError> {
    ...
    let get_item_connections_action =
        get_action_by_name("itemConnections.get", &state.database_pool).await?;
...
async fn handle_delete_item_connection_request(
    ...
    let delete_item_connections_action =
        get_action_by_name("itemConnections.delete", &state.database_pool).await?;
    verify_delegate_permissions(
        authenticated_app_authorization
            .as_ref()
            .map(|app_authorization| &app_authorization.id),
    ...
    if let Err(error) = target_item_connection.delete(&state.database_pool).await {
        let http_error = HTTPError::InternalServerError(Some(format!(
            "Failed to delete item connection: {:?}",
            error
        )));
        http_error.log();
...
async fn handle_patch_item_connection_request(
...
) -> Result<Json<PatchResourceResponseBody<ItemConnection>>, HTTPError> {
    let item_connection_id = get_uuid_from_string(&item_connection_id, "item connection").await?;
    let updated_item_connection_properties = get_request_body_without_json_rejection(body).await?;

    let original_target_item_connection =
        get_item_connection_by_id(&item_connection_id, &state.database_pool).await?;
    let update_access_policy_action =
        get_action_by_name("itemConnections.update", &state.database_pool).await?;
    verify_delegate_permissions(
        authenticated_app_authorization
            .as_ref()
            .map(|app_authorization| &app_authorization.id),
        ...
        .await
    {
        ...
        Err(error) => {
            let http_error = HTTPError::InternalServerError(Some(format!(
                "Failed to update item connection: {:?}",
                error
            )));
            http_error.log();
            return Err(http_error);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/item-connections/{item_connection_id}",
            axum::routing::get(handle_get_item_connection_request),
        )
        .route(
            "/item-connections/{item_connection_id}",
            axum::routing::delete(handle_delete_item_connection_request),
        )
        .route(
            "/item-connections/{item_connection_id}",
            axum::routing::patch(handle_patch_item_connection_request),
        )
