...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  assert_eq!(response_item_connection.item_connection_type_id, item_connection.item_connection_type_id);
  assert_eq!(response_item_connection.inward_item_id, item_connection.inward_item_id);
  assert_eq!(response_item_connection.outward_item_id, item_connection.outward_item_id);
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  match ItemConnection::get_by_id(&item_connection.id, &test_environment.database_pool).await.expect_err("Expected an item connection not found error.") {
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.patch(&format!("/item-connections/{}", original_item_connection.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let updated_item_connection: ItemConnection = response.json();
  assert_eq!(original_item_connection.id, updated_item_connection.id);
  assert_eq!(updated_item_connection.item_connection_type_id, dummy_item_connection_type.id);
  assert_eq!(updated_item_connection.inward_item_id, original_item_connection.inward_item_id);
  assert_eq!(updated_item_connection.outward_item_id, original_item_connection.outward_item_id);
