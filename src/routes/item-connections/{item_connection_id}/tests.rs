...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let get_item_connection_response_body =
        response.json::<GetResourceResponseBody<ItemConnection>>();
    let response_item_connection = get_item_connection_response_body.data;
    assert_eq!(response_item_connection.id, item_connection.id);
    assert_eq!(
        response_item_connection.item_connection_type_id,
        item_connection.item_connection_type_id
    );
    assert_eq!(
        response_item_connection.inward_item_id,
        item_connection.inward_item_id
    );
    assert_eq!(
        response_item_connection.outward_item_id,
        item_connection.outward_item_id
    );
...
async fn verify_successful_deletion_when_deleting_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let delete_item_connections_action =
        Action::get_by_name("itemConnections.delete", &test_environment.database_pool).await?;
    test_environment
        .create_server_access_policy(
            &user.id,
    ...
    match ItemConnection::get_by_id(&item_connection.id, &test_environment.database_pool)
        .await
        .expect_err("Expected an item connection not found error.")
    {
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
    ...
    let update_fields_action =
        Action::get_by_name("itemConnections.update", &test_environment.database_pool).await?;
    test_environment
    ...
    let dummy_item_connection_type = test_environment
        ...
        .await?;
    let updated_item_connection_properties = EditableItemConnectionProperties {
        item_connection_type_id: Some(Some(dummy_item_connection_type.id)),
    };
    ...
    let patch_item_connection_response_body: PatchResourceResponseBody<ItemConnection> =
        response.json();
    let updated_item_connection = patch_item_connection_response_body.data;
    assert_eq!(original_item_connection.id, updated_item_connection.id);
    assert_eq!(
        updated_item_connection.item_connection_type_id,
        dummy_item_connection_type.id
    );
    assert_eq!(
        updated_item_connection.inward_item_id,
        original_item_connection.inward_item_id
    );
    assert_eq!(
        updated_item_connection.outward_item_id,
        original_item_connection.outward_item_id
    );
