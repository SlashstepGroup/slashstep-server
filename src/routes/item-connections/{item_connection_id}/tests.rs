...
use crate::{
  Action, AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    access_policy::ActionPermissionLevel, item_connection::ItemConnection
...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let get_item_connections_action = Action::get_by_name("slashstep.itemConnections.get", &test_environment.database_pool).await?;
  test_environment.create_instance_access_policy(&user.id, &get_item_connections_action.id, &ActionPermissionLevel::User).await?;
...
//   // Grant access to the "slashstep.itemConnections.delete" action to the user.
//   let delete_fields_action = Action::get_by_name("slashstep.itemConnections.delete", &test_environment.database_pool).await?;
...
//   match App::get_by_id(&item_connection.id, &test_environment.database_pool).await.expect_err("Expected an app not found error.") {
...
//   let json_web_token_private_key = get_json_web_token_private_key().await?;
//   let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
//   let update_fields_action = Action::get_by_name("slashstep.itemConnections.update", &test_environment.database_pool).await?;
//   AccessPolicy::create(&InitialAccessPolicyProperties {
