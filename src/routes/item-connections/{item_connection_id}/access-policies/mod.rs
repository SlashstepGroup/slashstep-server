use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_item_connection_by_id, get_principal_type_and_id_from_principal,
...
async fn handle_list_access_policies_request(
    ...
    let item_connection_id = get_uuid_from_string(&item_connection_id, "item connection").await?;
    let item_connection =
        get_item_connection_by_id(&item_connection_id, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
    ...
    trace!(
        "Creating access policy for item connection {}...",
        item_connection_id
    );
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/item-connections/{item_connection_id}/access-policies",
