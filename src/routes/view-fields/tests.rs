...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "viewFields.get" action to the user.
  let user = test_environment.create_random_user().await?;
  let session = test_environment.create_random_session(Some(&user.id)).await?;
  let json_web_token_private_key = get_json_web_token_private_key().await?;
  let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
  let get_delegation_policies_action = Action::get_by_name("viewFields.get", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &get_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Grant access to the "viewFields.list" action to the user.
  let list_delegation_policies_action = Action::get_by_name("viewFields.list", &test_environment.database_pool).await?;
  ...
  test_environment.create_random_view_field(None, None).await?;
  ...
  let response = test_server.get(&format!("/view-fields"))
  ...
  let response_json: ListResourcesResponseBody::<ViewField> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_view_field_count = ViewField::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_view_field_count);

  let actual_delegation_policies = ViewField::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_view_field in actual_delegation_policies {

    let found_access_policy = response_json.resources.iter().find(|view_field| view_field.id == actual_view_field.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Create a dummy delegation policy.
  let dummy_view_field = test_environment.create_random_view_field(None, None).await?;
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_view_field.id.to_string()));
  let response = test_server.get(&format!("/view-fields"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let actual_view_field_count = ViewField::count(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_view_field_count);

  let actual_delegation_policies = ViewField::list(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_view_field in actual_delegation_policies {

    let found_action = response_json.resources.iter().find(|view_field| view_field.id == actual_view_field.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let view_field_count = ViewField::count("", &test_environment.database_pool, None, None).await?;
  let parent_view = test_environment.create_random_view(None, None).await?;
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - view_field_count + 1) {

    test_environment.create_random_view_field(Some(&parent_view.id), None).await?;
  ...
  let response_body: ListResourcesResponseBody::<ViewField> = response.json();
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/view-fields"))
