...
async fn handle_list_access_policies_request(
  ...
  println!("{:?}", state.database_pool.status());
  ...
  return list_access_policies(Query(query_parameters), State(state), Extension(http_transaction), Extension(authenticated_user), Extension(authenticated_app), resource_hierarchy, ActionLogEntryTargetResourceType::ActionLogEntry, Some(action_log_entry.id)).await;
...
async fn handle_create_access_policy_request(
  ...
  ServerLogEntry::trace(&format!("Creating access policy for action log entry {}...", action_log_entry_id), Some(&http_transaction.id), &postgres_client).await.ok();
