...
use crate::{
    ...
    utilities::route_handler_utilities::{
        get_action_by_name, get_action_log_entry_by_id, get_action_log_entry_expiration_timestamp,
...
async fn handle_delete_action_log_entry_request(
    ...
    if let Err(error) = target_action_log_entry.delete(&state.database_pool).await {
        let http_error = HTTPError::InternalServerError(Some(format!(
            "Failed to delete action log entry {}: {:?}",
            target_action_log_entry.id, error
        )));
        http_error.log();
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/action-log-entries/{action_log_entry_id}",
            axum::routing::get(handle_get_action_log_entry_request),
        )
        .route(
            "/action-log-entries/{action_log_entry_id}",
            axum::routing::delete(handle_delete_action_log_entry_request),
        )
