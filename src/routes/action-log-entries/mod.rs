use crate::{
    ...
    resources::{
        ...
        action_log_entry::{
            ActionLogEntry, ActionLogEntryActorType, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT,
...
async fn handle_list_action_log_entries_request(
        ...
        Err(error) => {
            let http_error = match error {
                ...
                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list action log entries: {:?}",
                    error
                ))),
            };
    ...
    ServerLogEntry::trace(
        &format!("Counting action log entries..."),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        .route(
            "/action-log-entries",
            axum::routing::get(handle_list_action_log_entries_request),
        )
