...
async fn handle_list_action_log_entries_request(
  ...
  let queried_resources = match ActionLogEntry::list(&query, &state.database_pool, Some(&principal_type), Some(&principal_id)).await {
    ...
    Err(error) => {
      ...
      let http_error = match error {
        ...
        ResourceError::SlashstepQLError(error) => match_slashstepql_error(&error, &DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, "action log entries"),
  ...
  let resource_count = match ActionLogEntry::count(&query, &state.database_pool, Some(&principal_type), Some(&principal_id)).await {
