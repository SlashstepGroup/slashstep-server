...
async fn handle_list_iterations_request(
  ...
  ServerLogEntry::trace("Listing iterations...", Some(&http_transaction.id), &state.database_pool).await.ok();
  ...
  let queried_iteration_list_length = queried_resources.len();
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_iteration_list_length, if queried_iteration_list_length == 1 { "iteration" } else { "iterations" }), Some(&http_transaction.id), &state.database_pool).await.ok();
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/iterations", axum::routing::get(handle_list_iterations_request))
    ...
    .merge(iteration_id::get_router(state.clone()));
  return router;
