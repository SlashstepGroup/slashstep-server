use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_iteration_by_id, get_principal_type_and_id_from_principal,
...
async fn handle_list_access_policies_request(
    ...
    let iteration = get_iteration_by_id(&iteration_id, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
    ...
    trace!("Creating access policy for iteration {}...", iteration_id);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/iterations/{iteration_id}/access-policies",
