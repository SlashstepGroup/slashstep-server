...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  assert_eq!(response_json.data.len(), actual_iterations.len());

  for actual_iteration in actual_iterations {

    let found_access_policy = response_json.data.iter().find(|iteration| iteration.id == actual_iteration.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  for actual_iteration in actual_iterations {
    ...
    let found_action = response_json.data.iter().find(|iteration| iteration.id == actual_iteration.id);
