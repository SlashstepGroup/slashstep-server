...
use crate::{
  AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, action::Action, iteration::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Iteration},
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "iterations.get" action to the user.
  let user = test_environment.create_random_user().await?;
  let session = test_environment.create_random_session(Some(&user.id)).await?;
  let json_web_token_private_key = get_json_web_token_private_key().await?;
  let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
  let get_delegation_policies_action = Action::get_by_name("iterations.get", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &get_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Grant access to the "iterations.list" action to the user.
  let list_delegation_policies_action = Action::get_by_name("iterations.list", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &list_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Create a dummy iteration.
  test_environment.create_random_iteration(None).await?;
  ...
  let response = test_server.get(&format!("/iterations"))
  ...
  let response_json: ListResourcesResponseBody::<Iteration> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_iteration_count = Iteration::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_iteration_count);

  let actual_iterations = Iteration::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_iterations.len());

  for actual_iteration in actual_iterations {

    let found_access_policy = response_json.resources.iter().find(|iteration| iteration.id == actual_iteration.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_iteration.id.to_string()));
  let response = test_server.get(&format!("/iterations"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  for actual_iteration in actual_iterations {
    ...
    let found_action = response_json.resources.iter().find(|iteration| iteration.id == actual_iteration.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  let iteration_count = Iteration::count("", &test_environment.database_pool, None, None).await?;
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - iteration_count + 1) {
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/iterations"))
