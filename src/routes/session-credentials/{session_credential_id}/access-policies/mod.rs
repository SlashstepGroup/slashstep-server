/*
 *
 * Any functionality for /session-credentials/{session_credential_id}/access-policies should be handled here.
 *
 * Programmers:
...
use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_session_credential_by_id, get_uuid_from_string, is_authenticated_user_anonymous,
        match_db_error, match_slashstepql_error, verify_delegate_permissions,
...
/// GET /session-credentials/{session_credential_id}/access-policies
///
/// Lists access policies for a session.
#[axum::debug_handler]
async fn handle_list_access_policies_request(
    Path(session_credential_id): Path<String>,
    ...
    let session_credential_id =
        get_uuid_from_string(&session_credential_id, "session credential").await?;
    let session_credential =
        get_session_credential_by_id(&session_credential_id, &state.database_pool).await?;
    ...
    verify_principal_permissions(
        ...
        &ResourceType::SessionCredential,
        Some(&session_credential.id),
    ...
    let query = format!(
        "scoped_resource_type = 'Session' AND scoped_session_credential_id = {}{}",
        quote_literal(&session_credential_id.to_string()),
        query_parameters
            .query
    ...
    ActionLogEntry::create(
        &InitialActionLogEntryProperties {
            ...
            target_session_credential_id: Some(session_credential_id),
            ..Default::default()
        },
        &state.database_pool,
    )
...
/// POST /session-credentials/{session_credential_id}/access-policies
///
...
async fn handle_create_access_policy_request(
    ...
    trace!(
        "Creating access policy for session credential {}...",
    ...
    let access_policy = match AccessPolicy::create(
        &InitialAccessPolicyProperties {
            ...
            scoped_resource_type: ResourceType::SessionCredential,
            scoped_session_credential_id: Some(target_session.id),
            ..Default::default()
        },
        &state.database_pool,
    )
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/session-credentials/{session_credential_id}/access-policies",
