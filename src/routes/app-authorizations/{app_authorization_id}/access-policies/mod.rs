...
async fn handle_list_access_policies_request(
  ...
  let app_authorization = get_app_authorization_by_id(&app_authorization_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppAuthorization, Some(&app_authorization.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
  ...
  // Make sure the authenticated_user can create access policies for the target app authorization.
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppAuthorization, Some(&target_app_authorization.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::AppAuthorization, Some(&target_app_authorization.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;
