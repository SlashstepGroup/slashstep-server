...
use crate::{
  ...
  resources::{
    ResourceError, access_policy::ActionPermissionLevel, app_authorization::AppAuthorization, session::Session
  }, 
...
async fn verify_successful_deletion_when_deleting_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_instance_access_policy(&user.id, &delete_app_authorizations_action.id, &ActionPermissionLevel::User).await?;
