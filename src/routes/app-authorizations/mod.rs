...
async fn handle_list_app_authorizations_request(
  ...
  let queried_resources = match AppAuthorization::list(&query, &state.database_pool, Some(&individual_principal)).await {
  ...
  let resource_count = match AppAuthorization::count(&query, &state.database_pool, Some(&individual_principal)).await {
