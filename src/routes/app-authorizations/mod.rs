...
async fn handle_list_app_authorizations_request(
    ...
    (
        StatusCode,
        Json<ListResourcesResponseBody<AppAuthorization>>,
    ),
    HTTPError,
> {
        ...
        Err(error) => {
            let http_error = match error {
                ...
                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list app authorizations: {:?}",
                    error
                ))),
            };
    ...
    ServerLogEntry::trace(
        &format!("Counting app authorizations..."),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
...
pub fn get_router(state: AppState) -> Router<AppState> {
    let router = Router::<AppState>::new()
        .route(
            "/app-authorizations",
            axum::routing::get(handle_list_app_authorizations_request),
        )
