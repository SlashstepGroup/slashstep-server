use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_user_by_id, get_uuid_from_string, is_authenticated_user_anonymous, match_db_error,
...
async fn handle_create_access_policy_request(
    ...
    ServerLogEntry::trace(
        &format!("Creating access policy for user {}...", user_id),
        Some(&http_transaction.id),
        &state.database_pool,
    )
    .await
