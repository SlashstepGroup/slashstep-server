...
async fn verify_successful_creation() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_server_access_policy(&user.id, &create_oauth_authorizations_action.id, &PermissionLevel::User).await?;
  let authorize_app_action = Action::get_by_name("apps.authorize", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &authorize_app_action.id, &PermissionLevel::User).await?;
