...
use crate::{
  ...
  utilities::route_handler_utilities::{get_action_by_name, get_action_log_entry_expiration_timestamp, get_user_by_id, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}
};
...
async fn handle_get_user_request(
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &get_user_action.id, &http_transaction.id, &ActionPermissionLevel::User, &state.database_pool).await?;

  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ActionLogEntry, Some(&action_log_entry.id), &get_user_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  
  let expiration_timestamp = get_action_log_entry_expiration_timestamp(&http_transaction, &state.database_pool).await?;
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    action_id: get_user_action.id,
    ...
    target_resource_type: ActionLogEntryTargetResourceType::User,
...
//   let user_id = get_uuid_from_string(&user_id, "app", &http_transaction, &state.database_pool).await?;
...
//     |user_id, database_pool| Box::new(App::get_by_id(user_id, database_pool))
//   ).await;
...
//   let original_target_field = get_app_by_id(&user_id, &http_transaction, &state.database_pool).await?;
...
//     actor_user_id: if let AuthenticatedPrincipal::App(authenticated_app) = &authenticated_principal { Some(authenticated_app.id.clone()) } else { None },
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    ...
    // .route("/users/{user_id}", axum::routing::delete(handle_delete_app_request))
    // .route("/users/{user_id}", axum::routing::patch(handle_patch_app_request))
