...
use crate::{
    AppState, HTTPError,
    middleware::{
        authentication_middleware::{self, get_decoding_key},
        http_transaction_middleware, rate_limit_middleware,
    },
    resources::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_json_web_token_public_key, get_password_reset_authorization_by_id,
...
async fn handle_update_user_password_request(
                    ...
                    let http_error = match error.kind() {
                        ...
                        | jsonwebtoken::errors::ErrorKind::MissingRequiredClaim(_) => {
                            HTTPError::Unauthorized(Some(
                                "The authorization code is invalid.".to_string(),
                            ))
                        }

                        _ => HTTPError::InternalServerError(Some(format!(
                            "Failed to decode and verify password reset token: {:?}",
                            error
                        ))),
                    };
    ...
    async fn verify_password_meets_requirements(
        ...
        let minimum_password_length = match minimum_password_length_configuration
            .number_value
            .unwrap_or(
                minimum_password_length_configuration
    ...
    .await?;
    let authenticated_app_authorization_id = authenticated_app_authorization
        .as_ref()
    ...
    let updated_user = match target_user
        .update(
    ...
    if update_user_password_request_body.should_delete_other_sessions {
        ...
        let delete_all_user_session_rows_except_one_query = include_str!(
            "../../../../queries/sessions/delete_all_user_session_rows_except_one.sql"
        );
