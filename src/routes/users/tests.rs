...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  let get_delegation_policies_action = Action::get_by_name("users.get", &test_environment.database_pool).await?;
  ...
  let list_delegation_policies_action = Action::get_by_name("users.list", &test_environment.database_pool).await?;
  ...
  test_environment.create_random_user().await?;
  ...
  let response = test_server.get(&format!("/users"))
  ...
  let actual_user_count = User::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_user_count);

  let actual_delegation_policies = User::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_user in actual_delegation_policies {

    let found_access_policy = response_json.resources.iter().find(|user| user.id == actual_user.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let actual_user_count = User::count(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_user_count);

  let actual_delegation_policies = User::list(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_user in actual_delegation_policies {

    let found_action = response_json.resources.iter().find(|user| user.id == actual_user.id);
