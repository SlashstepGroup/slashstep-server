/**
 * 
 * Any test cases for /statuses should be handled here.
 * 
 * Programmers: 
...
use crate::{
  AppState, get_json_web_token_private_key, initialize_required_tables, predefinitions::{
    ...
    }, action::Action, status::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DEFAULT_RESOURCE_LIST_LIMIT, Status},
...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "statuses.get" action to the user.
  let user = test_environment.create_random_user().await?;
  let session = test_environment.create_random_session(Some(&user.id)).await?;
  let json_web_token_private_key = get_json_web_token_private_key().await?;
  let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
  let get_delegation_policies_action = Action::get_by_name("statuses.get", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &get_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Grant access to the "statuses.list" action to the user.
  let list_delegation_policies_action = Action::get_by_name("statuses.list", &test_environment.database_pool).await?;
  ...
  test_environment.create_random_status(None).await?;
  ...
  let response = test_server.get(&format!("/statuses"))
  ...
  let response_json: ListResourcesResponseBody::<Status> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_status_count = Status::count("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.total_count, actual_status_count);

  let actual_delegation_policies = Status::list("", &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_status in actual_delegation_policies {

    let found_access_policy = response_json.resources.iter().find(|status| status.id == actual_status.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let query = format!("id = {}", quote_literal(&dummy_status.id.to_string()));
  let response = test_server.get(&format!("/statuses"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let actual_delegation_policies = Status::list(&query, &test_environment.database_pool, Some(&AccessPolicyPrincipalType::User), Some(&user.id)).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_status in actual_delegation_policies {

    let found_action = response_json.resources.iter().find(|status| status.id == actual_status.id);
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/statuses"))
