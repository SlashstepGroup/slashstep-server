...
use crate::{
    ...
    resources::{
        ...
        delegation_policy::{DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, DelegationPolicy},
...
async fn handle_list_delegation_policies_request(
    ...
    (
        StatusCode,
        Json<ListResourcesResponseBody<DelegationPolicy>>,
    ),
    HTTPError,
> {
        ...
        Err(error) => {
            let http_error = match error {
                ...
                ResourceError::PostgresError(error) => {
                    match_db_error(&error, "delegation policies")
                }

                _ => HTTPError::InternalServerError(Some(format!(
                    "Failed to list delegation policies: {:?}",
                    error
                ))),
            };
    ...
    info!("Successfully returned {} {}.", queried_delegation_policy_list_length, if queried_delegation_policy_list_length == 1 { "delegation policy" } else { "delegation policies" });
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/delegation-policies",
            axum::routing::get(handle_list_delegation_policies_request),
        )
