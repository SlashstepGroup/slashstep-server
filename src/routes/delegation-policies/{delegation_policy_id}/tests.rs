...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.get(&format!("/delegation-policies/{}", delegation_policy.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .await;
  
  assert_eq!(response.status_code(), StatusCode::OK);

  let response_delegation_policy: DelegationPolicy = response.json();
  assert_eq!(response_delegation_policy.id, delegation_policy.id);
  assert_eq!(response_delegation_policy.action_id, delegation_policy.action_id);
  assert_eq!(response_delegation_policy.maximum_permission_level, delegation_policy.maximum_permission_level);
  assert_eq!(response_delegation_policy.delegate_app_authorization_id, delegation_policy.delegate_app_authorization_id);
  assert_eq!(response_delegation_policy.principal_user_id, delegation_policy.principal_user_id);
...
async fn verify_uuid_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.get("/delegation-policies/not-a-uuid")
    .await;
...
async fn verify_not_found_when_getting_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.get(&format!("/delegation-policies/{}", uuid::Uuid::now_v7()))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
    .await;
...
//   let delete_fields_action = Action::get_by_name("slashstep.delegationPolicies.delete", &test_environment.database_pool).await?;
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.delete(&format!("/delegation-policies/{}", delegation_policy.id))
//     .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
//     .await;
  
//   assert_eq!(response.status_code(), StatusCode::NO_CONTENT);

//   match App::get_by_id(&delegation_policy.id, &test_environment.database_pool).await.expect_err("Expected an app not found error.") {
...
//   let response = test_server.delete("/delegation-policies/not-a-uuid")
//     .await;
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.delete(&format!("/delegation-policies/{}", uuid::Uuid::now_v7()))
//     .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
//     .await;
...
async fn verify_successful_patch_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.patch(&format!("/delegation-policies/{}", original_delegation_policy.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let updated_delegation_policy: DelegationPolicy = response.json();
  assert_eq!(updated_delegation_policy.id, original_delegation_policy.id);
  assert_eq!(updated_delegation_policy.action_id, original_delegation_policy.action_id);
  assert_eq!(updated_delegation_policy.maximum_permission_level, new_maximum_permission_level);
  assert_eq!(updated_delegation_policy.delegate_app_authorization_id, original_delegation_policy.delegate_app_authorization_id);
  assert_eq!(updated_delegation_policy.principal_user_id, original_delegation_policy.principal_user_id);
...
async fn verify_content_type_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.patch("/delegation-policies/not-a-uuid")
    .await;
...
async fn verify_request_body_json_when_patching_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.patch(&format!("/delegation-policies/{}", delegation_policy.id))
    .add_header("Content-Type", "application/json")
    .json(&serde_json::json!({
...
async fn verify_resource_exists_when_patching() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.patch(&format!("/delegation-policies/{}", Uuid::now_v7()))
    .json(&serde_json::json!({
      "display_name": Uuid::now_v7().to_string()
    }))
    .await;
