...
async fn handle_get_item_connection_type_request(
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &get_item_connection_types_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemConnectionType, Some(&target_item_connection_type.id), &get_item_connection_types_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
...
async fn handle_delete_item_connection_type_request(
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &delete_item_connection_types_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemConnectionType, Some(&target_item_connection_type.id), &delete_item_connection_types_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;

  if let Err(error) = target_item_connection_type.delete(&state.database_pool).await {

    let http_error = HTTPError::InternalServerError(Some(format!("Failed to delete item connection type: {:?}", error)));
  ...
  ServerLogEntry::success(&format!("Successfully deleted item connection type {}.", target_item_connection_type.id), Some(&http_transaction.id), &state.database_pool).await.ok();
...
async fn handle_patch_item_connection_type_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemConnectionType, Some(&original_target_item_connection_type.id), &update_access_policy_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
