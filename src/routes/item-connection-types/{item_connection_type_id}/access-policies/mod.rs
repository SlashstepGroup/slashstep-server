/**
 * 
 * Any functionality for /item-connection-types/{item_connection_type_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
/// 
/// Lists access policies for an item connection type.
#[axum::debug_handler]
async fn handle_list_access_policies_request(
  ...
  let item_connection_type = get_item_connection_type_by_id(&item_connection_type_id, &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemConnectionType, Some(&item_connection_type.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;

  let query = format!(
    "scoped_resource_type = 'ItemConnectionType' AND scoped_item_connection_type_id = {}{}", 
    quote_literal(&item_connection_type_id.to_string()), 
...
/// POST /item-connection-types/{item_connection_type_id}/access-policies
/// 
/// Creates an access policy for an item connection type.
#[axum::debug_handler]
async fn handle_create_access_policy_request(
  ...
  // Make sure the authenticated_user can create access policies for the target item connection type.
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemConnectionType, Some(&target_item_connection_type.id), &create_access_policies_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::ItemConnectionType, Some(&target_item_connection_type.id), &access_policy_action, &http_transaction, &minimum_permission_level, &state.database_pool).await?;

  // Create the access policy.
  ServerLogEntry::trace(&format!("Creating access policy for item connection type {}...", item_connection_type_id), Some(&http_transaction.id), &state.database_pool).await.ok();
  let access_policy = match AccessPolicy::create(&InitialAccessPolicyProperties {
    ...
    scoped_item_connection_type_id: Some(target_item_connection_type.id),
    ..Default::default()
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/item-connection-types/{item_connection_type_id}/access-policies", axum::routing::get(handle_list_access_policies_request))
    .route("/item-connection-types/{item_connection_type_id}/access-policies", axum::routing::post(handle_create_access_policy_request))
