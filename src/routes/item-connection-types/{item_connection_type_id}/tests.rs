...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_instance_access_policy(&user.id, &get_item_connection_types_action.id, &ActionPermissionLevel::User).await?;
...
//   // Grant access to the "slashstep.itemConnectionTypes.delete" action to the user.
//   let delete_fields_action = Action::get_by_name("slashstep.itemConnectionTypes.delete", &test_environment.database_pool).await?;
