...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  // Grant access to the "itemConnectionTypes.get" action to the user.
  let user = test_environment.create_random_user().await?;
  let session = test_environment.create_random_session(Some(&user.id)).await?;
  let json_web_token_private_key = get_json_web_token_private_key().await?;
  let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
  let get_delegation_policies_action = Action::get_by_name("itemConnectionTypes.get", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &get_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Grant access to the "itemConnectionTypes.list" action to the user.
  let list_delegation_policies_action = Action::get_by_name("itemConnectionTypes.list", &test_environment.database_pool).await?;
  test_environment.create_server_access_policy(&user.id, &list_delegation_policies_action.id, &ActionPermissionLevel::User).await?;

  // Create a dummy item connection type.
  test_environment.create_random_item_connection_type().await?;
  ...
  let response = test_server.get(&format!("/item-connection-types"))
  ...
  let response_json: ListResourcesResponseBody::<ItemConnectionType> = response.json();
  assert!(response_json.total_count > 0);
  assert!(response_json.resources.len() > 0);

  let actual_item_connection_type_count = ItemConnectionType::count("", &test_environment.database_pool, Some(&IndividualPrincipal::User(user.id))).await?;
  assert_eq!(response_json.total_count, actual_item_connection_type_count);

  let actual_delegation_policies = ItemConnectionType::list("", &test_environment.database_pool, Some(&IndividualPrincipal::User(user.id))).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_item_connection_type in actual_delegation_policies {

    let found_access_policy = response_json.resources.iter().find(|item_connection_type| item_connection_type.id == actual_item_connection_type.id);
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let query = format!("id = {}", quote_literal(&dummy_item_connection_type.id.to_string()));
  let response = test_server.get(&format!("/item-connection-types"))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
  ...
  let actual_item_connection_type_count = ItemConnectionType::count(&query, &test_environment.database_pool, Some(&IndividualPrincipal::User(user.id))).await?;
  assert_eq!(response_json.total_count, actual_item_connection_type_count);

  let actual_delegation_policies = ItemConnectionType::list(&query, &test_environment.database_pool, Some(&IndividualPrincipal::User(user.id))).await?;
  assert_eq!(response_json.resources.len(), actual_delegation_policies.len());

  for actual_item_connection_type in actual_delegation_policies {

    let found_action = response_json.resources.iter().find(|item_connection_type| item_connection_type.id == actual_item_connection_type.id);
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  // Create dummy item connection types.
  let item_connection_type_count = ItemConnectionType::count("", &test_environment.database_pool, None).await?;
  for _ in 0..(DEFAULT_RESOURCE_LIST_LIMIT - item_connection_type_count + 1) {
...
async fn verify_query_validity() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/item-connection-types"))
