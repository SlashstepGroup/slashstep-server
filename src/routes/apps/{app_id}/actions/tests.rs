...
async fn verify_successful_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let initial_access_policy_properties = InitialActionPropertiesForPredefinedScope {
  ...
  assert_eq!(initial_access_policy_properties.name, response_action.name);
  assert_eq!(initial_access_policy_properties.display_name, response_action.display_name);
  assert_eq!(initial_access_policy_properties.description, response_action.description);
