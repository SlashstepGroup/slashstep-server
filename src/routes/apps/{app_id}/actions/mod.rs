use crate::{
    ...
    resources::{
        ...
        action::{
            Action, ActionParentResourceType, DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT,
...
async fn handle_list_actions_request(
    ...
    let list_resources_action =
        get_action_by_name("actions.list", &http_transaction, &state.database_pool).await?;
        ...
        Err(error) => {
            let http_error = match error {
                ResourceError::SlashstepQLError(error) => {
                    match_slashstepql_error(&error, &DEFAULT_MAXIMUM_RESOURCE_LIST_LIMIT, "actions")
                }
    ...
    info!("Successfully returned {} {}.", queried_resource_list_length, if queried_resource_list_length == 1 { "action" } else { "actions" });
...
async fn handle_create_action_request(
    ...
    let create_actions_action =
        get_action_by_name("actions.create", &http_transaction, &state.database_pool).await?;
    verify_delegate_permissions(
        authenticated_app_authorization
            .as_ref()
            .map(|app_authorization| &app_authorization.id),
        &create_actions_action.id,
        &http_transaction.id,
        &PermissionLevel::User,
        &state.database_pool,
    )
    ...
    // Create the action.
    trace!("Creating action for authenticated_app {}...", target_app.id);
    let created_action = match Action::create(
    ...
    .await
    .ok();
    info!("Successfully created action {}.", created_action.id);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        ...
        .route(
            "/apps/{app_id}/actions",
            axum::routing::post(handle_create_action_request),
        )
