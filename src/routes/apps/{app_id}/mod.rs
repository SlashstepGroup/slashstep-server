...
use crate::{
  ...
  resources::{
    ResourceType, access_policy::{PermissionLevel}, action_log_entry::{ActionLogEntry, ActionLogEntryActorType, InitialActionLogEntryProperties}, app::{App, EditableAppProperties}, app_authorization::AppAuthorization, http_transaction::HTTPTransaction, server_log_entry::ServerLogEntry, user::User
  }, 
  utilities::route_handler_utilities::{get_action_by_name, get_action_log_entry_expiration_timestamp, get_app_by_id, get_principal_type_and_id_from_principal, get_request_body_without_json_rejection, get_uuid_from_string, is_authenticated_user_anonymous, validate_field_length, verify_delegate_permissions, verify_principal_permissions}
};
...
async fn handle_get_app_request(
  ...
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &get_apps_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::App, Some(&target_app.id), &get_apps_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
...
async fn handle_delete_app_request(
  ...
  let app_id = get_uuid_from_string(&app_id, "app", &http_transaction, &state.database_pool).await?;
  let target_app = get_app_by_id(&app_id, &http_transaction, &state.database_pool).await?;
  let delete_apps_action = get_action_by_name("apps.delete", &http_transaction, &state.database_pool).await?;
  verify_delegate_permissions(authenticated_app_authorization.as_ref().map(|app_authorization| &app_authorization.id), &delete_apps_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::App, Some(&target_app.id), &delete_apps_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
  ...
  ServerLogEntry::success(&format!("Successfully deleted app {}.", target_app.id), Some(&http_transaction.id), &state.database_pool).await.ok();
...
async fn handle_patch_app_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::App, Some(&original_target_app.id), &update_access_policy_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
