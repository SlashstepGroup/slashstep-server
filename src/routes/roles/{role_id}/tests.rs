...
async fn verify_returned_resource_by_id() -> Result<(), TestSlashstepServerError> {
  ...
  let get_roles_action = Action::get_by_name("slashstep.roles.get", &test_environment.database_pool).await?;
  test_environment.create_instance_access_policy(&user.id, &get_roles_action.id, &ActionPermissionLevel::User).await?;
  ...
  assert_eq!(response_role.parent_group_id, role.parent_group_id);
...
//   // Grant access to the "slashstep.roles.delete" action to the user.
//   let delete_fields_action = Action::get_by_name("slashstep.roles.delete", &test_environment.database_pool).await?;
...
//   match App::get_by_id(&role.id, &test_environment.database_pool).await.expect_err("Expected an app not found error.") {
...
//   let json_web_token_private_key = get_json_web_token_private_key().await?;
//   let session_token = session.generate_json_web_token(&json_web_token_private_key).await?;
//   let update_fields_action = Action::get_by_name("slashstep.roles.update", &test_environment.database_pool).await?;
//   AccessPolicy::create(&InitialAccessPolicyProperties {
...
//     .into_make_service_with_connect_info::<SocketAddr>();
//   let test_server = TestServer::new(router)?;
//   let response = test_server.patch(&format!("/roles/{}", original_field.id))
//     .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", session_token)))
