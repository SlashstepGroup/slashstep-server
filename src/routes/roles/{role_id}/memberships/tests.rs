...
async fn verify_returned_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.get(&format!("/roles/{}/memberships", &dummy_role.id))
...
async fn verify_returned_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.get(&format!("/roles/{}/memberships", &dummy_membership.parent_role_id.unwrap()))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", &session_token)))
...
async fn verify_default_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  // Create dummy resources.
  let dummy_role = test_environment.create_random_role(None, None).await?;
  let membership_count = Membership::count(format!("parent_role_id = {}", quote_literal(&dummy_role.id.to_string())).as_str(), &test_environment.database_pool, None, None).await?;
...
async fn verify_query_when_listing_resources() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/roles/{}/memberships", &dummy_role.id))
...
async fn verify_parent_resource_not_found_when_listing_resources() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.get(&format!("/roles/{}/memberships", &Uuid::now_v7()))
    .await;
...
async fn verify_successful_creation() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router);
  let response = test_server.post(&format!("/roles/{}/memberships", dummy_role.id))
    .add_cookie(Cookie::new("session_access_token", format!("Bearer {}", session_token)))
  ...
  assert_eq!(response_membership.parent_resource_type, MembershipParentResourceType::Role);
...
async fn verify_not_found_when_creating_resource() -> Result<(), TestSlashstepServerError> {
  ...
  let response = test_server.post(&format!("/roles/{}/memberships", uuid::Uuid::now_v7()))
    .add_header("Content-Type", "application/json")
    .json(&serde_json::json!(initial_membership_properties))
    .await;
