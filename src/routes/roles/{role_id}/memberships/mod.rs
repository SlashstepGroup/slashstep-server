...
/// POST /roles/{role_id}/memberships
/// 
/// Creates a new membership for a user in a role.
#[axum::debug_handler]
pub async fn handle_create_membership_request(
  ...
  let join_roles_action = get_action_by_name("roles.join", &http_transaction, &state.database_pool).await?;
  ...
  if let Some(membership_invitation_id) = query_parameters.membership_invitation_id {
    ...
    // If the principal can create memberships but doesn't have the permission to add themselves to the role,
    // then the permission to create memberships takes priority over the permission to join roles.
    let can_delegate_add_self = is_user_only_adding_self && can_delegate_perform_action(app_authorization_id, &join_roles_action.id, &http_transaction.id, &ActionPermissionLevel::User, &state.database_pool).await?;
    ...
    can_principal_add_self = is_user_only_adding_self && can_principal_perform_action(&principal_type, &principal_id, &ResourceType::Role, Some(&target_role.id), &join_roles_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
    if !can_principal_add_self {

      verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Role, Some(&target_role.id), &create_memberships_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
  ...
  let expiration_timestamp = get_action_log_entry_expiration_timestamp(&http_transaction, &state.database_pool).await?;
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    action_id: if can_principal_add_self { join_roles_action.id } else { create_memberships_action.id },
    http_transaction_id: Some(http_transaction.id),
    expiration_timestamp: expiration_timestamp,
    reason: None, // TODO: Support reasons.
    actor_type: if authenticated_user.is_some() { ActionLogEntryActorType::User } else { ActionLogEntryActorType::App },
...
pub async fn handle_list_memberships_request(
  ...
  let target_role = get_role_by_id(&role_id, &http_transaction, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Role, Some(&target_role.id), &list_resources_action, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    .route("/roles/{role_id}/memberships", axum::routing::get(handle_list_memberships_request))
    .route("/roles/{role_id}/memberships", axum::routing::post(handle_create_membership_request))
