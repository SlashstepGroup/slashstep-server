...
pub async fn handle_create_membership_request(
  ...
  if let Some(membership_invitation_id) = query_parameters.membership_invitation_id {
    ...
    let can_delegate_add_self = is_user_only_adding_self && can_delegate_perform_action(app_authorization_id, &join_roles_action.id, &http_transaction.id, &PermissionLevel::User, &state.database_pool).await?;
    ...
    can_principal_add_self = is_user_only_adding_self && can_principal_perform_action(&principal_type, &principal_id, &ResourceType::Role, Some(&target_role.id), &join_roles_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
    if !can_principal_add_self {

      verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Role, Some(&target_role.id), &create_memberships_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
...
pub async fn handle_list_memberships_request(
  ...
  let target_role = get_role_by_id(&role_id, &http_transaction, &state.database_pool).await?;
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Role, Some(&target_role.id), &list_resources_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
