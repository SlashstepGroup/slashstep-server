...
use crate::{
    ...
    utilities::route_handler_utilities::{
        can_delegate_perform_action, can_principal_perform_action, get_action_by_name,
        get_action_log_entry_expiration_timestamp, get_membership_invitation_by_id,
        get_principal_type_and_id_from_principal, get_request_body_without_json_rejection,
        get_role_by_id, get_uuid_from_string, is_authenticated_user_anonymous, match_db_error,
...
pub async fn handle_create_membership_request(
    ...
    let join_roles_action = get_action_by_name("roles.join", &state.database_pool).await?;
    ...
    if let Some(membership_invitation_id) = query_parameters.membership_invitation_id {
        ...
        let can_delegate_add_self = is_user_only_adding_self
        ...
        can_principal_add_self = is_user_only_adding_self
    ...
    info!("Successfully created membership {}.", created_membership.id);
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        ...
        .route(
            "/roles/{role_id}/memberships",
            axum::routing::post(handle_create_membership_request),
        )
