...
use crate::{
  ...
  utilities::{route_handler_utilities::{AuthenticatedPrincipal, get_action_by_name, get_authenticated_principal, get_role_by_id, get_resource_hierarchy, get_uuid_from_string, verify_delegate_permissions, verify_principal_permissions}}
};
...
async fn handle_get_role_request(
  ...
  let get_roles_action = get_action_by_name("slashstep.roles.get", &http_transaction, &state.database_pool).await?;
  ...
  verify_principal_permissions(&authenticated_principal, &get_roles_action, &resource_hierarchy, &http_transaction, &ActionPermissionLevel::User, &state.database_pool).await?;
...
//   let role_id = get_uuid_from_string(&role_id, "app", &http_transaction, &state.database_pool).await?;
...
pub fn get_router(state: AppState) -> Router<AppState> {
  ...
  let router = Router::<AppState>::new()
    ...
    // .route("/roles/{role_id}", axum::routing::delete(handle_delete_app_request))
    // .route("/roles/{role_id}", axum::routing::patch(handle_patch_app_request))
