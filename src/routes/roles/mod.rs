...
async fn handle_list_roles_request(
  ...
  ServerLogEntry::trace("Listing roles...", Some(&http_transaction.id), &state.database_pool).await.ok();
  ...
  ServerLogEntry::success(&format!("Successfully returned {} {}.", queried_role_list_length, if queried_role_list_length == 1 { "role" } else { "roles" }), Some(&http_transaction.id), &state.database_pool).await.ok();
...
async fn handle_create_role_request(
  ...
  let (principal_type, principal_id) = get_principal_type_and_id_from_principal(authenticated_user.as_ref(), authenticated_app.as_ref())?;
  verify_principal_permissions(&principal_type, &principal_id, is_authenticated_user_anonymous(authenticated_user.as_ref()), &ResourceType::Server, None, &create_roles_action, &http_transaction, &PermissionLevel::User, &state.database_pool).await?;
