/**
 * 
 * Any test cases for /configurations/{configuration_id}/access-policies should be handled here.
 * 
 * Programmers: 
...
async fn create_configuration_access_policy(database_pool: &deadpool_postgres::Pool, scoped_configuration_id: &Uuid, user_id: &Uuid, action_id: &Uuid, permission_level: &ActionPermissionLevel) -> Result<AccessPolicy, TestSlashstepServerError> {
...
async fn verify_successful_access_policy_creation() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_instance_access_policy(&user.id, &create_access_policies_action.id, &ActionPermissionLevel::User).await?;
  ...
  test_environment.create_instance_access_policy(&user.id, &dummy_action.id, &ActionPermissionLevel::Editor).await?;
  ...
  let response = test_server.post(&format!("/configurations/{}/access-policies", dummy_configuration.id))
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError> {
  ...
  test_environment.create_instance_access_policy(&user.id, &get_access_policies_action.id, &ActionPermissionLevel::User).await?;
  ...
  test_environment.create_instance_access_policy(&user.id, &list_access_policies_action.id, &ActionPermissionLevel::User).await?;

  // Create a dummy action.
  let dummy_configuration = test_environment.create_random_configuration().await?;
  let shown_access_policy = create_configuration_access_policy(&test_environment.database_pool, &dummy_configuration.id, &user.id, &list_access_policies_action.id, &ActionPermissionLevel::User).await?;
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.get(&format!("/configurations/{}/access-policies", &dummy_configuration.id))
    .add_cookie(Cookie::new("sessionToken", format!("Bearer {}", &session_token)))
    .await;
  ...
  let query = format!("scoped_resource_type = 'Configuration' AND scoped_configuration_id = {}", quote_literal(&dummy_configuration.id.to_string()));
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
  ...
  let dummy_configuration = test_environment.create_random_configuration().await?;
  create_configuration_access_policy(&test_environment.database_pool, &dummy_configuration.id, &user.id, &list_access_policies_action.id, &ActionPermissionLevel::User).await?;

  let shown_access_policy = create_configuration_access_policy(&test_environment.database_pool, &dummy_configuration.id, &user.id, &get_access_policies_action.id, &ActionPermissionLevel::Editor).await?;
  ...
  let query = format!("scoped_resource_type = 'Configuration' AND scoped_configuration_id = {} and permission_level = 'Editor'", quote_literal(&dummy_configuration.id.to_string()));
  let actual_access_policy_count = AccessPolicy::count(&query, &test_environment.database_pool, Some(&IndividualPrincipal::User(user.id))).await?;
  assert_eq!(response_access_policies.total_count, actual_access_policy_count);
...
async fn verify_default_access_policy_list_limit() -> Result<(), TestSlashstepServerError> {
  ...
  for _ in 0..(DEFAULT_ACCESS_POLICY_LIST_LIMIT + 1) {
    ...
    create_configuration_access_policy(&test_environment.database_pool, &dummy_configuration.id, &random_user.id, &random_action.id, &ActionPermissionLevel::User).await?;
...
async fn verify_query_when_listing_access_policies() -> Result<(), TestSlashstepServerError> {
  ...
  let bad_requests = vec![
    test_server.get(&format!("/configurations/{}/access-policies", dummy_configuration.id))
...
async fn verify_parent_resource_not_found_when_listing_resources() -> Result<(), TestSlashstepServerError> {
  ...
  let router = super::get_router(state.clone())
    ...
    .into_make_service_with_connect_info::<SocketAddr>();
  let test_server = TestServer::new(router)?;
  let response = test_server.get(&format!("/configurations/{}/access-policies", &Uuid::now_v7()))
    .await;
