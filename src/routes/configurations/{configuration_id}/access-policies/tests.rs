...
async fn create_configuration_access_policy(
    database_pool: &deadpool_postgres::Pool,
    scoped_configuration_id: &Uuid,
    user_id: &Uuid,
    action_id: &Uuid,
...
async fn verify_returned_access_policy_list_without_query() -> Result<(), TestSlashstepServerError>
{
    ...
    let dummy_configuration = test_environment.create_random_configuration().await?;
    let shown_access_policy = create_configuration_access_policy(
    ...
    let query = format!(
        ...
        quote_literal(&dummy_configuration.id.to_string())
    );
...
async fn verify_returned_access_policy_list_with_query() -> Result<(), TestSlashstepServerError> {
    ...
    let dummy_configuration = test_environment.create_random_configuration().await?;
    create_configuration_access_policy(
        &test_environment.database_pool,
        &dummy_configuration.id,
        &user.id,
        &list_access_policies_action.id,
    ...
    let query = format!(
        "scoped_resource_type = 'Configuration' AND scoped_configuration_id = {} and permission_level = 'Editor'",
        quote_literal(&dummy_configuration.id.to_string())
    );
    let actual_access_policy_count = AccessPolicy::count(
        &query,
