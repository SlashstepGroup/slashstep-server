use crate::{
    ...
    utilities::route_handler_utilities::{
        ...
        get_configuration_by_id, get_principal_type_and_id_from_principal,
...
async fn handle_list_access_policies_request(
    ...
    let configuration = get_configuration_by_id(&configuration_id, &state.database_pool).await?;
...
async fn handle_create_access_policy_request(
    ...
    trace!(
        "Creating access policy for configuration {}...",
        configuration_id
    );
...
pub fn get_router(state: AppState) -> Router<AppState> {
    Router::<AppState>::new()
        .route(
            "/configurations/{configuration_id}/access-policies",
