...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'predefined_group_type') THEN
    CREATE TYPE predefined_group_type AS ENUM (
      'AnonymousUsers',
      'RegisteredUsers'
    );
  END IF;
  ...
  CREATE TABLE IF NOT EXISTS groups (
    ...
    predefined_group_type predefined_group_type
  );
