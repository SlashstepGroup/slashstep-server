...
pub async fn get_status_by_id(status_id: &Uuid, http_transaction: &HTTPTransaction, database_pool: &deadpool_postgres::Pool) -> Result<Status, HTTPError> {

  let status = get_resource_by_id::<Status, _>("status", &status_id, &http_transaction, &database_pool, |status_id, database_pool| Box::new(Status::get_by_id(status_id, database_pool))).await?;
  return Ok(status);
