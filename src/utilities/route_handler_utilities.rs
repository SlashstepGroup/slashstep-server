...
use crate::{HTTPError, resources::{ResourceError, ResourceType, access_policy::{AccessPolicy, AccessPolicyPrincipalType, ActionPermissionLevel}, action::Action, action_log_entry::ActionLogEntry, app::App, app_authorization::AppAuthorization, app_authorization_credential::AppAuthorizationCredential, app_credential::AppCredential, configuration::Configuration, delegation_policy::DelegationPolicy, field::Field, field_choice::FieldChoice, field_value::FieldValue, group::Group, http_transaction::HTTPTransaction, item::Item, item_connection::ItemConnection, item_connection_type::ItemConnectionType, item_type::ItemType, item_type_icon::ItemTypeIcon, iteration::Iteration, membership::Membership, membership_invitation::MembershipInvitation, milestone::Milestone, project::Project, role::Role, server_log_entry::ServerLogEntry, session::Session, user::User, view::View, view_field::ViewField, workspace::Workspace}, utilities::slashstepql::SlashstepQLError};
...
pub async fn get_item_type_by_id(item_type_id: &Uuid, http_transaction: &HTTPTransaction, database_pool: &deadpool_postgres::Pool) -> Result<ItemType, HTTPError> {
...
pub async fn get_item_type_icon_by_id(item_type_icon_id: &Uuid, http_transaction: &HTTPTransaction, database_pool: &deadpool_postgres::Pool) -> Result<ItemTypeIcon, HTTPError> {

  let target_item_type_icon = get_resource_by_id::<ItemTypeIcon, _>("item type icon", &item_type_icon_id, &http_transaction, &database_pool, |item_type_icon_id, database_pool| Box::new(ItemTypeIcon::get_by_id(item_type_icon_id, database_pool))).await?;
