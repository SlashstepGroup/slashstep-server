use crate::{
    ...
    resources::{
        ...
        access_policy::{AccessPolicy, AccessPolicyPrincipalType, PermissionLevel},
        action::Action,
...
pub async fn get_action_log_entry_expiration_timestamp(
    ...
    let should_action_log_entries_expire_configuration = match Configuration::list(
        &format!(
            "name = {} LIMIT 1",
            quote_literal("actionLogEntries.shouldExpire")
        ),
        database_pool,
        None,
        None,
    ...
    let should_action_log_entries_expire = should_action_log_entries_expire_configuration
        .boolean_value
        .or(should_action_log_entries_expire_configuration.default_boolean_value)
        .unwrap_or(false);
    ...
    let action_log_entry_expiration_duration_configuration = match Configuration::list(
        &format!(
            "name = {} LIMIT 1",
            quote_literal("actionLogEntries.expirationDurationMilliseconds")
        ),
        database_pool,
        None,
        None,
    ...
    let expiration_duration_milliseconds = action_log_entry_expiration_duration_configuration
        .number_value
        .or(action_log_entry_expiration_duration_configuration.default_number_value)
        .and_then(|decimal| decimal.to_i64());

    let expiration_timestamp = expiration_duration_milliseconds.and_then(|duration| {
        Utc::now().checked_add_signed(chrono::Duration::milliseconds(duration))
    });
...
pub async fn can_delegate_perform_action(
    ...
    let query = format!(
        "action_id = {} AND delegate_app_authorization_id = {} LIMIT 1",
...
pub async fn verify_delegate_permissions(
    ...
    .await?
    {
        return Err(HTTPError::Forbidden(Some(format!(
            "The app authorization {} does not have access to the action {}.",
            app_authorization_id, action_id
        ))));
    }
...
pub async fn can_principal_perform_action(
    ...
    let permission_level_row = match database_client
        .query_one(
            "SELECT get_principal_permission_level($1, $2, $3, $4, $5)",
            &[
                &principal_type,
                &principal_id,
                &resource_type,
    ...
    Ok(&actual_permission_level >= minimum_permission_level)
}

pub async fn verify_principal_permissions(
    principal_type: &AccessPolicyPrincipalType,
    principal_id: &Uuid,
    is_principal_anonymous: bool,
    resource_type: &ResourceType,
    resource_id: Option<&Uuid>,
    action: &Action,
    http_transaction: &HTTPTransaction,
    ...
    .await?
    {
        let location = match resource_type {
            ...
            _ => format!(
                ...
                resource_id.unwrap_or(&Uuid::nil())
            ),
        };
        let message = format!(
...
pub async fn get_uuid_from_string(
    ...
    let uuid = match Uuid::parse_str(string) {
        ...
        Err(_) => {
            let http_error = HTTPError::BadRequest(Some(format!(
...
pub async fn get_item_connection_type_by_id(
    ...
    .await?;
    Ok(target_item_connection_type)
}
...
pub async fn get_membership_invitation_by_id(
    ...
    .await?;
    Ok(target_membership_invitation)
}
...
where
    GetResourceByIDFunction: for<'a> Fn(
        &'a Uuid,
        &'a deadpool_postgres::Pool,
    ) -> Box<
...
pub fn match_slashstepql_error(
    error: &SlashstepQLError,
    maximum_limit: &i64,
    ...
    match error {
        SlashstepQLError::SlashstepQLInvalidLimitError(error) => {
            HTTPError::UnprocessableEntity(Some(format!(
                ...
                maximum_limit, error.limit_string
            )))
        } // TODO: Make this configurable through resource policies.

        SlashstepQLError::InvalidFieldError(field) => {
            HTTPError::UnprocessableEntity(Some(format!(
                "The provided query is invalid. The field \"{}\" is not allowed.",
                field
            )))
        }

        SlashstepQLError::InvalidQueryError(()) => {
            HTTPError::BadRequest(Some("The provided query is invalid.".to_string()))
        }

        _ => HTTPError::InternalServerError(Some(format!(
            "Failed to list {}: {:?}",
            resource_type_plural, error
        ))),
    }
}
...
pub fn get_principal_type_and_id_from_principal(
    ...
    Err(HTTPError::InternalServerError(Some("Couldn't find a user or app for the request. This is a bug. Make sure the authentication middleware is installed and is working properly.".to_string())))
}
...
pub async fn get_configuration_by_name(
    ...
    let configuration = match Configuration::get_by_name(configuration_name, database_pool).await {
        ...
        Err(error) => {
            let http_error = match error {
                ResourceError::NotFoundError(_) => HTTPError::InternalServerError(Some(format!(
                    "Missing configuration for {}. It may have been deleted by a user or an app. Try restarting the server to restore this configuration.",
                    configuration_name
                ))),
...
pub async fn validate_field_length(
    name: &str,
    configuration_name: &str,
    field_name: &str,
    http_transaction: &HTTPTransaction,
...
pub async fn validate_resource_name(
    name: &str,
    configuration_name: &str,
    resource_type_name_singular: &str,
    http_transaction: &HTTPTransaction,
    ...
    let regex = match regex::Regex::new(&allowed_name_regex_string) {
        ...
        Err(error) => {
            let http_error = HTTPError::InternalServerError(Some(format!(
                "Failed to create regex for validating {} names: {:?}",
                resource_type_name_singular.to_lowercase(),
                error
            )));
            http_error.log();
...
pub async fn validate_resource_display_name(
    name: &str,
    configuration_name: &str,
    resource_type_name_singular: &str,
    http_transaction: &HTTPTransaction,
    ...
    let regex = match regex::Regex::new(&allowed_display_name_regex_string) {
        ...
        Err(error) => {
            let http_error = HTTPError::InternalServerError(Some(format!(
                "Failed to create regex for validating {} display names: {:?}",
                resource_type_name_singular.to_lowercase(),
                error
            )));
            http_error.log();
...
pub async fn validate_decimal_is_within_range(
    decimal: &Decimal,
