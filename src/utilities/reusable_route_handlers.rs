  ...
  ListResourcesFunction: for<'a> Fn(&'a str, &'a deadpool_postgres::Pool, Option<&'a IndividualPrincipal>) -> Box<dyn Future<Output = Result<Vec<ResourceType>, ResourceError>> + 'a + Send>
{
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_delegation_policy_id: if action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::DelegationPolicy { action_log_entry_target_resource_id.clone() } else { None },
    ...
    target_item_connection_id: if action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::ItemConnection { action_log_entry_target_resource_id.clone() } else { None },
    target_item_connection_type_id: if action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::ItemConnectionType { action_log_entry_target_resource_id.clone() } else { None },
    target_membership_id: if action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::Membership { action_log_entry_target_resource_id.clone() } else { None },
    target_membership_invitation_id: if action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::MembershipInvitation { action_log_entry_target_resource_id.clone() } else { None },
