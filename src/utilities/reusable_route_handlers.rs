  ...
  ListResourcesFunction: for<'a> Fn(&'a str, &'a deadpool_postgres::Pool, Option<&'a IndividualPrincipal>) -> Box<dyn Future<Output = Result<Vec<ResourceType>, ResourceError>> + 'a + Send>
{
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_default_field_value_id: if action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::DefaultFieldValue { action_log_entry_target_resource_id.clone() } else { None },
  ...
  GetResourceByIDFunction: for<'a> Fn(&'a Uuid, &'a deadpool_postgres::Pool) -> Box<dyn Future<Output = Result<ResourceStruct, ResourceError>> + 'a + Send>
{
  ...
  ActionLogEntry::create(&InitialActionLogEntryProperties {
    ...
    target_default_field_value_id: if *action_log_entry_target_resource_type == ActionLogEntryTargetResourceType::DefaultFieldValue { Some(resource_id.clone()) } else { None },
