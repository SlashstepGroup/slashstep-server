...
impl fmt::Display for SlashstepQLInvalidLimitError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "Invalid limit \"{}\" in filter query. It must be a non-negative integer{}.",
            self.limit_string,
            if let Some(maximum_limit) = self.maximum_limit {
                format!(" and must be less than or equal to {}", maximum_limit)
            } else {
                "".to_string()
            }
        )
...
pub struct SlashstepQLSanitizeFunctionOptions {
    ...
    pub translate_assignment:
        fn(
            SlashstepQLAssignmentProperties,
...
pub fn translate_normal_assignment(
    mut assignment_properties: SlashstepQLAssignmentProperties,
    ...
    let injected_value = if assignment_properties.has_null_value {
        "NULL".to_string()
    } else {
        formatted_value
    };
    assignment_properties.where_clause.push_str(&format!(
        "{} {} {}",
        identifier, assignment_properties.operator, injected_value
    ));

    if let Some(parameterized_value) = assignment_properties.string_value {
        assignment_properties.parameters.push((
            assignment_properties.key,
            SlashstepQLParameterType::String(parameterized_value),
        ));
    } else if let Some(parameterized_value) = assignment_properties.number_value {
        assignment_properties.parameters.push((
            assignment_properties.key,
            SlashstepQLParameterType::Number(parameterized_value),
        ));
    } else if let Some(parameterized_value) = assignment_properties.boolean_value {
        assignment_properties.parameters.push((
            assignment_properties.key,
            SlashstepQLParameterType::Boolean(parameterized_value),
        ));
    }
...
impl SlashstepQLFilterSanitizer {
    pub fn sanitize(
        options: &SlashstepQLSanitizeFunctionOptions,
    ) -> Result<SlashstepQLSanitizedFilter, SlashstepQLError> {
            ...
            if let Some(regex_captures) = regex_captures {
                if regex_captures.name("openParenthesis").is_some() {
                    ...
                    if open_parenthesis_count == 0 {
                        return Err(SlashstepQLError::InvalidFilterSyntaxError(
                            "Unexpected closed parenthesis in filter query.".to_string(),
                        ));
                    }
                        ...
                        .and_then(|string_match| Some(string_match.as_str().to_string()))
                    {
                        ...
                        let assignment_translation_result =
                            (options.translate_assignment)(assignment_properties)?;
                        ...
                        if let Ok(new_limit) = limit_string.as_str().parse::<i64>() {
                            if let Some(maximum_limit) = maximum_limit_result {
                                if new_limit > maximum_limit || new_limit < 0 {
                                    ...
                                    return Err(SlashstepQLError::SlashstepQLInvalidLimitError(
                                        error,
                                    ));
                                }
                            } else {
                    ...
                    if let Some(offset_string) = regex_captures.name("offset") {
                        if let Ok(new_offset) = offset_string.as_str().parse::<i64>() {
                            ...
                            return Err(SlashstepQLError::InvalidOffsetError(format!(
                                "Invalid offset \"{}\" in filter query. It must be a non-negative integer.",
                                offset_string.as_str()
                            )));
                        }
                    }
                } else {
    ...
    pub fn build_query_from_sanitized_filter(
        ...
        let where_clause = sanitized_filter
            .where_clause
            .clone()
            .unwrap_or("".to_string());
        let where_clause = match principal_type {
            Some(principal_type) => {
                ...
                let additional_condition = format!(
                    "get_principal_permission_level({}, {}, {}, {}.id, {}) >= 'User'",
                    quote_literal(&principal_type.to_string()),
                    quote_literal(&principal_id.to_string()),
...
pub fn add_parameter_to_query<T: ToSql + Sync + Clone + Send + 'static>(
    mut parameter_boxes: Vec<Box<dyn ToSql + Sync + Send>>,
    mut query: String,
    key: &str,
